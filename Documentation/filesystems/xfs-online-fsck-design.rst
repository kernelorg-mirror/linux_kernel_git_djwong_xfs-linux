...
..
        ...
        Sections are manually numbered because apparently that's what we do
        in the kernel.
...
This design document is split into seven sections.
The first section defines what fsck tools are and the motivations for writing a
new one.
Sections two and three present a high level overview of how online fsck
process works and how it is tested to ensure correct functionality.
The fourth section discusses the user interface and the intended usage modes of
the program.
Sections five and six first present the high level components and how they fit
together, and then present case studies of how each repair function actually
works.
The seventh section sums up what has been discussed so far and speculates about
what else might be built atop online fsck.
...
**Question**: Can we use the full scan and live update code used to facilitate
a repair be used to implement a comprehensive check?
...
255-byte sequence (name) to an inode number.
...
name with an inode number and, optionally, a file type.
...
2. Walk the blocks of the first partition looking for directory entries.
   ...
   b. Does the inode number correspond to an actual, allocated inode?
...
``xfs_scrub`` finishes repairing and revalidating the metadata.

For repairs going on within a shard of the filesystem, these properties
overcome the delays inherent in locking the shard while repairing parts of the
shard.
Unfortunately, repairs to the reverse mapping btree cannot use the "standard"
btree repair strategy because it must scan every file in the filesystem without
incurring extended downtime.
To avoid these delays, this class of repair relaxes the atomicity property by
permitting the filesystem to continue running while the repair function walks
the entire filesystem.
A scan of a live filesystem requires the ability to propagate metadata updates
from the filesystem into the data being collected by the scan.
Because xfarrays do not enforce record ordering, they are unsuitable for
performing indexed lookups or record removals of the new rmap records.
Conveniently, XFS already has a library to create and maintain ordered reverse
...
Therefore, in-memory btrees adapt the XFS buffer cache to direct its operations
to an xfile instead of a block device.
...
The next few sections describe how xfile btrees (``xfbtree``) actually works.
...
per-AG structure.
The second change is to modify the ``ioapply`` function to "read" cached
...
In other wrods, users of the xfile-backed buffer cache use exactly the same
APIs as users of the disk-backed buffer cache.
...
The very first block an xfile backing an xfbtree is a header block.
...
fallocate it, and hand it back.
If no holes are found, extend the length of the xfile by one page, fallocate
it, and hand it back.
When an xfbtree frees a btree block, it must use ``xfile_discard`` (which
internally uses ``FALLOC_FL_PUNCH_HOLE``) to remove the memory page from the
xfile.
...
1. Create the underlying xfile.

2. Create a buffer cache target structure (``struct xfs_buftarg``) pointing to
   the xfile.

3. Pass the buffer cache target to ``xfbtree_create`` to write an initial
   tree header and root block to the xfile.
...
5. Pass the btree cursor to the regular rmap functions to make queries against
   and to update the in-memory rmap btree.
...
by an xfile because the AIL checkpoints logged transactions into the ondisk
filesystem asynchronously.
The xfile may have been torn down completely by the time the AIL runs.
Furthermore, leaving unusual log items attached to transactions could result in
transaction overruns and ephemeral data incorrectly being written to the ondisk
log, which will cause a lot of confusion for the AIL and log recovery.

Users of in-memory btrees must perform the following prior to committing a
transaction:
...
6. Submit the delwri list to commit the changes to the xfile, if desired.
...
The first step of bulk loading is to call ``xfs_btree_bload_compute_geometry``
to compute the shape of the btree from the record set, the type of btree, and
any load factor preferences.
...
needs one block.
This level is the root level, so the height of the new tree is known, and the
number of blocks needed for the new tree is the summation of the number of
blocks per level.
...
its in-memory object to the space reservation.
If the system goes down, log recovery will use the unfinished EFIs to free the
unused space, the free space, leaving the filesystem untouched.
...
rest of the block with records, and adds the block to a list of written blocks.
Sibling pointers are set every time a new block is added to the level.
When it finishes writing the record blocks, it moves on to the nodes.
...
recently, so the builder must remove the (stale) buffer from the AIL list
before it can write the new blocks to disk.
Blocks are queued for IO using a delwri list.

Once the new blocks have been persisted to disk, online repair needs to log the
location of the new root to a transaction.
Repair also cleans up the space reservations by walking the incore reservation
list and to create Extent Freeing Done (EFD) items for all the space that was
consumed by the btree builder.
Unclaimed reservations are converted to regular deferred extent free work items
to be freed after the new structure is committed.
The EFDs logged to the committing transaction must not overrun it.
Finally, the transaction is committed to cement the new btree into the
filesystem.
Repair moves on to reaping the old blocks, which will be discussed after a few
case studies.

Case Study: Rebuilding the Inode Index
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

To rebuild the inode index btree:
...
6. Commit the new btree root block to the AGI.
...
For the space represented by each inode cluster, first ensure that there are no
...
numbers in the inode number keyspace.
...
To rebuild the reference count btree:
...
5. Use ``xfs_btree_bload`` to write the xfarray records to btree blocks and
   generate the internal node blocks.
6. Commit the new btree root block to the AGF.
7. Reap the old btree blocks using the bitmap created in step 1.
...
the free space btree records which blocks are free.
...
(``np``), reference count information is reconstructed as follows:
...
The bag-like structure in this case is a type 3 xfarray as discussed in the
...
To rebuild a data/attr fork mapping structure:
...
1. Walk the reverse mapping records to generate ``struct xfs_bmbt_rec``
   ...
   Compute the bitmap of the old bmap btree blocks.
...
suspect, there is a question of how to dispose of the blocks that allegedly
...
Freeing all of the blocks that repair *thinks* compose an old data structure is
...
To rebuild the free space index btree:
...
The second complication is much more serious: here, repair cannot use the
common btree reservation code because blocks are reserved out of the free space
btrees.
...
More complicated are the data and attr fork extent counts and the file block
counts, which require traversing the forks, or leaving the fields invalid
and waiting for the fork fsck functions to run.

The proposed patchsets is the
`inode
<https://git.kernel.org/pub/scm/linux/kernel/git/djwong/xfs-linux.git/log/?h=repair-inodes>`_
repair series.
...
Quota usage counters are checked and repaired separately by
:ref:`live quotacheck <quotacheck>`.
...
1. ``xrep_tempfile_copyin_xfile`` can be used to set the contents of a regular
   temporary file from an xfile.

2. The regular directory, symbolic link, and extended attribute functions can
   be used to write non-regular files.
...
Once repair has built a temporary file with a new data structure written into
it, it must commit the new changes into the existing file in such a way that
log recovery can finish the operation if the system goes down.
To satisfy that requirement, it was necessary to create a deferred operation
and a new type of log intent item so that the log can track the progress of the
...
3. Until ``sxi_blockcount`` of the deferred extent swap work item is zero,
   ...
   k. Log an extent swap done log item for the extent swap intent log item
      that was logged just prior to the start of step 3.
   l. Log a new extent swap intent log item reflecting the advanced state of
      the work item.
...
The filesystem must never be run completely out of free space, nor can the
extent swap ever add more extent mappings to a fork than it can support.
...
referential integrity, so prior to performing the extent swap, walk every block
in the new data structure to update the owner field and flush the buffer to
disk.
...
Online repair functions should stage whatever metadata they want in the scrub
temporary file and commit the scrub transaction.
Next, call ``xrep_tempswap_trans_alloc`` to allocate a new scrub transaction
with the appropriate resource reservations, locks, and fill out a ``struct
xfs_swapext_req`` with the details of the swap operation.
Finally, call ``xrep_tempswap_contents`` to swap the contents and commit the
repair.
...
1. Lock the realtime bitmap and summary files.
...
When the incore summary is complete, compare the contents of the two files
...
This section presents the key algorithms and data structures employed by the
driver program, ``xfs_scrub``, to check and repair metadata, verify file data,
and look for other potential problems.
...
smaller files because the inode scan dispatch function was not sufficiently
granular.
...
phase 2 before trying the work scheduled by phase 3.
...
The flexibility of Unicode comes at a cost -- malicious actors can create names
that can be used to mislead users.
...
mapping ioctl to map the recorded media errors back to the metadata structures
or files that own the space, and report what has been lost.
...
uses the new fallocate call to map any free space in that region to the space
collector.
Next, it find all metadata blocks in that region by way of ``GETFSMAP``,
...
cleared.
After each relocation, the program should call the "map free space" function
again to collect any newly freed space in the region being cleared.

To clear all the file data out of a portion of the physical storage,
``clearspace`` uses the FSMAP information to find relevant file data blocks.
Having identified a good target, it should use the ``FICLONERANGE`` call on
that file to try to share the physical space with a dummy file.
...
``Clearspace`` can then make its own copy of the frozen extent in an area that
is not being cleared, and use ``FIEDEUPRANGE`` (or the
:ref:`atomic extent swap <swapext_if_unchanged>` feature) to change the target
file's data extent mapping away from the area being cleared.
When all other mappings have been moved, reflink the space into the space
collector file so that it's permanently unavailable.
...
In fact, these chunks should be moved first to maximize sharing factor after
the operation completes.
To make this work smoothly, add a new ioctl (``FS_IOC_GETREFCOUNTS``) to report
reference count information to userspace.
With the refcount information exposed, ``clearspace`` can quickly find the
longest, most shared data extents in the filesystem, and target them first.
...
This would eliminate the overhead until a revocation happens.

...
Online fsck for XFS has been designed to separate the regular filesystem from
the checking and repair code as much as possible.
However, there are a few parts of fsck (such as the intent drains, and later,
live update hooks) where it would be useful for the fsck code to know what's
going on in the rest of the filesystem.
Since it is not expected that online fsck will be constantly running in the
background, we would like to minimize the runtime overhead imposed by these
hooks when online fsck is compiled into the kernel but not actively running on
behalf of userspace.

Fortunately, the kernel supports dynamic code patching, which enables us to
replace static branches to hook code with nop sleds when online fsck isn't
running.
Taking locks on notifier chains and workqueues just to find out that online
fsck isn't running has (on the author's computer) an overhead of 40-50ns.
Static keys move that hook code out of the main execution stream of the calling
function and add a ``nop`` sled in its place.
...
enter online fsck at the same time, or multiple filesystems are being checked
at the same time.
...
Concretely, this means that fsck functions must enable the static key in the
scrub setup function, and the scrub dispatch code will disable the static key
on the way out.

For more information, please see Documentation/staging/static-keys.rst.
...
3. Large binary objects (BLOBs) of variable sizes (directory and extended
   attribute repairs)
4. Bitmaps (realtime space management)
5. Staging btrees in memory (reverse mapping btrees)

To support the first four use cases, high level data structures have been
created to wrap the xfile to share functionality between online fsck functions.
The rest of this section discusses the interface that the xfile presents to
those five higher level data structures.
The fourth use case will be discussed in the :ref:`realtime summary <rtsummary>`
use case, and the fifth use case will be discussed in the :ref:`in-memory btree
<xfbtree>` section.

The most general storage interface supported by the xfile allows the reading
and writing of arbitrary quantities of data at arbitrary offsets in the xfile.
This capability is exposed by way of ``xfile_pread`` and ``xfile_pwrite``
functions, which behave similarly to their userspace counterparts.
However, XFS is very record-based, which suggests that the ability to load and
store complete records is important.
...
All five types of xfile usecases can be serviced by these four functions.

However, no discussion of file access idioms is complete without answering the
question, "But what about mmap?"
It would be *much* more convenient if we could access pageable kernel memory
with pointers, just like we do with regular memory.
...
For security reasons, xfiles are owned privately by the kernel.
They are marked private to prevent interference from the security system,
they must never be mapped into process file descriptor tables,
and their pages must never be mapped into userspace processes.
...
xfile writes are performed by calling the ``->write_begin`` and ``->write_end``
functions of the xfile's address space to grab writable pages, copy the
caller's buffer into the page, and release the pages.
xfile reads are performed by calling ``shmem_read_mapping_page_gfp`` to grab
pages directly and copy the contents into the caller's buffer.
...
Although this requirement could be satisfied by calling the read and write
methods of the xfile directly, it would be simpler for callers if there was
a higher level abstraction to take care of computing array offsets, provide
...
Gaps are defined to be null records where all bytes are zeroes.
They can be detected by calling ``xfarray_element_is_null``, and are created
either by calling ``xfarray_unset`` to null out an existing record or by never
storing anything to the array index.
...
at any time, and uniqueness of records is not enforced.
The ``xfarray_store_anywhere`` function is used to insert a record in any
null record slot; and the ``xfarray_unset`` function removes a record from
the bag.
...
Note that any users of such an idiom must be prepared to handle null records or
know that there aren't any.

For xfarray users that want to iterate a sparse array, the ``xfarray_iter``
function ignores indices in the xfarray that have never been written by using
``xfile_seek_data`` (which internally uses ``SEEK_DATA``) to skip areas of the
array that are not populated with memory pages.
Once it finds a page, it will skip the zeroed areas of the page.

.. code-block:: c

	i = XFARRAY_CURSOR_INIT;
...
During the next review iteration after the creation of the xfarray, a community
...
Later, callers provide this cookie to the load and free methods to find the
object.
The ``xfblob_free`` function frees one blob, and the ``xfblob_truncate``
...
To rebuild the inode index btree, we:
...
Reverse mapping records with an owner of ``XFS_RMAP_OWN_INOBT`` tell us the
...
Once we have accumulated one chunk's worth of data, we call ``xfarray_append``
...
To rebuild the reference count btree, we:
...
To rebuild a data/attr fork mapping structure, we:
...
1. Walk the reverse mapping records to generate ``struct xfs_bmbt_rec``
   ...
   We also compute the bitmap of the old bmap btree blocks.
...
To rebuild the free space index btree, we:
...
Inode records (dinodes) must be handled carefully, because they have both
ondisk metadata and an in-memory ("cached") representation.
...
The only repairs we want to make to the ondisk metadata is whatever is
...
Similar to inode records, quota records (dquots) also have both ondisk metadata
and an in-memory ("cached") representation, and hence are subject to the same
cache coherency issues.

Quota records have fewer attributes than inodes, so we only need to repair the
ondisk metadata that gets tripped up by the dquote cluster buffer verifier.
...
:ref:`live quotacheck <_quotacheck>`.
...
Multiple access to xfile data is controlled by the buffer locks.
...
8. Allocate that many blocks, like all the other btree rebuilders.
...
To check the summary file against the bitmap, we:

...
Most primary repair functions stage their intermediate results in an in-memory
array prior to formatting the new ondisk structure, which is very similar to
the list-based algorithm discussed in section 2.3 ("List-Based Algorithms") of
Srinivasan.
...
Resource and lock acquisition for scrub code do not follow the same order the
same paths as regular filesystem access, and may involve full filesystem scans.
...
Online fsck is not necessarily a universally desirable feature for all use
cases.
Users who demand maximum performance at any cost, including repairability,
are not going to want to pay any overhead associated with this feature.
To that end, the reverse mapping space index and directory parent pointers can
be disabled at format time.
Distributors who do not wish to support this feature can disable it from the
kernel build with Kconfig, because the online fsck implementation is largely
self-contained.
Major functionality in the main filesystem that only exist to serve scrub are
disabled when online fsck is not built.
The userspace driver program and its associated background ``xfs_scrub`` can
also be stripped from the xfsprogs build.
...
Finally, the flags field provides extra information about the space usage usage
...
1. Add a higher level lock to allocation groups and require writer threads to
   ...
   This would be very difficult to implement in practice because we don't know
...
encountered during the walk would leave the filesystem with a shut down
filesystem containing a half-rebuilt data structure.
...
A survey of the intended uses of xfiles suggested three main use cases:
...
Choosing a quicksport pivot is known to be tricky business.
...
the repair succeeded, and the new btree blocks if the system went down.
...
as being owned by ``XFS_RMAP_OWN_INOBT`` but cannot be found by visiting each
...
3. If there are no more inodes left in this AG:
   ...
   b. Adjust the scanned inode cursor to indicate that we have "scanned" the
      ...
      Recall that XFS inode numbers are segmented, so we need to mark the as
...
After removing xfile logged buffers from the transaction, the transaction be
committed.

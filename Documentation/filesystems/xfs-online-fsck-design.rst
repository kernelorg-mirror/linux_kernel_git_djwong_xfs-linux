...
**Future Work Question**: How will repair ensure that the ``dirent_pos`` fields
...
**Future Work Question**: Can static keys be used to add a revoke bailout
return to *every* code path coming in from userspace?
Could we finally support revoke() in XFS?

...
``swap_file_range``
	called by the ioctl(2) system call for FISWAPRANGE to exchange the
	contents of two file ranges.  An implementation should exchange
	fsr.length bytes starting at fsr.file1_offset in file1 with the same
	number of bytes starting at fsr.file2_offset in file2.  Refer to
	swaprange.h file for more information.  Implementations must call
	generic_swap_file_range_prep to prepare the two files prior to taking
	locks; they must call generic_swap_file_range_check_fresh once the

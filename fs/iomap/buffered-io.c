		...
		struct folio *folio)
{
	...
	if (WARN_ON_ONCE(size > iomap->length)) {
		fserror_report_io(iter->inode, GFP_NOFS, FSERR_BUFFERED_READ,
				  iomap->offset, size, -EIO);
		return -EIO;
	}
	if (offset > 0)
		...
		size_t len, int error)
{
	...
	if (error)
		fserror_report_io(folio->mapping->host, GFP_ATOMIC,
				  FSERR_BUFFERED_READ, folio_pos(folio) + off,
				  len, error);
		...
		struct folio *folio, loff_t pos, size_t len)
{
	...
	if (ret)
		fserror_report_io(iter->inode, GFP_NOFS, FSERR_BUFFERED_WRITE,
				  pos, len, ret);
	return ret;
}
#else
...
int iomap_writeback_folio(struct iomap_writepage_ctx *wpc, struct folio *folio)
{
	...
	if (error && pos > orig_pos)
		fserror_report_io(inode, GFP_NOFS, FSERR_BUFFERED_WRITE,
				  orig_pos, 0, error);

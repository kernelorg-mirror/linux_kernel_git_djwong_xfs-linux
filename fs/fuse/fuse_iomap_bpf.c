...
fuse_bpf_inode_orig_ino(const struct fuse_bpf_inode *fbi)
{
	...
	return fi ? fi->orig_ino : 0;
}

__bpf_kfunc int
fuse_bpf_iomap_inval_mappings(struct fuse_bpf_inode *fbi,
			      const struct fuse_range *read,
			      const struct fuse_range *write)
{
	...
	if (read)
		memcpy(&outarg.read, read, sizeof(outarg.read));
	if (write)
		memcpy(&outarg.write, write, sizeof(outarg.write));
...
fuse_bpf_iomap_upsert_mappings(struct fuse_bpf_inode *fbi,
			       const struct fuse_iomap_io *read,
			       const struct fuse_iomap_io *write)
{
	struct fuse_inode *fi = fuse_inode_from_bpf(fbi);
	if (!fi)
		return -EBADF;
...
BTF_ID_FLAGS(func, fuse_bpf_inode_nodeid)
BTF_ID_FLAGS(func, fuse_bpf_inode_orig_ino)
BTF_ID_FLAGS(func, fuse_bpf_iomap_inval_mappings, KF_SLEEPABLE)
BTF_ID_FLAGS(func, fuse_bpf_iomap_upsert_mappings, KF_SLEEPABLE)

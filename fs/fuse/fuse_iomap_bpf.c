...
static int __iomap_begin(uint64_t nodeid, uint64_t pos, uint64_t count,
			 uint32_t opflags, struct fuse_iomap_begin_out *outarg)
{
	return -ENOSYS;
}

static int __iomap_end(uint64_t nodeid, uint64_t pos, uint64_t count,
		       int64_t written, uint32_t opflags)
{
	return -ENOSYS;
}

static int __iomap_ioend(uint64_t nodeid, uint64_t pos, int64_t written,

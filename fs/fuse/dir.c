...
void fuse_epoch_work(struct work_struct *work)
{
	...
	inode = fuse_ilookup(fc, fc->root_nodeid, &fm);
	if (inode) {
		iput(inode);
		      ...
		      struct dentry *entry, umode_t mode, dev_t rdev)
{
	...
	args.in_args[1].size = entry->d_name.len + 1;
	args.in_args[1].value = entry->d_name.name;
	err = create_new_nondir(idmap, fm, &args, dir, entry, mode,
				default_acl, acl);
	posix_acl_release(default_acl);
	posix_acl_release(acl);
	return err;
				 ...
				 struct dentry *entry, umode_t mode)
{
	...
	args.in_args[1].size = entry->d_name.len + 1;
	args.in_args[1].value = entry->d_name.name;
	ret = create_new_entry(idmap, fm, &args, dir, entry, S_IFDIR,
			       default_acl, acl);
	posix_acl_release(default_acl);
	posix_acl_release(acl);
	return ret;
			...
			struct dentry *entry, const char *link)
{
	...
	err = create_new_nondir(idmap, fm, &args, dir, entry, S_IFLNK,
				default_acl, acl);
	posix_acl_release(default_acl);
	posix_acl_release(acl);
	return err;
		    ...
		    struct iattr *attr, struct file *file)
{
	...
	if (fc->handle_killpriv_v2 && !S_ISDIR(inode->i_mode) &&
	    attr->ia_valid & (ATTR_UID | ATTR_GID) && !is_iomap)
	...
	if (attr->ia_valid & ATTR_SIZE) {
		...
		if (fc->handle_killpriv_v2 && !is_iomap && !capable(CAP_FSETID))
			inarg.valid |= FATTR_KILL_SUIDGID;
	}

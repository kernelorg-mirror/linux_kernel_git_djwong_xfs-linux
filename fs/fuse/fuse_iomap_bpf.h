...
struct fuse_iomap_bpf_ops {
	 ...
	 * details.
	 */
	enum fuse_iomap_bpf_ret (*iomap_begin)(uint64_t nodeid, uint64_t pos,
	 ...
	 * details.
	 */
	enum fuse_iomap_bpf_ret (*iomap_end)(uint64_t nodeid, uint64_t pos,
			uint64_t count, int64_t written, uint32_t opflags);

	/**
	 * @iomap_ioend: override iomap_ioend.  See FUSE_IOMAP_IOEND for
	 * details.
	 */
	enum fuse_iomap_bpf_ret (*iomap_ioend)(uint64_t nodeid, uint64_t pos,
			int64_t written, uint32_t ioendflags, int error,
	...
	/* private: don't show to the world */
	struct fuse_conn *fc;
};
...
# define fuse_iomap_detach_bpf(...)	((void)0)

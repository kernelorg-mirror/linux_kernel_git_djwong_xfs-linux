...
static int fuse_open(struct inode *inode, struct file *file)
{
	...
	bool is_wb_truncate = is_truncate && (fuse_has_iomap_pagecache(inode) ||
					      fc->writeback_cache);
...
static int fuse_flush(struct file *file, fl_owner_t id)
{
	...
	if ((ff->open_flags & FOPEN_NOFLUSH) &&
	    ...
	    !fuse_has_iomap_pagecache(inode))
		return 0;
...
inval_attr_out:
	 ...
	 */
	if (!err && (fuse_has_iomap_pagecache(inode) || fm->fc->writeback_cache))
		...
		loff_t pos, unsigned len, struct folio **foliop, void **fsdata)
{
	...
	WARN_ON(fuse_has_iomap_pagecache(mapping->host));
				      ...
				      size_t len, unsigned int flags)
{
	...
	bool is_unstable = (!fc->writeback_cache &&
			    !fuse_has_iomap_pagecache(inode_out)) &&
	...
	if (fuse_has_iomap_pagecache(inode_out))

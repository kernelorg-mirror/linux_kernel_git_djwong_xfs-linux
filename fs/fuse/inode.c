				     ...
				     u64 attr_version, u64 evict_ctr)
{
	...
	if (cache_mask & STATX_ATIME) {
		attr->atime = inode_get_atime_sec(inode);
		attr->atimensec = inode_get_atime_nsec(inode);
	}
...
static int fuse_parse_param(struct fs_context *fsc, struct fs_parameter *param)
{
	...
	case OPT_ROOT_NODEID:
		if (result.uint_64 == 0)
			return invalfc(fsc, "root_nodeid cannot be zero");
...
static struct fuse_init_args *fuse_new_init(struct fuse_mount *fm)
{
	...
	/*
	 * iomap requires that all nodeids share the same struct inode so that

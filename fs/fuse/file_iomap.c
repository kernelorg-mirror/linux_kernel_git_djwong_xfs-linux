	...
	uint64_t			map_length)
{
	...
	if (map_addr != FUSE_IOMAP_NULL_ADDR &&
	    BAD_DATA(check_add_overflow(map_addr, map_length, &map_end)))
		return -EIO;
		      ...
		      const struct fuse_iomap_upsert_out *outarg)
{
	...
	/*
	 * The unlocked check here is safe because we hold LOCK_EXCL, the bit
	 * is not cleared until eviction, and the consequence for the only
	 * other reader seeing a stale 0 is a cache miss.
	 */
	if (!(fi->state & FUSE_I_IOMAP_CACHE))

...
TRACE_DEFINE_ENUM(FUSE_I_IOMAP_ATOMIC);
...
#define FUSE_IFLAG_STRINGS \
	...
	{ 1 << FUSE_I_IOMAP_ATOMIC,		"iomap_atomic" }
...
TRACE_EVENT(fuse_iomap_dev_class,
	...
	TP_fast_assign(
		...
		if (iodev) {
			struct inode *inode = file_inode(iodev->file);
...
TRACE_EVENT(fuse_iomap_submit_ioend,
	...
	TP_fast_assign(
		FUSE_INODE_ASSIGN(wpc->ioend->io_inode, fi, fm);
		__entry->nr_folios	=	wpc->nr_folios;
		__entry->offset		=	wpc->ioend->io_offset;
		__entry->length		=	wpc->ioend->io_size;

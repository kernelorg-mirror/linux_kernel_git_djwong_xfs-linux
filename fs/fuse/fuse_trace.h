...
DECLARE_EVENT_CLASS(fuse_iomap_file_ioend_class,
	...
	TP_printk(FUSE_IO_RANGE_FMT() " bytes 0x%zx error %d",
		  FUSE_IO_RANGE_PRINTK_ARGS(),
		  __entry->bytes,
		  __entry->error)
)
...
TRACE_EVENT(fuse_iomap_end_ioend,
	TP_PROTO(const struct iomap_ioend *ioend, int error),
...
TRACE_EVENT(fuse_iomap_writeback_range_end,
	TP_PROTO(const struct inode *inode, u64 offset, unsigned int count,
		 u64 end_pos, ssize_t ret),

	TP_ARGS(inode, offset, count, end_pos, ret),
	...
	TP_printk(FUSE_IO_RANGE_FMT() " end_pos 0x%llx bytes 0x%zx error %d",
		  FUSE_IO_RANGE_PRINTK_ARGS(),
		  __entry->end_pos,
		  __entry->bytes,
		  __entry->error)
...
TRACE_EVENT(fuse_iomap_writepages,
	...
	TP_fast_assign(
		...
		if (wbc->range_end == LLONG_MAX)
			__entry->length	=	i_size_read(inode) - wbc->range_start;
		else
...
DECLARE_EVENT_CLASS(fuse_iomap_inline_class,
	...
	TP_STRUCT__entry(
		FUSE_IO_RANGE_FIELDS()
		__field(uint64_t,		mapoffset)
		__field(uint64_t,		maplength)
	...
	TP_printk(FUSE_IO_RANGE_FMT() "offset 0x%llx length 0x%llx has_buf? %d cookie 0x%llx",
		  FUSE_IO_RANGE_PRINTK_ARGS(),
		  __entry->mapoffset,
		  __entry->maplength,
		  __entry->has_buf,
...
TRACE_EVENT(fuse_iomap_config,
	...
	TP_printk("connection %u root_ino 0x%llx flags (%s) blocksize 0x%x max_links %u time_gran %u time_min %lld time_max %lld maxbytes 0x%llx uuid_len %u cache_maxbytes %u",
		  ...
		  __print_flags_u64(__entry->flags, "|", FUSE_IOMAP_CONFIG_STRINGS),
...
TRACE_EVENT(fuse_iomap_backing_inval,
	TP_PROTO(const struct fuse_conn *fc,
...
DECLARE_EVENT_CLASS(fuse_iext_update_class,
...
DECLARE_EVENT_CLASS(fuse_iext_alt_update_class,
	TP_PROTO(const struct inode *inode, const struct fuse_iomap_io *map),
	TP_ARGS(inode, map),
...
TRACE_EVENT(fuse_iomap_cache_cleanup,
	...
	TP_printk(FUSE_INODE_FMT FUSE_IOMAP_IODIR_FMT " bytes 0x%llx",
		  FUSE_INODE_PRINTK_ARGS,
...
DECLARE_EVENT_CLASS(fuse_iomap_cached_mapping_class,
	TP_PROTO(const struct inode *inode, unsigned int iodir,

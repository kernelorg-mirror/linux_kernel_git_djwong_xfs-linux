	...
	void				*data)
{
	...
	const xfs_agblock_t		notify_end =
			notify->startblock + notify->blockcount;
	...
	if (notify->startblock > rec->rm_startblock) {
		fileoff += notify->startblock - rec->rm_startblock;
		blocks -= notify->startblock - rec->rm_startblock;
	}
	if (rmap_end > notify_end)
		blocks -= rmap_end - notify_end;
	...
	u64			bblen)
{
	...
	while ((xg = xfs_group_next_range(mp, xg, start_gno, end_gno, type))) {
		...
		struct xfs_rmap_irec	ri_low = { };
		struct xfs_rmap_irec	ri_high;
		struct xfs_failure_info	notify = { };
		...
		error = xfs_rmap_query_range(cur, &ri_low, &ri_high,
				xfs_report_one_data_loss, &notify);
		xfs_btree_del_cursor(cur, error);
		if (agf_bp)
			xfs_trans_brelse(tp, agf_bp);
		if (rtg)
	...
	struct xfs_media_error __user	*arg)
{
	...
	if (xfs_has_rmapbt(mp) && type != -1)
		return xfs_report_data_loss(mp, type, me.daddr, me.bbcount);

	...
	bool				pre_remove)
{
	with_xfs_healthmon(mp, hmon) {
		...
		xfs_healthmon_report_media(hmon, &p);
	}
}
	...
	void				*data)
{
	...
	fserror_report_data_lost(VFS_I(ip), GFP_NOFS, (u64)pgoff << PAGE_SHIFT,
			(u64)pgcnt << PAGE_SHIFT);
	...
	int			mf_flags)
{
	...
	xfs_report_media_error(mp, XFS_FAILED_LOGDEV, daddr, bblen,
	...
	enum xfs_group_type	type)
{
	...
	xfs_report_media_error(mp, type == XG_TYPE_RTG ?
	...
	void				*data)
{
	...
	fserror_report_data_lost(VFS_I(ip), GFP_NOFS, XFS_FSB_TO_B(mp, fileoff),
	...
	struct xfs_media_error __user	*arg)
{
	...
	xfs_report_media_error(mp, fdev, me.daddr, me.bbcount, false);

	...
	unsigned int			new_mask)
{
	struct xfs_healthmon		*hm =
		new_mask ? xfs_healthmon_get(mp) : NULL;
	...
	if (hm) {
		...
		xfs_healthmon_report_metadata(hm, &p);
...
{
	struct xfs_healthmon		*hm =
		new_mask ? xfs_healthmon_get(xg->xg_mount) : NULL;
...
{
	struct xfs_healthmon		*hm =
		new_mask ? xfs_healthmon_get(ip->i_mount) : NULL;

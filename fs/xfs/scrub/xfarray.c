...
#define XFARRAY_MAX_FOLIOSORT	(524288)	/* 512k */

/*
 * Sort a bunch of records that all live in the same memory folio.  Returns
 * 1 if it sorted, 0 if it did not, or a negative errno.
 */
STATIC int
xfarray_foliosort(
	struct xfarray_sortinfo	*si,
	...
	xfarray_idx_t		hi)
{
	...
	/* Partition more aggressively to avoid large memory accesses. */
	if (len > XFARRAY_MAX_FOLIOSORT)
		return 0;
	...
	/* Folio does not cover the range we want. */
	if (folio_pos(folio) > lo_pos ||
	    folio_pos(folio) + folio_size(folio) < lo_pos + len) {
	...
	unsigned int		flags)
{
	...
	while (si->stack_depth >= 0) {
		...
		/*
		 * If directly mapping the xfile and sorting can solve our
		 * problems, we're done.
		 */

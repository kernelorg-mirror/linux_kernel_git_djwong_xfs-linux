	...
	size_t		obj_size)
{
	...
	array->nr = 0;
	array->max_nr = div_u64(MAX_LFS_FILESIZE, obj_size);
	return array;
out_xfile:
	xfile_destroy(xfile);
	...
	void		*ptr)
{
	...
	/*
	 * We assume that array elements are either fully written or are null.
	 * Therefore, it is safe to use SEEK_DATA on the first byte of the
	 * array element to turn reading from a hole into a simple memset
	 * operation, which means we skip a lot of unnecessary work on sparse
	 * arrays.
	 */
	pos = xfbma_offset(array, idx);
	offset = xfile_seek_data(array->xfile, pos);
	if (pos != offset) {
		memset(ptr, 0, array->obj_size);
		return 0;
	}

	return xfile_pread(array->xfile, ptr, array->obj_size, pos);
}
	...
	void		*priv)
{
	...
	for (pos = 0, i = 0; pos < max_bytes; i++) {
		...
		/*
		 * Skip holes.  We know that array elements are either fully
		 * written or not, so SEEK_DATA dropping us off in the middle
		 * of a record means that we should round up to the next full
		 * element.
		 */
		offset = xfile_seek_data(array->xfile, pos);
		if (offset == -ENXIO)
		...
		if (offset < 0)
			return offset;
		if (offset > pos)
			pos = roundup_64(offset, array->obj_size);

		error = xfile_pread(array->xfile, temp, array->obj_size, pos);
		if (error)
			break;
...
next:
		...
		if (pagenr != oldpagenr)
			xfile_discard(array->xfile, oldpagenr << PAGE_SHIFT,
					pos - 1);
		oldpagenr = pagenr;
		pos += array->obj_size;
	}
...
/*
 * Fetch the next non-null array entry and advance the index to set up the next
 * get_advance call.  Returns ENODATA if we reach the end of the array.
 */
int
xfbma_iter_get(
	struct xfbma	*array,
	...
	void		*rec)
{
	...
	do {
		/*
		 * If we SEEK_DATA on a hole in the array file, the lseek
		 * call will return the address of the next written address in
		 * the file.  Use this property to skip empty parts of the
		 * array.  ENXIO (i.e. EOF) means there isn't any more data.
		 */
		offset = xfile_seek_data(array->xfile, cur * array->obj_size);
		if (offset == -ENXIO)
			return -ENODATA;
		if (offset < 0)
			return offset;

		cur = div_u64(offset, array->obj_size);

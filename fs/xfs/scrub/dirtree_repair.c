	...
	struct xchk_dirpath_step	*step)
{
	...
	return xfblob_load(dl->path_names, step->name_cookie, dl->namebuf,
			step->name_len);
}
	...
	struct xchk_dirpath_step	*step)
{
	...
	error = xfs_dir_removename(sc->tp, dp, &dl->name, sc->ip->i_ino,
			resblks);
	if (error) {
		ASSERT(error != -ENOENT);
		goto out_trans_cancel;
	 ...
	 */
	xfs_dir_update_hook(dp, sc->ip, -1, &dl->name);
	error = xrep_dirtree_purge_dentry(dl, dp, &dl->name);
	if (error)
		goto out_trans_cancel;
	...
	struct xchk_dirtree		*dl)
{
	...
	trace_xrep_dirtree_create_adoption(sc, sc->ip, dl->nr_paths,
			&dl->adoption.xname, &dl->pptr);

	error = xchk_dirpath_append(dl, sc->ip, path, &dl->adoption.xname,
			&dl->pptr);
	if (error)
		goto out_path;
	...
	struct xchk_dirtree		*dl)
{
	...
	error = xrep_adoption_compute_name(&dl->adoption, dl->namebuf);
	if (error)
		goto out_trans;

	...
	struct xchk_dirpath_step	*step)
{
	...
	/*
	 * Try to take the IOLOCK on the parent before deleting the link.
	 * We hold IOLOCK_EXCL on @sc->ip.
	 */
	while (!xfs_ilock_nowait(dp, XFS_IOLOCK_EXCL)) {
		...
		error = xrep_dirtree_was_stale(dl);
		if (error)
			return error;
	}

	/* Set up parent pointer operation. */
	...
	/* Cancel if someone invalidate the paths while we were unlocked */
	mutex_lock(&dl->lock);
	if (dl->stale) {
		mutex_unlock(&dl->lock);
		error = -ESTALE;
	...
	if (parent) {
		error = xfs_parent_remove(sc->tp, parent, dp, &name, sc->ip);
		if (error)
			return error;
	}
	...
	struct xchk_dirpath		*path)
{
	...
	/* Retake the resources we had at the start of this function. */
	xchk_irele(sc, dp);
	...
	struct xchk_dirtree		*dl)
{
	...
	error = xrep_adoption_init(sc, &dl->adoption);
	if (error)
		return error;

	/*
	 * Try to take the IOLOCK of the the orphanage.  In theory, @sc->ip
	 * isn't linked anywhere, so there should not be another thread holding
	 * the orphanage's IOLOCK while trying to get @sc->ip's IOLOCK.  But
	 * we'll be careful to check for stale scans.
	 */
	while (!xrep_orphanage_ilock_nowait(sc, XFS_IOLOCK_EXCL)) {
		if (xchk_should_terminate(sc, &error))
			goto out_adoption;
		error = xrep_dirtree_was_stale(dl);
		if (error)
	...
	/*
	 * Allocate transaction, reserve disk space, and take the ILOCKs.
	 * Check for stale scan data before we try the lengthly adoption name
	 * computation.
	 */
	error = xfs_trans_alloc(mp, &M_RES(mp)->tr_link, 0, 0, 0, &sc->tp);
	if (error)
		goto out_iolock;
	error = xrep_adoption_prep(&dl->adoption);
	if (error)
		goto out_ilock;

	/*
	 * The directory tree fixer runs after the link counts have been
	 * corrected.  Therefore, we must bump the child's link count since
	 * there will be no further opportunity to fix errors.
	 */
	dl->adoption.bump_child_nlink = true;

	/*
	 * Abort the adoption if the scan data is out of date.  @sc->ip
	 * might have picked up another path to the root directory while it was
	 * unlocked.
	 */
	error = xrep_dirtree_was_stale(dl);
	if (error)
	...
	/*
	 * Create the new name in the orphanage and commit the transaction.
	 * After this point the adoption structure is dead no matter what.
	 */
	error = xrep_adoption_commit(&dl->adoption);
	if (error)
		xchk_trans_cancel(sc);
	else
		error = xrep_trans_commit(sc);
...
out_ilock:
	if (sc->ilock_flags & XFS_ILOCK_EXCL)
		xchk_iunlock(sc, XFS_ILOCK_EXCL);
	if (sc->orphanage_ilock_flags & XFS_ILOCK_EXCL)
		xrep_orphanage_iunlock(sc, XFS_ILOCK_EXCL);
	xchk_trans_cancel(sc);
out_iolock:
	xrep_orphanage_iunlock(sc, XFS_IOLOCK_EXCL);
out_adoption:
	xrep_adoption_cancel(&dl->adoption, error);
	return error;
}
	...
	struct xchk_dirtree		*dl)
{
	...
	/* Retake all the resources we had at the beginning. */
	xchk_trans_alloc_empty(sc);
	xchk_ilock(sc, XFS_ILOCK_EXCL);
	mutex_lock(&dl->lock);
	if (!error && dl->stale)

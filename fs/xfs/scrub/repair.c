	...
	xfs_fsblock_t		fsbno)
{
	 ...
	 * The buffer cache only lets us look for one buffer at a time, so we
	 * have to look one block at a time.  Avoid invalidating AG headers and
	 * post-EOFS blocks because we never own those; and if we can't TRYLOCK
	 * the buffer we assume it's owned by someone else.
	 */

	...
	struct xfs_alloc_arg	*args)
{
	 ...
	 * btree.
	 *
	 * Therefore, log and hold both AG headers so that we don't lose our
	 * hold on the resources needed to commit the new btree, and call
	...
	if (sc->tp->t_flags & XFS_TRANS_DIRTY) {
		...
		/*
		 * The defer ops finishing function will only hold buffers for
		 * us if they've been logged to the current transaction.
		 */
		 ...
		 * types are still set on the AG header buffers.
		 */
	...
	struct xrep_rmap	*rr)
{
	...
	rr->rmap_bload.get_records = xrep_rmap_get_records;
	...
	rmap_cur = xfs_rmapbt_stage_cursor(sc->mp, &rr->new_btree_info.afake,
			pag);
	 ...
	 * to disk.
	 */
	pag->pagf_alt_levels[XFS_BTNUM_RMAPi] = rr->rmap_bload.btree_height;

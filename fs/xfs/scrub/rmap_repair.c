...
struct xrep_rmap {
	...
	/* Current incore maxlevels for the rmap btree. */
	...
	struct xrep_rmap	*rr)
{
	 ...
	 */
	pag->pagf_levels[XFS_BTNUM_RMAPi] = max(rr->old_level,
						rr->rmap_bload.btree_height);
	...
	struct xrep_rmap	*rr)
{
	...
	sc->sa.pag->pagf_levels[XFS_BTNUM_RMAPi] = rr->rmap_bload.btree_height;
out_bitmap:
	...
	struct xfs_scrub	*sc)
{
	...
	rr->sc = sc;
	mutex_init(&rr->lock);
	rr->old_level = levels[XFS_BTNUM_RMAPi];
...
out_rr:
	if (error)
		levels[XFS_BTNUM_RMAPi] = rr->old_level;

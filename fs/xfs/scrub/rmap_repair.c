...
end_agscan:
	 ...
	 * a bnobt cursor.
	 */
	error = xfs_btree_mem_head_read_buf(rr->rmap_btree->target, NULL,
			&mhead_bp);
	if (error)
		return error;
	...
	struct xrep_rmap	*rr)
{
	 ...
	 */
	error = xrep_reap_extents(sc, &rfg.rmap_gaps, &XFS_RMAP_OINFO_ANY_OWNER,
			XFS_AG_RESV_RMAPBT);
	if (error)
		goto out_bitmap;
	...
	void				*data)
{
	...
	rr = container_of(nb, struct xrep_rmap, mod_hook);
	mp = rr->sc->mp;
	...
	struct xfs_scrub	*sc)
{
	...
	if (IS_ERR(rr->rmap_btree)) {
		error = PTR_ERR(rr->rmap_btree);
		goto out_rr;
	}
	 ...
	 */
	error = xfs_hook_add(&sc->sa.pag->pag_rmap_mod_hooks, &rr->mod_hook,
			xrep_rmapbt_live_update);
	if (error)
		goto out_records;
...
out_abort:
	xchk_iscan_abort(&rr->iscan);
	xfs_hook_del(&sc->sa.pag->pag_rmap_mod_hooks, &rr->mod_hook);
out_records:

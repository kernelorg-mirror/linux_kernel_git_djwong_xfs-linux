	...
	void			*ret_ip)
{
	...
	default:
		if (cur->bc_flags & XFS_BTREE_ROOT_IN_INODE)
		{
			...
			xchk_whine(sc->mp, "ino 0x%llx fork %d type %s btnum %d level %d ptr %d agno 0x%x agbno 0x%x error %d ret_ip %pS",
					cur->bc_ino.ip->i_ino,
		...
		{
			xfs_fsblock_t fsbno = xchk_btree_cur_fsbno(cur, level);
			xchk_whine(sc->mp, "type %s btnum %d level %d ptr %d agno 0x%x agbno 0x%x error %d ret_ip %pS",
					xchk_type_string(sc->sm->sm_type),
					cur->bc_btnum,
					level,
					cur->bc_levels[level].ptr,
	...
	void			*ret_ip)
{
	...
	{
		xfs_fsblock_t fsbno = xchk_btree_cur_fsbno(cur, level);
		xchk_whine(sc->mp, "ino 0x%llx fork %d type %s btnum %d level %d ptr %d agno 0x%x agbno 0x%x ret_ip %pS",
				cur->bc_ino.ip->i_ino,
				cur->bc_ino.whichfork,
				xchk_type_string(sc->sm->sm_type),
				cur->bc_btnum,
	...
	{
		xfs_fsblock_t fsbno = xchk_btree_cur_fsbno(cur, level);
		xchk_whine(sc->mp, "type %s btnum %d level %d ptr %d agno 0x%x agbno 0x%x ret_ip %pS",
				xchk_type_string(sc->sm->sm_type),
				cur->bc_btnum,
				level,
				cur->bc_levels[level].ptr,

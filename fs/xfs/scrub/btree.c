	...
	int				level)
{
	if (level < cur->bc_nlevels && cur->bc_levels[level].bp)
		return XFS_DADDR_TO_FSB(cur->bc_mp,
		 ...
		 (xfs_btree_has_iroot(cur)))
		return XFS_INO_TO_FSB(cur->bc_mp, cur->bc_ino.ip->i_ino);
	else if (!xfs_btree_has_long_ptrs(cur))
	...
	struct xchk_btree	*bs)
{
	...
	if (!xfs_btree_has_overlapping(cur))
		return;
	...
	union xfs_btree_ptr	*ptr)
{
	...
	if (xfs_btree_has_iroot(bs->cur) && level == bs->cur->bc_nlevels)
		return true;

	/* Otherwise, check the pointers. */
	if (xfs_btree_has_long_ptrs(bs->cur))
	...
	xfs_daddr_t		daddr)
{
	...
	init_sa = xfs_btree_has_long_ptrs(bs->cur);
	if (init_sa) {

	...
	struct xrep_ibt		*ri)
{
	 ...
	 * get the same AGI buffer.
	 */
	ASSERT(pag->pagi_init);
	pag->pagi_init = 0;
	error = xfs_ialloc_read_agi(sc->mp, sc->tp, sc->sa.pag->pag_agno, &bp);
	if (error)
		return error;
	...
	void			*priv)
{
	...
	error = xrep_newbt_relog_autoreap(&ri->new_inobt_info);
	if (error)
		return error;
	...
	void			*priv)
{
	...
	error = xrep_newbt_relog_autoreap(&ri->new_finobt_info);
	if (error)
		return error;
	...
	struct xrep_ibt		*ri)
{
	...
	ri->ino_bload.get_records = xrep_ibt_get_records;
	xrep_bload_estimate_slack(ri->sc, &ri->ino_bload);

	if (need_finobt) {
		ri->fino_bload.claim_block = xrep_fibt_claim_block;
		ri->fino_bload.get_records = xrep_fibt_get_records;
	...
	if (need_finobt) {
		error = xrep_newbt_destroy(&ri->new_finobt_info);
		if (error)
			goto err_inobt;
	}
	error = xrep_newbt_destroy(&ri->new_inobt_info);
	if (error)
		return error;
...
err_finobt:
	if (need_finobt)
		xrep_newbt_cancel(&ri->new_finobt_info);
err_inobt:
	xrep_newbt_cancel(&ri->new_inobt_info);
	return error;
}

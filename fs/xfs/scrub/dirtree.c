	...
	const struct xfs_parent_irec	*pptr)
{
	...
	error = xfblob_store(dl->path_names, &step.name_cookie, name->name,
			name->len);
	if (error)
		return error;
	...
	void				*priv)
{
	...
	trace_xchk_dirtree_create_path(sc, ip, dl->nr_paths, &xname, &dl->pptr);
	...
	ret = xchk_dirpath_append(dl, sc->ip, path, &xname, &dl->pptr);
	if (ret)
		goto out_path;
	...
	struct xchk_dirpath		*path)
{
	...
	dl->name.len = step.name_len;
	error = xfblob_load(dl->path_names, step.name_cookie, dl->namebuf,
			step.name_len);
	if (error)
		return error;
	 ...
	 */
	error = xfs_parent_lookup(sc->tp, sc->ip, &dl->name, &dl->pptr,
			&dl->pptr_scratch);
	if (error == -ENOATTR) {
		trace_xchk_dirpath_disappeared(dl->sc, sc->ip, path->path_nr,
				path->first_step, &dl->name, &dl->pptr);
		dl->stale = true;
		return -ESTALE;
	}
	...
	struct xchk_dirpath	*path)
{
	...
	error = xchk_dirpath_append(dl, dp, path, &dl->name, &dl->pptr);
	if (error)
		goto out_scanlock;
	...
	struct xchk_dirpath	*path)
{
	...
	trace_xchk_dirpath_walk_upwards(sc, sc->ip, path->path_nr, &dl->name,
			&dl->pptr);

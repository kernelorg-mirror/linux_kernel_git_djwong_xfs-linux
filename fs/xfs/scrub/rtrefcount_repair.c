	...
	xfs_nlink_t			refcount)
{
	...
	trace_xrep_rtrefc_found(rr->sc->mp, bno, len, refcount);
	...
	void			*priv)
{
	...
	/*
	 * The realtime refcount inode should never have extended attributes,
	 * and all blocks should have the bmbt block flag set.
	 */
	...
	xfs_agnumber_t		agno)
{
	...
	error = xfs_rmap_query_all(cur, xrep_rtrefc_walk_rmap, rr);
	...
	struct xrep_rtrefc	*rr)
{
	...
	cur = xfs_rtrmapbt_init_cursor(sc->mp, sc->tp, sc->mp->m_rrefcountip);
	...
	/* Set up some storage */
	rmap_bag = xfbma_init("rtrmap bag",
			sizeof(struct xrep_rtrefc_rmap));
	if (IS_ERR(rmap_bag)) {
	...
	while (xfs_btree_has_more_records(cur)) {
		...
		error = xrep_rtrefc_next_rrm(cur, rr, &rrm, &have);
		if (error)
			goto out;
		if (!have)
			break;
		...
		while (stack_sz) {
			...
			while (have && rrm.startblock == nbno) {
				...
				if (error)
					goto out;
				stack_sz++;
				error = xrep_rtrefc_next_rrm(cur, rr, &rrm,
						&have);
				if (error)
					goto out;
			}

	...
	struct xrep_rtrefc	*rr)
{
	...
	/* Set up some storage */
	rmap_bag = xfarray_create("rtrmap bag", sizeof(struct xrep_rtrefc_rmap));
	if (IS_ERR(rmap_bag)) {
	...
	error = xfs_rmap_lookup_le(sc->sr.rmap_cur, 0, 0, 0, 0, NULL, &have_gt);
	if (error)
		goto out_bag;
	ASSERT(have_gt == 0);
	...
	struct xrep_rtrefc	*rr)
{
	 ...
	 * rmapbt.
	 */
	delta = ifake->if_blocks - mp->m_rrefcountip->i_nblocks;
	mp->m_rrefcountip->i_nblocks = ifake->if_blocks;
	...
	struct xrep_rtrefc	*rr)
{
	 ...
	 * it.
	 */
	error = xrep_reap_extents(rr->sc, &rr->old_rtrefcountbt_blocks,
	 ...
	 * don't make bad decisions later.
	 */
	...
	struct xfs_scrub	*sc)
{
	...
	/* Set up some storage */
	rr->refcount_records = xfarray_create("rtrefcount records",

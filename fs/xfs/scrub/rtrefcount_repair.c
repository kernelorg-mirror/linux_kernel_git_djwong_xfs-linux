...
{
	xfs_rgblock_t			rgbno = rec->rc_startblock;
	xfs_rtblock_t			rtbno;

	if (rgbno >= XFS_RTREFC_COW_START)
		rgbno -= XFS_RTREFC_COW_START;

	/* Must be within the rt device. */
	if (!xfs_verify_rgbext(sc->sr.rtg, rgbno, rec->rc_blockcount))
		return -EFSCORRUPTED;
	...
	xfs_rgblock_t		*nbnop)
{
	...
	xfs_rgblock_t		nbno = NULLRGBLOCK;
	int			error;
	...
	while ((error = xfarray_iter(rmap_bag, &array_cur, &rrm)) == 1)
		nbno = min_t(xfs_rgblock_t, nbno, RRM_NEXT(rrm));
	...
	struct xrep_rtrefc	*rr)
{
	 ...
	 * worth of rtrmapbt records.
	 */
	error = xfarray_create(sc->mp, "rtrmap bag", XFS_RTREFC_REFCOUNT_MAX,
	...
	struct xrep_rtrefc	*rr)
{
	...
	error = xfs_trans_reserve_more_inode(sc->tp, rtg->rtg_refcountip,
			rr->rtrefc_bload.nr_blocks, 0, true);
	if (error)
		goto err_cur;

 ...
 * inode to make disposal simpler.  The inode had better not be in FREEING
 * state!
 */
int
xchk_scan_this_inode(
	struct xfs_scrub	*sc,
	struct xfs_inode	*ip)
{
	if (!igrab(VFS_I(ip)))
	...
	struct xfs_inode	*ip_in)
{
	...
	if (sc->sm->sm_ino == 0 || sc->sm->sm_ino == ip_in->i_ino)
		return xchk_scan_this_inode(sc, ip_in);

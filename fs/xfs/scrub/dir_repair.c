	...
	xfs_ino_t		ino)
{
	...
	trace_xrep_directory_salvage_entry(sc->ip, name, entry.namelen, ino);
	...
	struct xfs_buf			*bp)
{
	...
	if (dabno < geo->leafblk) {
		switch (data3->hdr.magic) {
		case cpu_to_be32(XFS_DIR3_BLOCK_MAGIC):
			bp->b_ops = &xfs_dir3_block_buf_ops;
			xfs_trans_buf_set_type(sc->tp, bp,
		 ...
		 */
		if (data3->hdr.magic == cpu_to_be32(XFS_DIR3_BLOCK_MAGIC)) {
			dep = bp->b_addr + geo->data_entry_offset;
			if (dep->namelen != 1 || dep->name[0] != '.')
				return -EFSCORRUPTED;

			dep->inumber = cpu_to_be64(sc->ip->i_ino);
			xfs_trans_log_buf(sc->tp, bp, geo->data_entry_offset,
					geo->data_entry_offset +
					sizeof(dep->inumber) - 1);
		}

		data3->hdr.owner = cpu_to_be64(sc->ip->i_ino);
		xfs_trans_log_buf(sc->tp, bp,
				offsetof(struct xfs_dir3_data_hdr, hdr.owner),
				offsetof(struct xfs_dir3_data_hdr, hdr.owner) +
				sizeof(data3->hdr.owner) - 1);
		return 0;
	}

	/* Directory leaf and da node blocks. */
	if (dabno < geo->freeblk) {
		switch (info3->hdr.magic) {
		...
		xfs_trans_log_buf(sc->tp, bp,
				offsetof(struct xfs_da3_blkinfo, owner),
				offsetof(struct xfs_da3_blkinfo, owner) +
				sizeof(info3->owner) - 1);
		return 0;
	}

	/* Directory free blocks. */
	if (free3->hdr.magic != cpu_to_be32(XFS_DIR3_FREE_MAGIC))
		return -EFSCORRUPTED;

	bp->b_ops = &xfs_dir3_free_buf_ops;
	xfs_trans_buf_set_type(sc->tp, bp, XFS_BLFT_DIR_FREE_BUF);

	free3->hdr.owner = cpu_to_be64(sc->ip->i_ino);
	xfs_trans_log_buf(sc->tp, bp,
			offsetof(struct xfs_dir3_free_hdr, hdr.owner),
			offsetof(struct xfs_dir3_free_hdr, hdr.owner) +
				sizeof(free3->hdr.owner) - 1);
	return 0;
}
	...
	struct xfs_scrub		*sc)
{
	 ...
	 * owner updates per transaction.  This is an under-estimate since
	 * we're logging just the header, not full blocks.
	 */
	max_logged = howmany(xfs_allocfree_block_count(mp, 1), geo->fsbcount);
	...
	for (offset = 0;
		...
		for (dabno = round_up(map.br_startoff, geo->fsbcount);
			...
			if (++logged >= max_logged) {
				error = xrep_tempfile_roll_trans(sc);
				if (error)
					return error;
				logged = 0;
			}

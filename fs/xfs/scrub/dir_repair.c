...
struct xrep_dir {
	...
	/* Mutex protecting namebuf, dir_entries, dir_names, and parent_ino. */
	struct mutex		lock;
	...
	void			*priv)
{
	...
	xfs_init_parent_name_irec(&pptr, name);
	...
	mutex_lock(&rd->lock);
	error = xrep_dir_add_dirent(rd, &xname, ip->i_ino, pptr.p_diroffset);
	mutex_unlock(&rd->lock);
	return error;
}
	...
	struct xrep_dir		*rd)
{
	...
	/*
	 * Filesystem scans are time consuming.  Drop all resources for the
	 * duration of the scan and hope for the best.
	 */
	...
	struct xrep_dir		*rd)
{
	...
	if (sc->ip == sc->mp->m_rootip) {
		...
		/* Should have found a parent somewhere. */
		ASSERT(rd->parent_ino != NULLFSINO);
	}

	...
	struct xrep_dir		*rd)
{
	...
	/*
	 * We rely on the caller's hold on @sc->ip's IOLOCK_EXCL to quiesce all
	 * possible directory updates during the time when we did not hold the
	 * ILOCK.  There should not be any dirent updates to replay, but check
	 * anyway.
	 */

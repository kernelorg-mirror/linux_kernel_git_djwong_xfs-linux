...
struct xrep_dir {
	...
	/* Scratch buffer for reading parent pointers from child files. */
	...
	const struct xrep_dirent	*dirent)
{
	...
	case XREP_DIRENT_ADD:
		 ...
		 */
#ifdef DEBUG
		error = xchk_dir_lookup(rd->sc, rd->sc->tempip, &rd->name,
				&ino);
		if (error != -ENOENT) {
			ASSERT(error != -ENOENT);
			goto out_cancel;
		...
		error = xrep_dir_replay_createname(rd, &rd->name, dirent->ino,
				resblks);
		if (error)
			goto out_cancel;

		if (dirent->ftype == XFS_DIR3_FT_DIR)
			rd->subdirs++;
		rd->dirents++;
		break;
	case XREP_DIRENT_REMOVE:
		...
		error = xrep_dir_replay_removename(rd, &rd->name, resblks);
		if (error)
			goto out_cancel;
	...
	struct xrep_dir		*rd)
{
	...
	foreach_xfarray_idx(rd->dir_entries, array_cur) {
		...
		if (error)
			return error;
		rd->name.len = dirent.namelen;
		rd->name.type = dirent.ftype;

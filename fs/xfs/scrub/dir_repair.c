	...
	const struct xrep_dirent	*entry)
{
	...
	struct xfs_mount		*mp = rd->sc->mp;
	unsigned char			*namebuf = rd->namebuf;
	xfs_ino_t			ino;
	uint				resblks;
	int				error;
	...
	struct xrep_dir		*rd)
{
	 ...
	 */
	return xrep_dir_find_entries(rd);
}
	...
	struct xrep_dir		*rd)
{
	 ...
	 */
	if (dp->i_prev_unlinked == 0) {
		set_nlink(VFS_I(sc->ip), rd->subdirs + 2);
		return 0;
	}
	...
	struct xrep_dir		*rd)
{
	...
	/*
	 * We drop the iLOCK on sc->ip to lock the orphanage and prepare for
	 * the adoption.  Therefore, look up the old dotdot entry for sc->ip so
	 * that we can compare it after we re-lock sc->ip.
	 */

...
DEFINE_REPAIR_EXTENT_EVENT(xrep_bmapi_reap_binval);
...
DEFINE_EVENT(xrep_rtgroup_extent_class, name, \
	TP_PROTO(struct xfs_rtgroup *rtg, xfs_rgblock_t rgbno, \
		 xfs_extlen_t len), \
	TP_ARGS(rtg, rgbno, len))
DEFINE_REPAIR_RTGROUP_EXTENT_EVENT(xrep_dispose_unmap_extent_rt);
DEFINE_REPAIR_RTGROUP_EXTENT_EVENT(xrep_dispose_free_extent_rt);
...
DEFINE_EVENT(xrep_reap_find_class, name, \
	TP_PROTO(struct xfs_perag *pag, xfs_agblock_t agbno, xfs_extlen_t len, \
		 bool crosslinked), \
	TP_ARGS(pag, agbno, len, crosslinked))
DEFINE_REPAIR_REAP_FIND_EVENT(xrep_agextent_reap_find);
DEFINE_REPAIR_REAP_FIND_EVENT(xrep_bmapi_reap_find);
...
DEFINE_EVENT(xrep_rtgroup_reap_find_class, name, \
	...
	TP_ARGS(rtg, rgbno, len, crosslinked))
DEFINE_REPAIR_RTGROUP_REAP_FIND_EVENT(xrep_rtextent_reap_find);
...
DEFINE_NEWBT_EXTENT_EVENT(xrep_newbt_cancel_blocks);
...
TRACE_EVENT(xrep_bmapi_reap_binval_scan,
...
TRACE_EVENT(xrep_rtrefc_found,
	...
	TP_printk("dev %d:%d rtdev %d:%d rgbno 0x%x fsbcount 0x%x refcount %u",
		  MAJOR(__entry->dev), MINOR(__entry->dev),
		  MAJOR(__entry->rtdev), MINOR(__entry->rtdev),
		  __entry->startblock,
		  __entry->blockcount,

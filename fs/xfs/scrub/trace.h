...
DECLARE_EVENT_CLASS(xchk_vector_head_class,
	...
	TP_printk("dev %d:%d ino 0x%llx agno %u inum 0x%llx gen %u flags 0x%x rest_us %u nr_vecs %u",
		  MAJOR(__entry->dev), MINOR(__entry->dev),
		  __entry->ino,
		  __entry->agno,
		  __entry->inum,
...
TRACE_EVENT(xchk_qcheck_error,
	...
	TP_printk("dev %d:%d dquot type %s id %u ret_ip %pS",
		  MAJOR(__entry->dev), MINOR(__entry->dev),
...
TRACE_EVENT(xrep_setup_tempfile,
	...
	TP_printk("dev %d:%d ino 0x%llx type %s inum %llu gen %u flags 0x%x temp_inum 0x%llx",
		  MAJOR(__entry->dev), MINOR(__entry->dev),
		  __entry->ino,
		  __print_symbolic(__entry->type, XFS_SCRUB_TYPE_STRINGS),
		  __entry->inum,
...
DECLARE_EVENT_CLASS(xrep_dir_class,
	TP_PROTO(struct xfs_inode *dp, xfs_ino_t old_parent, xfs_ino_t parent_ino),
	TP_ARGS(dp, old_parent, parent_ino),
	TP_STRUCT__entry(
		__field(dev_t, dev)
		__field(xfs_ino_t, dir_ino)
		__field(xfs_ino_t, old_parent)
		__field(xfs_ino_t, parent_ino)
	),
	TP_fast_assign(
		__entry->dev = dp->i_mount->m_super->s_dev;
		__entry->dir_ino = dp->i_ino;
		__entry->old_parent = old_parent;
		__entry->parent_ino = parent_ino;
	),
	TP_printk("dev %d:%d dir 0x%llx parent 0x%llx -> 0x%llx",
		  MAJOR(__entry->dev), MINOR(__entry->dev),
		  __entry->dir_ino,
		  __entry->old_parent,
		  __entry->parent_ino)
...
TRACE_EVENT(xrep_rtrmap_found,
	...
	TP_printk("dev %d:%d rtbno %llu len %llu owner %lld offset %llu flags 0x%x",
		  MAJOR(__entry->dev), MINOR(__entry->dev),
		  __entry->rtbno,
		  __entry->len,
		  __entry->owner,

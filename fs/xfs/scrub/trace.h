...
DECLARE_EVENT_CLASS(xrep_inode_class,
	...
	TP_fast_assign(
		__entry->dev = sc->mp->m_super->s_dev;
		__entry->ino = sc->sm->sm_ino;
		__entry->size = sc->ip->i_d.di_size;
		__entry->nblocks = sc->ip->i_d.di_nblocks;
		__entry->flags = sc->ip->i_d.di_flags;
		__entry->flags2 = sc->ip->i_d.di_flags2;
		__entry->nextents = sc->ip->i_df.if_nextents;
		__entry->format = sc->ip->i_df.if_format;
		__entry->anextents = sc->ip->i_afp ?
					sc->ip->i_afp->if_nextents : 0;
		__entry->aformat = sc->ip->i_afp ? sc->ip->i_afp->if_format :

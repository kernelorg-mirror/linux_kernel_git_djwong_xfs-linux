	...
	struct xchk_rtsum_compute *state)
{
	...
	error = xfile_io(state->sumfile, XFILE_IO_READ, &pos, &v,
	...
	/* ...and write it back. */
	error = xfile_io(state->sumfile, XFILE_IO_WRITE, &pos, &v,
			sizeof(xfs_suminfo_t));
	if (error)
		return error;
	...
	struct file		*sumfile)
{
	...
	state.sumfile = sumfile;
	rtbmp_bytes = howmany(mp->m_sb.sb_rextents, NBBY);
	end_off = howmany(rtbmp_bytes, mp->m_sb.sb_blocksize);

	/* If the bitmap size doesn't match the computed size, bail. */
	if (roundup(rtbmp_bytes, mp->m_sb.sb_blocksize) !=
	...
	struct file		*sumfile)
{
	...
	for (off = 0, pos = 0;
		...
		error = xfile_io(sumfile, XFILE_IO_READ, &ppos, sc->buf, count);
		if (error) {
			xfs_trans_brelse(sc->tp, bp);
			break;
		}
	...
	struct xfs_scrub	*sc)
{
	 ...
	 */
	sumfile = xfile_create("rtsummary", mp->m_rsumsize);
	if (IS_ERR(sumfile)) {
		error = PTR_ERR(sumfile);
		goto out_rbm;
	}

	/* Construct the new summary file from the rtbitmap. */
	error = xchk_rtsum_compute(sc, sumfile);

	...
	uint8_t			*nr_inodes)
{
	...
	for (; !error; error = xfs_btree_increment(cur, 0, &has_rec)) {
		...
		if (rec.ir_freecount < XFS_INODES_PER_CHUNK) {
			...
			*inusep = (~rec.ir_free) >> next;
			*nr_inodes = XFS_INODES_PER_CHUNK - next;
			break;
		}
	}
	...
	uint8_t			*nr_inodes)
{
	...
	do {
		...
		ret = xchk_iscan_find_next(iscan, agi_bp, pag, inusep, &agino,
				nr_inodes);
		if (ret)
			goto out_buf;
	...
	uint8_t			nr_inodes)
{
	...
	for (i = 1; i < nr_inodes; i++, ino++, inuse >>= 1) {
		if (!(inuse & 1)) {
	...
	struct xchk_iscan	*iscan)
{
	...
	do {
		...
		xfs_inoused_t	inuse = 0;
		uint8_t		nr_inodes = 0;

		ret = xchk_iscan_advance(iscan, &pag, &agi_bp, &inuse,
				&nr_inodes);
		if (ret != 1)
			return ret;
		...
		ret = xchk_iscan_iget(iscan, pag, agi_bp, inuse, nr_inodes);
	} while (ret == -EAGAIN);

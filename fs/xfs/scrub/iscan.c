 ...
 * to be notified when a metadata update occurs.  Live updates for files that
 * have not yet been scanned must be ignored.  To that end, the live scanner
 * can tell the hook code which inodes have been visited if the scanner thread
 * keeps us in the loop by calling xchk_iscan_visit.
 */
...
/*
 * Grab an inode as part of an inode scan.  Returns 0 and an incore inode;
 * -EAGAIN if the caller should call xchk_iscan_advance immediately; -ECANCELED
 *  if we couldn't grab an inode; or some other errno.
 */
int
xchk_iscan_iget(
	struct xfs_scrub	*sc,
	...
	struct xfs_inode	**ipp)
{
	...
	case -ENOENT:
		 ...
		 * to clear the pending work and then try again.
		 */
		xfs_inodegc_flush(mp);
		fallthrough;
	case -EINVAL:
		/*
		 * We thought the inode was allocated, but iget failed to find
		 * it.  This could be because the inobt lookup failed, or
		 * because there's an incore inode that thinks it's marked
		 * free.  Either way, we back up the scan by one inode and tell
		 * the caller to call _advance again if we haven't run out
		 * of retries.
		 */
...
/* Mark this inode as having been visited. */
void
__xchk_iscan_visit(struct xchk_iscan *iscan, struct xfs_inode *ip)
{

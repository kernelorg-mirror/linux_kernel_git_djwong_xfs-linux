	...
	struct xfs_imap		*imap)
{
	...
	ri = kmem_zalloc(sizeof(struct xrep_inode), KM_NOFS | KM_MAYFAIL);
	if (!ri)
		return -ENOMEM;
	...
	struct xfs_perag	*pag)
{
	...
	error = xfs_alloc_read_agf(ri->sc->mp, ri->sc->tp, pag->pag_agno, 0,
			&agf);
	if (error)
		return error;
	...
	struct xrep_inode	*ri)
{
	...
	error = xrep_dinode_count_rt_rmaps(ri);
	if (error)
		return error;

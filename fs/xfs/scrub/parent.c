	...
	xfs_dir2_dataptr_t	diroffset)
{
	...
	if (diroffset != child_diroffset) {
		trace_xchk_parent_bad_dapos(sc->ip, diroffset, dp->i_ino,
				child_diroffset, name, namelen);
		xchk_fblock_xref_set_corrupt(sc, XFS_ATTR_FORK, 0);
		return 0;
	}
...
xchk_parent_iget(
	struct xfs_scrub			*sc,
	const struct xfs_parent_name_rec	*attr_key,
	...
	xfs_dir2_dataptr_t			*diroffsetp)
{
	...
	xfs_init_parent_name_irec(&irec, attr_key);

	/* Validate inode number. */
	error = xfs_dir_ino_validate(sc->mp, irec.p_ino);
	if (error) {
		xchk_fblock_set_corrupt(sc, XFS_ATTR_FORK, 0);
		return -ECANCELED;
	}

	error = xchk_iget(sc, irec.p_ino, &ip);
	if (!xchk_fblock_xref_process_error(sc, XFS_ATTR_FORK, 0, &error))
		return error;
	...
	if (VFS_I(ip)->i_generation != irec.p_gen) {
		xchk_fblock_set_corrupt(sc, XFS_ATTR_FORK, 0);
		goto out_rele;
	}

	*dpp = ip;
	*diroffsetp = irec.p_diroffset;
	return 0;
out_rele:
	xchk_irele(sc, ip);
	return -ECANCELED;
	...
	void			*priv)
{
	...
	error = xchk_parent_iget(sc, attr_key, &dp, &diroffset);
	if (error)
		return error;
	...
	error = xchk_parent_dirent(pp, dp, value, valuelen, diroffset);
	if (error)
		goto out_unlock;

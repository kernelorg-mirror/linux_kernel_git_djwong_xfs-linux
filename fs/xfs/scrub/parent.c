	...
	xfs_ino_t		parent_ino)
{
	...
	/*
	 * The game changes if we get here.  We failed to iolock the alleged
	 * parent while holding the child iolock.  Release the lock on the
	 * child, and then try to lock the alleged parent and child at the
	 ...
	 * the expected nlink and the '..' dirent.
	 */
	expected_nlink = VFS_I(sc->ip)->i_nlink == 0 ? 0 : 1;
	...
	/*
	 * After relocking the child directory, the '..' entry points to a
	 * different parent than before.  Someone moved the child, which
	 * implies that the parent link is now correct.
	 */

	...
	void				*priv)
{
	...
	ret = xfs_parent_from_xattr(sc->mp, attr_flags, name, namelen,
			value, valuelen, &pp->pptr);
	if (ret < 0)
		return ret;

	if (ret == 1 && pp->parent_ino == pp->pptr.p_ino)
		return -ECANCELED;
	...
	void			*priv)
{
	...
	if (!lockmode) {
		...
		ret = xfblob_store(pp->pptr_names, &save_pp.name_cookie,
				name, namelen);
	...
	struct xchk_pptrs	*pp)
{
	...
	error = xfs_parent_lookup(sc->tp, sc->ip, &pp->name, &pp->pptr,
			&pp->pptr_scratch);
	if (error == -ENOATTR) {
	...
	struct xchk_pptr	*pptr)
{
	...
	pp->name.len = pptr->namelen;

	error = xfblob_load(pp->pptr_names, pptr->name_cookie, pp->namebuf,
			pptr->namelen);
	if (error)
		return error;
	...
	lockmode = xchk_parent_lock_dir(sc, dp);
	if (lockmode) {
		trace_xchk_parent_slowpath(sc->ip, &pp->name, dp->i_ino);
		goto check_dirent;
	}
	...
	trace_xchk_parent_ultraslowpath(sc->ip, &pp->name, dp->i_ino);
...
check_dirent:
	error = xchk_parent_dirent(pp, &pp->name, dp);
out_unlock:
	xfs_iunlock(dp, lockmode);
out_rele:
	xchk_irele(sc, dp);

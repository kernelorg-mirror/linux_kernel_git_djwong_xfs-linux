	...
	void			*priv)
{
	...
	delta = (int64_t)xcdq.icount - dqp->q_icount;
	if (delta) {
		dqp->q_res_icount += delta;
		dqp->q_icount += delta;
		dirty = true;
	}

	delta = (int64_t)xcdq.bcount - dqp->q_bcount;
	if (delta) {
		dqp->q_res_bcount += delta;
		dqp->q_bcount += delta;
		dirty = true;
	}

	delta = (int64_t)xcdq.rtbcount - dqp->q_rtbcount;
	if (delta) {
		dqp->q_res_rtbcount += delta;
		dqp->q_rtbcount += delta;
		dirty = true;
	}
	mutex_unlock(&xqc->lock);
	...
	struct xfs_scrub	*sc)
{
	...
	if (xqc->ucounts) {
		error = xqcheck_commit_dqtype(xqc, XFS_DQ_USER);
		if (error)
			return error;
	}
	if (xqc->gcounts) {
		error = xqcheck_commit_dqtype(xqc, XFS_DQ_GROUP);
		if (error)
			return error;
	}
	if (xqc->pcounts) {
		error = xqcheck_commit_dqtype(xqc, XFS_DQ_PROJ);
		if (error)
			return error;
	}

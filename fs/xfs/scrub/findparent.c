...
/* Look for dotdot updates to the inode being scanned. */
STATIC int
	...
	struct xrep_parent_scan_info	*pscan)
{
	...
	xchk_iscan_start(&pscan->iscan, 30000, 100);
	 ...
	 */
	xfs_hook_setup(&pscan->hooks.delta_hook, xrep_findparent_live_update);
	error = xfs_dirent_hook_add(sc->mp, &pscan->hooks);
	if (error)
		goto out_iscan;
	...
	struct xrep_parent_scan_info	*pscan)
{
	...
	while ((ret = xchk_iscan_iter(sc, &pscan->iscan, &fpi.dp)) == 1) {
...
xrep_findparent_scan_teardown(
	struct xrep_parent_scan_info	*pscan)
{
	xfs_dirent_hook_del(pscan->sc->mp, &pscan->hooks);
	xchk_iscan_teardown(&pscan->iscan);
	mutex_destroy(&pscan->lock);
}

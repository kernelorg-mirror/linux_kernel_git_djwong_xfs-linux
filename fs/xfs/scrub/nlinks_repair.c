	...
	void			*data)
{
	...
	struct xfs_trans	*tp;
	xfs_nlink_t		observed_nlink, inode_nlink;
	int			error;
	...
	error = xchk_nlinks_get_shadow_count(xnc, ino, &observed_nlink);
	if (error)
		goto out_unlock;
	 ...
	 */
	if (S_ISDIR(VFS_I(ip)->i_mode) && observed_nlink == 1) {
		observed_nlink = 2;
		error = xrep_nlinks_ino_set(xnc, ip->i_ino, observed_nlink);
		if (error)
			goto out_unlock;
	}

	inode_nlink = VFS_I(ip)->i_nlink;
	if (observed_nlink == inode_nlink)
		goto out_unlock;

	/* XXX dropping an inode link count to zero isn't written yet */
	if (inode_nlink > 0 && observed_nlink == 0) {
		error = -EL2HLT;
		goto out_unlock;
	}
	...
	trace_xrep_nlinks_commit_inode(mp, ino, inode_nlink, observed_nlink);

	xfs_trans_ijoin(tp, ip, XFS_ILOCK_EXCL);
	set_nlink(VFS_I(ip), observed_nlink);

	...
	struct xchk_nlink	*obs)
{
	...
	/*
	 * Add one link count from lost+found to our file.  We still hold the
	 * file's ILOCK, so @obs is still valid after cycling @xnc->lock.  If
	 * the link count update would causes an overflow, don't touch
	 * anything.
	 */
	new_value = (uint64_t)obs->parents + 1;
	if (new_value > U32_MAX) {
		error = -EMLINK;
		goto out_unlock;
	}
	obs->parents = new_value;
	...
	/*
	 * If the child is a directory, we need to bump the incore link count
	 * of the orphanage to account for the new orphan's child subdirectory
	 * entry.  If that would overflow the orphanage's link count, don't
	 * touch anything.
	 */
	if (S_ISDIR(VFS_I(ip)->i_mode)) {
		struct xchk_nlink	orphanage_obs;
		...
		new_value = (uint64_t)obs->children + 1;
		if (new_value > U32_MAX) {
			error = -EMLINK;
			goto out_unlock;
		}

		orphanage_obs.flags |= XCHK_NLINK_WRITTEN;
		orphanage_obs.children = new_value;

		error = xrep_nlinks_set_record(xnc, sc->orphanage->i_ino,
				&orphanage_obs);
		if (error)
			goto out_unlock;
	}
	...
	struct xchk_nlink_ctrs	*xnc)
{
	...
	if (orphanage_available &&
		...
		error = xrep_nlinks_adopt(xnc, &obs);
		if (error == -EMLINK) {
			trace_xrep_nlinks_unfixable_inode(mp, ip, &obs);
			error = 0;
			goto out_trans;

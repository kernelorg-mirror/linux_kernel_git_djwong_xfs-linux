	...
	const union xfs_btree_ptr	*ptr)
{
	...
	BUILD_BUG_ON(xfbtree_bbsize() != xfo_to_bb(1));
	...
	const struct xfbtree_config	*cfg)
{
	...
	if (cfg->flags & XFBTREE_CREATE_LONG_PTRS) {
		if (xfs_has_crc(mp))
			blocklen -= XFS_BTREE_LBLOCK_CRC_LEN;
		else
			blocklen -= XFS_BTREE_LBLOCK_LEN;
		return blocklen;
	}
	...
	const struct xfbtree_config	*cfg)
{
	...
	xfs_btree_init_block_int(mp, bp->b_addr, daddr, cfg->btnum, 0, 0, 0,
			bc_flags);
	error = xfs_bwrite(bp);
	xfs_buf_relse(bp);
	if (error)
	...
	struct xfbtree		*xfbt)
{
	...
	xfs_btree_mem_head_init(bp, XFBTREE_INIT_LEAF_BLOCK);
	error = xfs_bwrite(bp);
	xfs_buf_relse(bp);
	if (error)
		return error;
	...
	struct xfbtree			**xfbtreep)
{
	...
	error = xfile_create(mp, cfg->description, 0, &xfbt->xfile);
	if (error)
		return error;
	...
	xfbt->maxrecs[1] = blocklen / cfg->btree_ops->key_len;
	...
	struct xfs_buf		**bpp)
{
	...
	daddr = xfo_to_bb(XFBTREE_HEAD_BLOCK);
	return xfs_trans_read_buf(mp, tp, btp, daddr, xfbtree_bbsize(), 0, bpp,
	...
	int				*stat)
{
	 ...
	 */
	bt_xfoff = xbitmap_take_first_set(xfbt->freespace, 0, -1ULL);
	if (bt_xfoff == -1ULL) {
	...
	error = xfile_prealloc(xfbt->xfile, pos, xfo_to_b(1));
	if (error)
		return error;
	...
	struct xfs_buf		*bp)
{
	...
	xfile_discard(xfbt->xfile, xfo_to_b(bt_xfoff), xfo_to_b(bt_xflen));
	return 0;
}
 ...
 * pointers, not to guarantee consistency of updates.
 */
int

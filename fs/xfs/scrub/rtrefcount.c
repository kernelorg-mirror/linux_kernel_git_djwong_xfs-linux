	...
	xfs_nlink_t		refcount)
{
	...
	rtbno = xfs_rgbno_to_rtb(sc->mp, sc->sr.rtg->rtg_rgno, bno);
	xchk_xref_is_used_rt_space(sc, rtbno, len);
...
{
	struct xfs_rtgroup		*rtg = bs->cur->bc_ino.rtg;
	...
	bno = be32_to_cpu(rec->rtrefc.rc_startblock);
	len = be32_to_cpu(rec->rtrefc.rc_blockcount);
	...
	if (bno + len <= bno ||
	    !xfs_verify_rgbno(rtg, bno) ||
	    !xfs_verify_rgbno(rtg, bno + len - 1))
		xchk_btree_set_corrupt(bs->sc, bs->cur, 0);

	/* We can only share full rt extents. */
	xfs_rtb_to_rtx(mp, bno, &mod);
	if (mod)
		xchk_btree_set_corrupt(bs->sc, bs->cur, 0);
	xfs_rtb_to_rtx(mp, len, &mod);
	if (mod)
		xchk_btree_set_corrupt(bs->sc, bs->cur, 0);
	...
	xchk_rtrefcountbt_check_mergeable(bs, rrc, bno, len, refcount);
	...
	if (!has_cowflag) {
		...
		rrc->next_unshared_rgbno = bno + len;
	}
	...
	xfs_extlen_t		len)
{
	...
	error = xfs_refcount_scan_keyfill(sc->sr.refc_cur, bno, len, &keyfill);
	if (!xchk_should_check_xref(sc, &error, &sc->sr.refc_cur))
		return;
	...
	xfs_extlen_t		len)
{
	...
	error = xfs_refcount_scan_keyfill(sc->sr.refc_cur,
			bno + XFS_RTREFC_COW_START, len, &keyfill);
	if (!xchk_should_check_xref(sc, &error, &sc->sr.refc_cur))
		return;
	if (keyfill != XFS_BTREE_KEYFILL_EMPTY)
		xchk_btree_xref_set_corrupt(sc, sc->sr.refc_cur, 0);

	...
	const union xfs_btree_rec	*rec)
{
	...
	if (irec.rc_startblock + irec.rc_blockcount <= irec.rc_startblock ||
	    !xfs_verify_rgbno(rtg, irec.rc_startblock) ||
	    !xfs_verify_rgbno(rtg, irec.rc_startblock + irec.rc_blockcount - 1))
		xchk_btree_set_corrupt(bs->sc, bs->cur, 0);
	...
	xfs_extlen_t			len)
{
	...
	error = xfs_refcount_lookup_le(sc->sr.refc_cur, XFS_RCDOM_COW, bno,
			&has_refcount);
	...
	xfs_extlen_t		len)
{
	...
	error = xfs_refcount_scan_keyfill(sc->sr.refc_cur, XFS_RCDOM_SHARED,
			bno, len, &keyfill);
	if (!xchk_should_check_xref(sc, &error, &sc->sr.refc_cur))
		return;
	...
	xfs_extlen_t		len)
{
	...
	error = xfs_refcount_scan_keyfill(sc->sr.refc_cur, XFS_RCDOM_COW, bno,
			len, &keyfill);
	if (!xchk_should_check_xref(sc, &error, &sc->sr.refc_cur))
		return;
	if (keyfill != XFS_BTREE_KEYFILL_EMPTY)

	...
	struct xfs_scrub	*sc)
{
	...
	error = xchk_install_live_inode(sc, sc->mp->m_rrefcountip);
	if (error)
		return error;
...
struct xchk_rtrefcbt_records {
	...
	/* The next rt block where we aren't expecting shared extents. */
	xfs_rtblock_t			next_unshared_rtbno;
	...
	xfs_rtblock_t		bno)
{
	...
	if (bno <= rrc->next_unshared_rtbno || !sc->sr.rmap_cur ||
            xchk_skip_xref(sc->sm))
		return;

	memset(&low, 0, sizeof(low));
	low.rm_startblock = rrc->next_unshared_rtbno;
	memset(&high, 0xFF, sizeof(high));
	...
	const union xfs_btree_rec	*rec)
{
	...
	if (!has_cowflag) {
		...
		rrc->next_unshared_rtbno = bno + len;
	}
	...
	xfs_filblks_t		len)
{
	...
	error = xfs_refcount_scan_keyfill(sc->sr.refc_cur, bno +
			XFS_RTREFC_COW_START, len, &keyfill);
	if (!xchk_should_check_xref(sc, &error, &sc->sr.refc_cur))
		return;
	if (keyfill != XFS_BTREE_KEYFILL_EMPTY)
		xchk_btree_xref_set_corrupt(sc, sc->sr.refc_cur, 0);

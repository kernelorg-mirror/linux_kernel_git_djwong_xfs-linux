	...
	struct xfs_inode	*ip)
{
	...
	error = xchk_scan_this_inode(sc, sc->mp->m_rrefcountip);
	if (error)
		return error;
	...
	union xfs_btree_rec	*rec)
{
	...
	xfs_filblks_t		*cow_blocks = bs->private;
	xfs_rtblock_t		bno;
	xfs_filblks_t		len;
	xfs_nlink_t		refcount;
	u32			mod;

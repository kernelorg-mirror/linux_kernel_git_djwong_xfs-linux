	...
	bool			btree_committed)
{
	...
	if (xnr->resv == XFS_AG_RESV_NONE) {
		...
		error = __xfs_free_extent_later(sc->tp, fsbno, free_aglen,
				&xnr->oinfo, true);
		if (error)
			return error;
	...
	if (xnr->resv == XFS_AG_RESV_RMAPBT ||
		...
		/*
		 * Do not free multiple extents to the same AG in one
		 * transaction because the transaction could be holding on to
		 * the only free space in the AG.  This could cause a livelock
		 * with the AGFL fixing code if it decides to expand the AGFL
		 * to prepare for the subsequent frees.  Return MAX_NEWBT_EFIS
		 * to force a roll.
		 */

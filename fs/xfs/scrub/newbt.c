	...
	struct xfs_scrub		*sc)
{
	...
	xnr->ifake.if_whichfork = XFS_DATA_FORK;
	return 0;
}
	...
	uint64_t		nr_blocks)
{
	...
	while (nr_blocks > 0) {
		...
		trace_xrep_newbt_alloc_ag_blocks(mp, agno, agbno, args.len,
				xnr->oinfo.oi_owner);
		...
		error = xrep_newbt_add_blocks(xnr, sc->sa.pag, agbno,
				args.len, true);
		if (error)
			return error;
	...
	uint64_t		nr_blocks)
{
	...
	while (nr_blocks > 0) {
		...
		trace_xrep_newbt_alloc_file_blocks(mp, agno, agbno, args.len,
				xnr->oinfo.oi_owner);
		...
		error = xrep_newbt_add_blocks(xnr, pag, agbno, args.len, true);
		xfs_perag_put(pag);
		if (error)
			return error;

 ...
 * boundary.  Returns 0 (and a page) if successful, -ENOTBLK if we cannot grab
 * the page, or the usual negative errno.
 */
int
xfile_obj_get_page(
	struct xfile		*xf,
	...
	struct page		**pagep)
{
	...
	/*
	 * We don't support passing fsdata to the caller, so if we get a
	 * non-null pointer we just bail out.
	 */
	if (fsdata != NULL) {
		int	ret;
...
/*
 * Release the page for a memory object.  The page must have been obtained by
 * xfile_obj_get_page.  Returns 0 or a negative errno.
 */
int
xfile_obj_put_page(
	struct xfile		*xf,

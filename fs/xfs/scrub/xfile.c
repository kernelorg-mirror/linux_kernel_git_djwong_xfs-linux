 ...
 * management; standard vfs locks (freezer and inode) are not taken.
 *
...
/*
 * Read a buffer from the xfile.
 *
 * We do not take any of the usual VFS locks or do any of the usual checks
 * because xfile users are responsible for concurrency control and keeping
 * xfiles away from userspace.  Read directly from the page cache.
 */
int
xfile_pread(
	struct xfile		*xf,
	...
	loff_t			offset)
{
	...
	gfp_t			gfp = kmem_flags_convert(KM_NOFS);
	unsigned int		pflags;
...
/*
 * Write a buffer to the xfile.
 *
 * We do not take any of the usual VFS locks or do any of the usual checks
 * because xfile users are responsible for concurrency control and keeping
 * xfiles away from userspace.  Write directly to the page cache.
 */
int
xfile_pwrite(
	struct xfile		*xf,

	...
	loff_t		size)
{
	 ...
	 * The tmpfs file is internal to XFS, so we skip timestamp updates
	 * and security checks.
	 */
	...
	loff_t		offset)
{
	...
	pflags = memalloc_nofs_save();
	ret = __kernel_write(xf->filp, buf, count, &offset);
	memalloc_nofs_restore(pflags);
...
xfile_seek_data(
	...
	loff_t		pos)
{
	return vfs_llseek(xf->filp, pos, SEEK_DATA);
}

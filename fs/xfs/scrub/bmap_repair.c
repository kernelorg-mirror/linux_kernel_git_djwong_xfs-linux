	...
	struct xfs_btree_cur	*bmap_cur)
{
	...
	struct xfs_ifork	*old_ifp = XFS_IFORK_PTR(rb->sc->ip, rb->whichfork);
	unsigned int		i;
	int			error;
	...
	struct xrep_bmap	*rb)
{
	 ...
	 */
	ifake->if_fork->if_nextents = xfarray_length(rb->bmap_records);
	if (ifake->if_fork->if_nextents <=
				XFS_IFORK_MAXEXT(sc->ip, rb->whichfork)) {

	...
	const struct xfs_rmap_irec	*rec)
{
	...
	error = xfs_ialloc_has_inodes_at_extent(sc->sa.ino_cur,
			rec->rm_startblock, rec->rm_blockcount, &keyfill);
	if (error)
		return error;
	if (keyfill != XFS_BTREE_KEYFILL_EMPTY)
		return -EFSCORRUPTED;
	...
	void			*priv)
{
	...
	for (loaded = 0; loaded < nr_wanted; loaded++, idx++) {
		do {
			error = xfarray_load_next(rb->bmap_records,
					&rb->array_cur, &rec);
			if (error)
				return error;
	...
	void			*priv)
{
	...
	error = xrep_newbt_relog_autoreap(&rb->new_fork_info);
	if (error)
		return error;
	...
	struct xrep_bmap	*rb)
{
	struct xfs_iext_cursor	icur;
	struct xfs_bmbt_irec	irec;
	struct xfs_ifork	*ifp = rb->new_fork_info.ifake.if_fork;
	xfarray_idx_t		array_cur;
	int			error;
	...
	struct xfs_btree_cur	*bmap_cur)
{
	...
	error = xfs_trans_reserve_more_inode(sc->tp, sc->ip,
			rb->bmap_bload.nr_blocks, 0, true);
	if (error)
		return error;
...
/* Ensure there are no overlaps in the file offset ranges. */
STATIC int
xrep_bmap_check_startoff(
	struct xrep_bmap	*rb)
{
	struct xfs_bmbt_irec	irec;
	...
	struct xrep_bmap	*rb)
{
	...
	error = xrep_bmap_check_startoff(rb);
	if (error)
		return error;
	...
	error = xrep_newbt_init_inode(&rb->new_fork_info, sc, rb->whichfork,
			&oinfo);
	if (error)
		return error;
	...
	error = xrep_newbt_destroy(&rb->new_fork_info);
	if (error)
		return error;
...
err_newbt:
	xrep_newbt_cancel(&rb->new_fork_info);
	return error;
}
	...
	bool			allow_unwritten)
{
	...
	switch (whichfork) {
	case XFS_DATA_FORK:
		max_bmbt_recs = MAXEXTNUM;
		break;
	case XFS_ATTR_FORK:
		max_bmbt_recs = MAXAEXTNUM;
		break;
	}

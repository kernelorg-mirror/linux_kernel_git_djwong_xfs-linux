	...
	struct xrep_bmap	*rb)
{
	 ...
	 * rmapbt.
	 */
	delta = rb->new_fork_info.ifake.if_blocks - rb->old_bmbt_block_count;
	...
	struct xfs_btree_cur	**bmap_curp)
{
	...
	*bmap_curp = NULL;
	...
	rb->iter = 0;
	*bmap_curp = xfs_bmbt_stage_cursor(sc->mp, sc->tp, sc->ip,
			&rb->new_fork_info.ifake);
	...
	struct xrep_bmap	*rb)
{
	...
	bmap_cur = xfs_bmbt_stage_cursor(sc->mp, sc->tp, sc->ip,
			&rb->new_fork_info.ifake);

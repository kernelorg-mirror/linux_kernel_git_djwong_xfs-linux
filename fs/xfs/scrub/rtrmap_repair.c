	...
	struct xrep_rtrmap	*rr)
{
	 ...
	 * new rtrmapbt.
	 */
	delta = ifake->if_blocks - mp->m_rrmapip->i_nblocks;
	mp->m_rrmapip->i_nblocks = ifake->if_blocks;
	...
	struct xrep_rtrmap	*rr)
{
	 ...
	 * aren't cross-linked with something else.  If the incore space
	 * reservation for the rtrmap inode is insufficient, this will refill
	 * it.
	 */
	error = xrep_reap_extents(rr->sc, &rr->old_rtrmapbt_blocks,
			&XFS_RMAP_OINFO_ANY_OWNER, XFS_AG_RESV_IMETA);
	if (error)
		return error;

	/*
	 * Ensure the proper reservation for the rtrmap inode so that we don't
	 * make bad decisions later.
	 */
...
{
	/* Ignore update if the scan failed already. */
	...
	return xchk_iscan_visited_ino(iscan, oi->oi_owner);
}
...
xrep_rtrmapbt_live_update(
	...
	void				*data)
{
	struct xfs_rmap_mod_params	*p = data;
	struct xrep_rtrmap		*rr;
	struct xfs_mount		*mp;
	struct xfs_btree_cur		*mcur;
	struct xfs_buf			*mhead_bp;
	...
	rr = container_of(nb, struct xrep_rtrmap, mod_hook);
	mp = rr->sc->mp;
	...
	struct xfs_scrub	*sc)
{
	 ...
	 */
	error = xfs_hook_add(&sc->mp->m_rtrmap_mod_hooks, &rr->mod_hook,
			xrep_rtrmapbt_live_update);
	if (error)
		goto out_records;
...
out_hook:
	xchk_iscan_abort(&rr->iscan);
	xfs_hook_del(&sc->mp->m_rtrmap_mod_hooks, &rr->mod_hook);
out_records:

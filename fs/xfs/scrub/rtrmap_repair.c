	...
	void				*data)
{
	...
	trace_xrep_rmap_live_update(&rr->sc->sr.rtg->rtg_group, action, p);
	...
	struct xrep_rtrmap	*rr)
{
	...
	error = xfs_rmap_hook_add(&sc->sr.rtg->rtg_group, &rr->rhook);
	if (error)
		goto out_iscan;
	return 0;
	...
	struct xrep_rtrmap	*rr)
{
	...
	xchk_iscan_abort(&rr->iscan);
	xfs_rmap_hook_del(&sc->sr.rtg->rtg_group, &rr->rhook);
	xchk_iscan_teardown(&rr->iscan);
	xfbtree_destroy(&rr->rtrmap_btree);
	xfsb_bitmap_destroy(&rr->old_rtrmapbt_blocks);
	mutex_destroy(&rr->lock);

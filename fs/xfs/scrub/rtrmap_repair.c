	...
	void				*priv)
{
	...
	if (!(irec->rc_startblock & XFS_RTREFC_COW_START) ||
	    irec->rc_refcount != 1)
		return -EFSCORRUPTED;

	rgbno = irec->rc_startblock - XFS_RTREFC_COW_START;
	return xbitmap_set(bitmap, rgbno, irec->rc_blockcount);
}
	...
	struct xrep_rtrmap	*rr)
{
	...
	struct xfs_refcount_irec low = {
		.rc_startblock	= XFS_RTREFC_COW_START,
	};

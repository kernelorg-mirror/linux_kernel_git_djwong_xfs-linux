	...
	void				*data)
{
	...
	/*
	 * The fs is frozen, which means that nobody should be holding any data
	 * file's ILOCK /and/ waiting for the rt metadata inodes.  However,
	 * this is technically an ABBA deadlock vector, so we'll back off if we
	 * can't get the ILOCK.
	 */
	while (!(locked = xfs_ilock_nowait(ip, XFS_ILOCK_EXCL)) && --retries)
		delay(HZ / 10);
	if (!locked) {
		error = -EDEADLOCK;
		goto out_rele;

	...
	struct xfs_inode	*ip)
{
	...
	if (xrep_is_rtmeta_ino(rr->sc, ip->i_ino))
		return 0;
	...
	struct xrep_rtrmap	*rr)
{
	...
	error = xrep_newbt_init_inode(&rr->new_btree_info, sc, XFS_DATA_FORK,
			&oinfo);
	if (error)
		return error;
	rmap_cur = xfs_rtrmapbt_stage_cursor(sc->mp, mp->m_rrmapip,
			&rr->new_btree_info.ifake);

	/* Compute how many blocks we'll need for the rmaps collected. */
	error = xfs_btree_bload_compute_geometry(rmap_cur, &rr->rtrmap_bload,
			rr->nr_records);
	if (error)
		goto err_cur;
	...
	error = xfs_trans_reserve_more_inode(sc->tp, mp->m_rrmapip,
			rr->rtrmap_bload.nr_blocks, 0, true);
	if (error)
		goto err_cur;
	...
	error = xfs_btree_bload(rmap_cur, &rr->rtrmap_bload, rr);
	if (error)
		goto err_mcur;
	...
	void				*data)
{
	...
	trace_xrep_rtrmap_live_update(mp, action, p);
	...
	mcur = xfs_rtrmapbt_mem_cursor(mp, tp, mhead_bp, rr->rtrmap_btree);
	...
	struct xfs_scrub	*sc)
{
	...
	/* Retry iget every tenth of a second for up to 2 seconds. */
	xchk_iscan_start(&rr->iscan, 2000, 100);
	...
	error = xfs_rtrmap_hook_add(sc->mp, &rr->hooks);
	if (error)
		goto out_records;
...
out_hook:
	xchk_iscan_abort(&rr->iscan);
	xfs_rtrmap_hook_del(sc->mp, &rr->hooks);
out_records:

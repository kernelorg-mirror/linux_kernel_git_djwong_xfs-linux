 ...
 * Returns 0 if we're ready to swap; a negative errno if something went wrong;
 * or -ENOATTR if there's nothing to swap and all we need to do is erase
 * @sc->ip's attr fork.  In this case, we'll return with both inodes ILOCKed
 * and a scrub transaction suitable for zapping that fork.
 */
STATIC int
	...
	struct xrep_parent	*rp)
{
	...
	/*
	 * If the temporary file has no xattrs at all, all we need to do is
	 * erase @sc->ip's attr fork.
	 */
	if (!xfs_inode_hasattr(sc->tempip))
		return -ENOATTR;

	/* If the file being repaired has an attr fork, we're ready to swap. */
	...
	struct xrep_parent	*rp)
{
	...
	/*
	 * The temp file has no xattrs or parent pointers, so we need only to
	 * erase the attr fork.  Roll the transaction after erasure to release
	 * the inode from the scrub transaction.
	 */
	if (zap_attrfork) {
	...
	struct xrep_parent	*rp)
{
	...
	descr = kasprintf(XCHK_GFP_FLAGS,
			"XFS (%s): inode 0x%llx parent pointer entries",
	...
	descr = kasprintf(XCHK_GFP_FLAGS,
			"XFS (%s): inode 0x%llx parent pointer names",
			sc->mp->m_super->s_id, sc->ip->i_ino);
	...
	descr = kasprintf(XCHK_GFP_FLAGS,
			"XFS (%s): inode 0x%llx parent pointer xattr names",
			sc->mp->m_super->s_id, sc->ip->i_ino);
	...
	descr = kasprintf(XCHK_GFP_FLAGS,
			"XFS (%s): inode 0x%llx parent pointer xattr values",
			sc->mp->m_super->s_id, sc->ip->i_ino);

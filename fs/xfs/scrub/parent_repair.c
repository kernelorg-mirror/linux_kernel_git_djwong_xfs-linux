	...
	struct xrep_parent	*rp)
{
	...
	error = xfs_bmap_add_attrfork(sc->tempip,
	...
	struct xrep_parent	*rp)
{
	 ...
	 * possible parent pointer updates during the time when we did not hold
	 * the ILOCK.  There should not be any pptr updates to replay, but
	 * check anyway.
	 */

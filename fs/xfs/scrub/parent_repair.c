...
struct xrep_pptrs {
	...
	/* Mutex protecting namebuf, parent_ptrs, pptr_names. */
	struct mutex		lock;
...
{
	struct xfs_name		dirname = {
		.name		= rp->namebuf,
		.len		= pptr->namelen,
	};
	struct xfs_scrub	*sc = rp->sc;

	/* Set up the da args structure. */
	memset(&rp->args, 0, sizeof(struct xfs_da_args));
	rp->args.attr_filter	= XFS_ATTR_PARENT;
	rp->args.dp		= sc->tempip;
	rp->args.geo		= sc->mp->m_attr_geo;
	rp->args.name		= (const unsigned char *)&pptr->attr_key;
	rp->args.namelen	= sizeof(pptr->attr_key);
	rp->args.trans		= sc->tp;
	rp->args.whichfork	= XFS_ATTR_FORK;

	if (pptr->action == XREP_PPTR_ADD) {
		/* Create parent pointer. */
		rp->args.attr_flags = XATTR_CREATE;
		rp->args.value = rp->namebuf;
		rp->args.valuelen = dirname.len;

		trace_xrep_pptr_createname(sc->tempip, &pptr->attr_key,
				&dirname);
	} else {
		/* Remove parent pointer. */
		trace_xrep_pptr_removename(sc->tempip, &pptr->attr_key,
				&dirname);
	}

	return xfs_attr_set(&rp->args);
}
	...
	struct xrep_pptrs	*rp)
{
	...
	foreach_xfarray_idx(rp->parent_ptrs, array_cur) {
		...
		error = xfblob_load(rp->pptr_names, pptr.name_cookie,
				rp->namebuf, pptr.namelen);
		if (error)
			goto out_unlock;
		rp->namebuf[MAXNAMELEN - 1] = 0;
	...
	xfs_dir2_dataptr_t	diroffset)
{
	struct xrep_pptr	pptr = {
		...
		.namelen	= name->len,
		.attr_key	= {
			.p_ino		= cpu_to_be64(dp->i_ino),
			.p_gen		= cpu_to_be32(VFS_IC(dp)->i_generation),
			.p_diroffset	= cpu_to_be32(diroffset),
		},
	};
	int			error;

	trace_xrep_pptr_add_pointer(rp->sc->tempip, &pptr.attr_key, name);
	...
	xfs_dir2_dataptr_t	diroffset)
{
	...
	trace_xrep_pptr_remove_pointer(rp->sc->tempip, &pptr.attr_key, name);

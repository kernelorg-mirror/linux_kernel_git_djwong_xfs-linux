	...
	const struct xrep_pptr	*pptr)
{
	...
	case XREP_PPTR_ADD:
		...
		trace_xrep_parent_replay_parentadd(sc->tempip, &rp->name,
				&rp->pptr);

		error = xfs_parent_set(sc->tempip, sc->ip->i_ino, &rp->name,
				&rp->pptr, &rp->pptr_scratch);
		if (error)
			return error;
		break;
	case XREP_PPTR_REMOVE:
		/* Remove parent pointer. */
		trace_xrep_parent_replay_parentremove(sc->tempip, &rp->name,
				&rp->pptr);

		error = xfs_parent_unset(sc->tempip, sc->ip->i_ino, &rp->name,
				&rp->pptr, &rp->pptr_scratch);
		if (error)
			return error;
		break;

...
struct xrep_parent {
	...
	/*
	 * This is the parent that we're going to set on the dotdot entry in
	 * the directory.
	 */
...
/* Reset a directory's dotdot entry. */
STATIC int
xrep_parent_reset(
	struct xrep_parent	*rp)
{
	...
	return xrep_parent_reset_dir(sc, rp->pscan.parent_ino);
}
	...
	struct xrep_parent	*rp)
{
	...
	xrep_adoption_compute_blkres(sc, &rp->adoption);

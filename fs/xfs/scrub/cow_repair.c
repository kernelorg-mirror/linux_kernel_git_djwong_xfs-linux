	...
	const struct xfs_refcount_irec	*src)
{
	...
	dst->rc_startblock = src->rc_startblock & ~XFS_REFC_COW_START;
	dst->rc_blockcount = src->rc_blockcount;
	dst->rc_refcount = src->rc_refcount;
	...
	struct xrep_cow			*xc)
{
	...
	rc_low.rc_startblock = xc->irec_startbno + XFS_REFC_COW_START;
	rc_high.rc_startblock = xc->irec_startbno + XFS_REFC_COW_START +
	...
	struct xfs_scrub	*sc)
{
	...
	/* Reap as many of the old blocks as we can. */
	error = xrep_reap_extents(sc, &xc->old_cowfork_fsblocks,

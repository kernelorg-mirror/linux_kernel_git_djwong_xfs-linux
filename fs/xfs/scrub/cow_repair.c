	...
	xfs_filblks_t		maxlen)
{
	...
	error = xfs_trans_reserve_more(sc->tp, 0, maxlen);
	if (error)
		return error;

	xfs_rtbitmap_lock(sc->tp, sc->mp);

	error = xfs_rtallocate_extent(sc->tp, 0, 1, maxlen, &rtxlen, 0, 1,
			&rtx);
	if (error)
		return error;
	if (rtx == NULLRTEXTNO)
	...
	xfs_extlen_t		*blockcount)
{
	...
	if (XFS_IS_REALTIME_INODE(sc->ip))
		error = xrep_cow_alloc_rt(sc, &rep, endoff - startoff);
	else
		error = xrep_cow_alloc(sc, &rep, endoff - startoff);
	if (error)
		return error;

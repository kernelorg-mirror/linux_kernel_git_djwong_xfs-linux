	...
	void				*priv)
{
	...
	if (rec->rc_domain != XFS_RCDOM_SHARED || rec->rc_refcount < 2)
		return -EFSCORRUPTED;
	...
	struct xrep_cow			*xc)
{
	...
	rc_low.rc_domain = rc_high.rc_domain = XFS_RCDOM_SHARED;
	...
	rc_low.rc_startblock = xc->irec_startbno;
	rc_high.rc_startblock = xc->irec_startbno + xc->irec.br_blockcount - 1;
	rc_low.rc_domain = rc_high.rc_domain = XFS_RCDOM_COW;

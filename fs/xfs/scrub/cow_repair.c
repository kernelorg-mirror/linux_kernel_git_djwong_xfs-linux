	...
	void				*priv)
{
	...
	if (XFS_IS_REALTIME_INODE(xc->sc->ip))
		fsbno = xfs_rgbno_to_rtb(cur->bc_ino.rtg, rrec.rc_startblock);
	else
		fsbno = xfs_agbno_to_fsb(cur->bc_ag.pag, rrec.rc_startblock);
	...
	void				*priv)
{
	...
	if (XFS_IS_REALTIME_INODE(xc->sc->ip))
		fsbno = xfs_rgbno_to_rtb(cur->bc_ino.rtg, xc->next_bno);
	else
		fsbno = xfs_agbno_to_fsb(cur->bc_ag.pag, xc->next_bno);
	...
	void				*priv)
{
	...
	if (XFS_IS_REALTIME_INODE(xc->sc->ip))
		fsbno = xfs_rgbno_to_rtb(cur->bc_ino.rtg, rec_bno);
	else
		fsbno = xfs_agbno_to_fsb(cur->bc_ag.pag, rec_bno);

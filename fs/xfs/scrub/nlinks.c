	...
	int			delta)
{
	...
	error = xchk_nlinks_get_record(xnc, ino, &nl);
	if (error)
		return error;
	...
	struct xfs_inode	*ip)
{
	...
	if (total_links != VFS_I(ip)->i_nlink)
		xchk_ino_set_corrupt(sc, ip->i_ino);

	/* Directories should have at least one child due to dot entry. */
	if (S_ISDIR(VFS_I(ip)->i_mode) && obs.child < 1)
		xchk_ino_set_corrupt(sc, ip->i_ino);
	...
	struct xchk_nlink_ctrs	*xnc)
{
	 ...
	 */
	mutex_lock(&xnc->lock);
	while (!(error = xfarray_load_next(xnc->nlinks, &nr, &nl))) {
		xfs_ino_t	ino = nr - 1;

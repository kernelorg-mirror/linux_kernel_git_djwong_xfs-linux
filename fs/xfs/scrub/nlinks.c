	...
	int			delta)
{
	...
	trace_xchk_nlinks_update_incore(xnc->sc->mp, dir, ino, delta,
			__return_address);
	...
	void				*data)
{
	...
	error = xchk_nlinks_update_incore(xnc, p->dir, p->ino, p->delta);
	if (error)
		xnc->hook_dead = true;
	...
	unsigned		type)
{
	...
	error = xchk_nlinks_update_incore(xwd->xnc, xwd->dir, ino, 1);
	if (error) {
		xchk_set_incomplete(xwd->xnc->sc);
	...
	xfs_ino_t		ino)
{
	...
	return xchk_nlinks_update_incore(xnc, NULLFSINO, ino, 1);
}
...
/*
 * Update the nlinks scan cursor so that the nlinks hooks will begin to capture
 * nlink updates being made by ongoing transactions.  Do this with the AGI held
 * so that no other threads can create new inodes in this AG.
 */
static inline void
xchk_nlinks_move_cursor(
	struct xchk_nlinks	*xnc,
	xfs_agnumber_t		agno,
	xfs_agino_t		agino,
	xfs_ino_t		*ino)
{
	mutex_lock(&xnc->lock);
	*ino = XFS_AGINO_TO_INO(xnc->sc->mp, agno, agino);
	xnc->last_ino = *ino - 1;
	mutex_unlock(&xnc->lock);
}
...
next_ag:
	...
	error = xchk_iwalk_find_next(sc->mp, sc->tp, agi_bp, pag, &agino);
	if (error)
		goto out_buf;
	if (agino == NULLAGINO) {
		xchk_nlinks_move_cursor(xnc, agno + 1, 0, ino);
		xfs_trans_brelse(sc->tp, agi_bp);
		xfs_perag_put(pag);
		goto next_ag;
	}

	xchk_nlinks_move_cursor(xnc, agno, agino, ino);

	...
	xfs_nlink_t		*nlinks)
{
	...
	error = xfarray_load(xnc->nlinks, ino, nlinks);
	if (error == -ENODATA) {
		/*
	...
	int			delta)
{
	...
	error = xfarray_store(xnc->nlinks, ino, &nlinks);
	if (error == -EFBIG) {
		/*
	...
	void				*data)
{
	...
	if (!xchk_iscan_visited(&xnc->iscan, p->dp) ||
	    xchk_iscan_aborted(&xnc->iscan))
		goto out_unlock;

	error = xchk_nlinks_update_incore(xnc, p->dp, p->ino, p->delta);
	if (error)
		xchk_iscan_abort(&xnc->iscan);
	...
	unsigned		type)
{
	...
	if (xchk_iscan_aborted(&xnc->iscan))
		goto out_incomplete;
	...
	struct xchk_nlinks	*xnc)
{
	...
	while ((ret = xchk_iscan_advance(sc, iscan)) == 1) {
		...
		ret = xchk_iscan_iget(sc, iscan, &ip);
		if (ret == -EAGAIN) {
			delay(HZ / 10);
			continue;
		}
		...
		if (S_ISDIR(VFS_I(ip)->i_mode))
			ret = xchk_nlinks_dir(xnc, ip);
		xchk_iscan_visit(iscan, ip);
		xchk_irele(sc, ip);
		if (ret)
			break;
	...
	void			*data)
{
	...
	ino_nlink = ip ? VFS_I(ip)->i_nlink : 0;
	...
	struct xchk_nlinks	*xnc)
{
	...
	xchk_iscan_lock(&xnc->iscan);
	while (!(error = xfarray_load_next(xnc->nlinks, &nr, &nlink))) {
		xchk_iscan_unlock(&xnc->iscan);
	...
	struct xchk_nlinks	*xnc)
{
	...
	xchk_iscan_start(&xnc->iscan, 20);

	error = -ENOMEM;
	xnc->nlinks = xfarray_create("link counts", sizeof(xfs_nlink_t));
	if (!xnc->nlinks)
		goto out_teardown;

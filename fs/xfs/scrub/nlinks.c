	...
	void				*data)
{
	...
	xnc = container_of(nb, struct xchk_nlink_ctrs, hooks.delta_hook.nb);
	...
	if (action == XFS_DIRENT_CHILD_DELTA) {
	...
	} else if (action == XFS_DIRENT_SELF_DELTA) {
		/*
		...
		if (error)
			goto out_abort;
	} else if (action == XFS_DIRENT_BACKREF_DELTA) {
		/*
	...
	void			*priv)
{
	...
	xfs_dirent_hook_del(xnc->sc->mp, &xnc->hooks);
	...
	struct xchk_nlink_ctrs	*xnc)
{
	...
	error = xfs_dirent_hook_add(mp, &xnc->hooks);
	if (error)
		goto out_teardown;

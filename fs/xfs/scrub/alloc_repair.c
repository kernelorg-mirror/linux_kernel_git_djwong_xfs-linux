...
struct xrep_abt {
	...
	/* Current incore maxlevels for the two btrees we're rebuilding */
	unsigned int		old_bno_level;
	unsigned int		old_cnt_level;
};
	...
	struct xrep_abt		*ra)
{
	 ...
	 * level taller than the old btree.  Update the perag struct's btree
	 * height to the max of the old and new levels so that we don't trip
	 * the verifiers when writing the new btree blocks to disk and the AIL
	 * won't trip over the old btree blocks if it writes one of them out
	 * before we can invalidate them.
	 */
	pag->pagf_levels[XFS_BTNUM_BNOi] = max(ra->old_bno_level,
					       ra->bno_bload.btree_height);
	pag->pagf_levels[XFS_BTNUM_CNTi] = max(ra->old_cnt_level,
					       ra->cnt_bload.btree_height);
...
xrep_abt_remove_old_trees(
	struct xrep_abt		*ra)
{
	uint8_t			*levels = ra->sc->sa.pag->pagf_levels;
	int			error;
	...
	/*
	 * Now that we've invalidated the old blocks, set the incore btree
	 * height to the new levels.
	 */
	levels[XFS_BTNUM_BNOi] = ra->bno_bload.btree_height;
	levels[XFS_BTNUM_CNTi] = ra->cnt_bload.btree_height;
	...
	struct xfs_scrub	*sc)
{
	struct xrep_abt		*ra;
	struct xfs_mount	*mp = sc->mp;
	uint8_t			*levels = sc->sa.pag->pagf_levels;
	int			error;
	...
	if (!ra)
		return -ENOMEM;
	ra->sc = sc;
	ra->old_bno_level = levels[XFS_BTNUM_BNOi];
	ra->old_cnt_level = levels[XFS_BTNUM_CNTi];
...
out_ra:
	if (error) {
		levels[XFS_BTNUM_BNOi] = ra->old_bno_level;
		levels[XFS_BTNUM_CNTi] = ra->old_cnt_level;
	}
	kmem_free(ra);
	return error;
}

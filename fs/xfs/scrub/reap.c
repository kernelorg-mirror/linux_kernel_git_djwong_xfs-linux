...
#else
# define xreap_imeta_rtextent	NULL
#endif /* CONFIG_XFS_RT */
	...
	const struct xfs_owner_info	*oinfo)
{
	...
	error = xrtb_bitmap_walk(bitmap, xreap_imeta_rtextent, &rs);
	if (error)
		return error;
	...
	struct xfs_bmbt_irec		*imap)
{
	...
	if (crosslinked) {
		 ...
		 * roll and retry.
		 */
		error = xrep_bmapi_binval(sc, ip, whichfork, imap);

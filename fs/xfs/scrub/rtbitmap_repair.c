...
copy_words:
	...
	wordoff++;
	wordcnt = XFS_BITTOWORD(mp, last_rbmblock_rtx - last_group_rtx);
	if (wordcnt > 0) {
	...
	xfs_rtword_t		mask)
{
	...
	pos = (rtx & ~XREP_RTBMP_WORDMASK) >> XFS_NBBYLOG;
	error = xfile_obj_load(rb->sc->xfile, &word, sizeof(word), pos);
	if (error)
		return error;

	trace_xrep_rgbitmap_or(rb->sc->mp, rtx, pos, mask, word);
	...
	xfs_rgblock_t		rgbno)
{
	...
	nextpos = nextrtx >> XFS_NBBYLOG;

	while (pos < nextpos) {
		loff_t	rem;
		size_t	count = min_t(loff_t, nextpos - pos,
				      mp->m_sb.sb_blocksize);

		/* Try to get us aligned to an even blocksize. */
		rem = pos & mp->m_blockmask;
		if (rem)

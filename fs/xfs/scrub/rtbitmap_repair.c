	...
	xfs_rtblock_t		rtbno)
{
	...
	if (!xfs_verify_rtext(mp, rb->next_rtbno, rtbno - rb->next_rtbno))
		return -EFSCORRUPTED;
	...
	if (mod)
		return -EFSCORRUPTED;
	next_ext = div_u64_rem(rtbno, mp->m_sb.sb_rextsize, &mod);
	if (mod)
		return -EFSCORRUPTED;
	...
	if (rb->sc->sr.refc_cur) {
		error = xfs_refcount_scan_keyfill(rb->sc->sr.refc_cur,
				rb->next_rtbno, rtbno - rb->next_rtbno,
				&keyfill);
		if (error)
			return error;
	...
	struct xrep_rtbmp	*rb)
{
	...
	if (error || mp->m_sb.sb_rblocks == rb->next_rtbno)
		return error;

	/*
	 * Mark as free every possible rt extent from the last one we saw to
	 * the end of the rt device.
	 */
	rblocks = rounddown_64(mp->m_sb.sb_rblocks, mp->m_sb.sb_rextsize);
	return xrep_rtbitmap_mark_free(rb, rblocks);
}

	...
	struct xchk_rgbitmap	*rgb)
{
	...
	/*
	 * Grab support for atomic file mapping exchanges before we allocate
	...
	xfs_rtword_t		*word)
{
	...
	error = xfile_load(rgb->sc->xfile, &urk,
	...
	const xfs_rtword_t	word)
{
	...
	return xfile_store(rgb->sc->xfile, &urk,
			sizeof(union xfs_rtword_raw),
			wordoff << XFS_WORDLOG);
}
...
{
	return xfile_store(rgb->sc->xfile, word, nr_words << XFS_WORDLOG,
			wordoff << XFS_WORDLOG);
}
...
xfbmp_copyout(
	...
	xrep_wordcnt_t		nr_words)
{
	return xfile_load(rgb->sc->xfile, word, nr_words << XFS_WORDLOG,
			wordoff << XFS_WORDLOG);
}

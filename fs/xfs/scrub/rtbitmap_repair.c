	...
	unsigned int		*resblks)
{
	...
	/*
	 * Create an xfile to hold our reconstructed bitmap.  Ask for enough
	 * bytes to handle the unused bytes in the last block of the file.
	 */
	rtbmp_words = roundup_64(
	...
	struct xrep_rgbmp	*rb)
{
	...
	if (wordcnt > 0) {
		...
		error = bitmap_copyin(rb, wordoff, p, wordcnt);
		if (error)
			goto out_rele;
	...
	error = bitmap_load(rb, wordoff, &xfile_word);
	if (error)
		goto out_rele;
	...
	error = bitmap_store(rb, wordoff, xfile_word);
	if (error)
		goto out_rele;
...
copy_words:
	...
	wordoff++;
	wordcnt = rtxlen_to_wordcnt(last_rbmblock_rtx - last_group_rtx + 1);
	if (wordcnt > 0) {
		xfs_rtword_raw_t	*p;
	...
	xfs_rtword_t		mask)
{
	...
	error = bitmap_load(rb, wordoff, &word);
	if (error)
		return error;

	trace_xrep_rgbitmap_or(rb->sc->mp, wordoff, mask, word);

	return bitmap_store(rb, wordoff, word | mask);
}
	...
	xfs_rgblock_t		rgbno)
{
	...
	if (rb->sc->sr.refc_cur) {
		error = xfs_refcount_scan_keyfill(rb->sc->sr.refc_cur,
				rb->next_rgbno, rgbno - rb->next_rgbno,
				&keyfill);
		if (error)
			return error;
		...
		error = xfs_refcount_scan_keyfill(rb->sc->sr.refc_cur,
				rb->next_rgbno + XFS_REFC_COW_START,
				rgbno - rb->next_rgbno,
				&keyfill);
		if (error)
			return error;
	...
	while (wordoff < endwordoff) {
		...
		error = bitmap_copyin(rb, wordoff, rb->sc->buf, wordcnt);
		if (error)
			return error;

...
struct xrep_rtbmp {
	...
	/* xfile holding the new bitmap contents. */
	struct file		*bmpfile;
	...
	xfs_rtword_t		mask)
{
	...
	error = xfile_io(rb->bmpfile, XFILE_IO_READ, &pos, &word, sizeof(word));
	if (error)
		return error;
	...
	word |= mask;
	pos = rtword_off(rt_ext);
	return xfile_io(rb->bmpfile, XFILE_IO_WRITE, &pos, &word, sizeof(word));
}
	...
	xfs_rtblock_t		next)
{
	...
	while (pos < endpos) {
		...
		error = xfile_io(rb->bmpfile, XFILE_IO_WRITE, &pos, rb->sc->buf,
				count);
		if (error)
			return error;
	}
...
/* Retrieve one block from the rtbitmap file. */
STATIC int
xrep_rtbitmap_get_buf(
	struct xfs_scrub	*sc,
	xfs_fileoff_t		off,
	struct xfs_buf		**bpp)
{
	...
	struct xfs_scrub	*sc)
{
	...
	rb.bmpfile = xfile_create("rtbitmap", bmp_bytes);
	if (IS_ERR(rb.bmpfile))
		return PTR_ERR(rb.bmpfile);
	...
	error = xfs_swapext_atomic(&sc->tp, &req);
out:
	fput(rb.bmpfile);
	return error;
}

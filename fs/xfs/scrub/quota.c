	...
	void			*priv)
{
	 ...
	 * the hard limit.
	 */
	if (dq->q_blk_hardlimit > mp->m_sb.sb_dblocks)
		xchk_fblock_set_warning(sc, XFS_DATA_FORK, offset);
	if (dq->q_blk_softlimit > dq->q_blk_hardlimit)
		xchk_fblock_set_corrupt(sc, XFS_DATA_FORK, offset);

	if (dq->q_ino_hardlimit > M_IGEO(mp)->maxicount)
		xchk_fblock_set_warning(sc, XFS_DATA_FORK, offset);
	if (dq->q_ino_softlimit > dq->q_ino_hardlimit)
		xchk_fblock_set_corrupt(sc, XFS_DATA_FORK, offset);

	if (dq->q_rtb_hardlimit > mp->m_sb.sb_rblocks)
		xchk_fblock_set_warning(sc, XFS_DATA_FORK, offset);
	if (dq->q_rtb_softlimit > dq->q_rtb_hardlimit)
		xchk_fblock_set_corrupt(sc, XFS_DATA_FORK, offset);
	...
	if (xfs_sb_version_hasreflink(&mp->m_sb)) {
		if (mp->m_sb.sb_dblocks < dq->q_bcount)
	...
	if (dq->q_icount > fs_icount || dq->q_rtbcount > mp->m_sb.sb_rblocks)
		xchk_fblock_set_corrupt(sc, XFS_DATA_FORK, offset);
	...
	if (dq->q_blk_hardlimit != 0 &&
	    dq->q_bcount > dq->q_blk_hardlimit)
		xchk_fblock_set_warning(sc, XFS_DATA_FORK, offset);

	if (dq->q_ino_hardlimit != 0 &&
	    dq->q_icount > dq->q_ino_hardlimit)
		xchk_fblock_set_warning(sc, XFS_DATA_FORK, offset);

	if (dq->q_rtb_hardlimit != 0 &&

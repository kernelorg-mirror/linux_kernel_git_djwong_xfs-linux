	...
	struct xrep_refc	*rr)
{
	 ...
	 */
	descr = kasprintf(XCHK_GFP_FLAGS, "XFS (%s): AG %u rmap bag",
			sc->mp->m_super->s_id, sc->sa.pag->pag_agno);
	...
	struct xfs_scrub	*sc)
{
	...
	descr = kasprintf(XCHK_GFP_FLAGS, "XFS (%s): AG %u refcount records",
			mp->m_super->s_id, sc->sa.pag->pag_agno);

...
struct xrep_refc {
	...
	/* Current incore maxlevels for the refcount btree. */
	...
	struct xrep_refc	*rr)
{
	 ...
	 */
	pag->pagf_refcount_level = max(rr->old_level,
				       rr->refc_bload.btree_height);
	...
	struct xrep_refc	*rr)
{
	 ...
	 * height to the new level and reset the block reservation.
	 */
	sc->sa.pag->pagf_refcount_level = rr->refc_bload.btree_height;
	...
	struct xfs_scrub	*sc)
{
	...
	if (!rr)
		return -ENOMEM;
	rr->sc = sc;
	rr->old_level = sc->sa.pag->pagf_refcount_level;
...
out_rr:
	if (error)
		sc->sa.pag->pagf_refcount_level = rr->old_level;
	kmem_free(rr);
	return error;
}

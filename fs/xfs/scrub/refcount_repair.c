	...
	bool			next_valid)
{
	...
	foreach_xfarray_item(rmap_bag, i, rrm)
	...
	struct xrep_refc	*rr)
{
	...
	/* Set up some storage */
	rmap_bag = xfarray_create("rmap bag", sizeof(struct xrep_refc_rmap));
	if (IS_ERR(rmap_bag)) {
		...
		while (stack_sz) {
			...
			foreach_xfarray_item(rmap_bag, i, rrm) {
				if (RRM_NEXT(rrm) != nbno)
					continue;
				error = xfarray_nullify(rmap_bag, i);
				if (error)
					goto out_bag;
				stack_sz--;
			}
	...
	struct xfs_scrub	*sc)
{
	...
	/* Set up some storage */
	rr->refcount_records = xfarray_create("refcount records",

	...
	struct xfs_inode	*ip)
{
	...
	if (!sc->buf)
		return -ENOMEM;
	sc->buf_cleanup = (void (*)(void *))xqcheck_teardown;
...
xqcheck_dqacct_obj_cmpfn(
	...
	const void			*obj)
{
	const unsigned long long	*key = arg->key;
...
static const struct rhashtable_params xqcheck_dqacct_hash_params = {
	.min_size		= 32,
	.key_len		= sizeof(struct xfs_trans *),
	...
	void			*data)
{
	...
	if (!dqa) {
		...
		dqa->tp = (unsigned long long)p->tp;
	...
	void			*data)
{
	...
	if (XFS_QM_ISUDQ(p->dqp))
		counts = xqc->ucounts;
	else if (XFS_QM_ISGDQ(p->dqp))
		counts = xqc->gcounts;
	else if (XFS_QM_ISPDQ(p->dqp))
		counts = xqc->pcounts;
	else
		return NOTIFY_DONE;
	...
	struct xfs_inode	*ip)
{
	...
	id = xfs_qm_id_for_quotatype(ip, XFS_DQ_USER);
	error = xqcheck_update_incore(xqc, xqc->ucounts, id, 1, nblks, rtblks);
	if (error)
		goto out_ilock;

	id = xfs_qm_id_for_quotatype(ip, XFS_DQ_GROUP);
	error = xqcheck_update_incore(xqc, xqc->gcounts, id, 1, nblks, rtblks);
	if (error)
		goto out_ilock;

	id = xfs_qm_id_for_quotatype(ip, XFS_DQ_PROJ);
	...
	void			*priv)
{
	...
	if (xcdq.icount != dqp->q_icount)
		xchk_qcheck_set_corrupt(xqc->sc, dqtype, dqp->q_id);

	if (xcdq.bcount != dqp->q_bcount)
		xchk_qcheck_set_corrupt(xqc->sc, dqtype, dqp->q_id);

	if (xcdq.rtbcount != dqp->q_rtbcount)
		xchk_qcheck_set_corrupt(xqc->sc, dqtype, dqp->q_id);
	...
	unsigned int		dqtype)
{
	 ...
	 * hit a corruption and are exiting early.
	 */
	if (error == -ENODATA || error == -ECANCELED)
		error = 0;
	...
	unsigned int		dqtype)
{
	...
	error = xfs_qm_dqiterate(sc->mp, dqtype, xqcheck_compare_dquot,
			xqc);
	...
	struct xqcheck		*xqc)
{
	...
	error = -ENOMEM;
	if (xfs_this_quota_on(sc->mp, XFS_DQ_USER)) {
	...
	if (xfs_this_quota_on(sc->mp, XFS_DQ_GROUP)) {
	...
	if (xfs_this_quota_on(sc->mp, XFS_DQ_PROJ)) {
	...
	struct xfs_scrub	*sc)
{
	...
	if (xqc->ucounts) {
		error = xqcheck_compare_dqtype(xqc, XFS_DQ_USER);
		if (error)
			goto out;
	}
	if (xqc->gcounts) {
		error = xqcheck_compare_dqtype(xqc, XFS_DQ_GROUP);
		if (error)
			goto out;
	}
	if (xqc->pcounts) {
		error = xqcheck_compare_dqtype(xqc, XFS_DQ_PROJ);
		if (error)
			goto out;
	}

	...
	void			*data)
{
	...
	if (!xchk_iscan_visited(&xqc->iscan, p->ip) ||
	    xchk_iscan_aborted(&xqc->iscan))
		goto out_unlock;
 ...
 */
static int
xqcheck_apply_live_dquot_deltas(
	struct notifier_block	*nb,
	unsigned long		arg,
	void			*data)
{
	...
	struct xfs_inode	*ip)
{
	...
	__xchk_iscan_visit(&xqc->iscan, ip);
	...
	xfs_dqtype_t		dqtype)
{
	...
	xchk_iscan_lock(&xqc->iscan);
	while (!(error = xfarray_load_next(counts, &nr, &xcdq))) {
		xfs_dqid_t	id = nr - 1;
	...
	struct xqcheck		*xqc)
{
	...
	error = xfs_hook_add(&qi->qi_apply_dquot_deltas_hooks,
			&xqc->apply_hook, xqcheck_apply_live_dquot_deltas);
	if (error)
		goto out_teardown;

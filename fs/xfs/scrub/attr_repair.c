	...
	struct xfs_buf		**bpp)
{
	struct xrep_buf_scan	scan = {
		...
		.max_daddrs	= xrep_max_buf_len(mp, max_len),
	...
	const struct xrep_xattr_key	*key)
{
	...
	error = xfblob_load(rx->xattr_blobs, key->value_cookie, value,
			key->valuelen);
	if (error)
		return error;
	...
	struct xrep_xattr	*rx)
{
	...
	while (!(error = xfarray_load_next(rx->xattr_records, &nr, &key))) {
	...
	for (offset = 0;
		...
		for (dabno = round_up(got.br_startoff, geo->fsbcount);
			...
			if (error)
				break;
			if (!bp) {
				/* No buffer found?  Advance by one dablock. */
				len = geo->fsbcount;
				continue;
			}

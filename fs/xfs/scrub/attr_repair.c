...
/* Set the owner for this xattr block to the file being repaired. */
STATIC int
xrep_xattr_reset_owner(
	struct xfs_scrub		*sc,
	struct xfs_buf			*bp)
{
	struct xfs_da3_blkinfo		*info = bp->b_addr;

	/* Set buffer metadata based on the type we find. */
	switch (info->hdr.magic) {
	case cpu_to_be16(XFS_ATTR3_LEAF_MAGIC):
		xfs_trans_buf_set_type(sc->tp, bp, XFS_BLFT_ATTR_LEAF_BUF);
		bp->b_ops = &xfs_attr3_leaf_buf_ops;
		break;
	case cpu_to_be16(XFS_DA3_NODE_MAGIC):
		xfs_trans_buf_set_type(sc->tp, bp, XFS_BLFT_DA_NODE_BUF);
...
{
	struct xfs_bmbt_irec		map;
	struct xfs_mount		*mp = xso->sc->mp;
	struct xfs_da_geometry		*geo = mp->m_attr_geo;
	 ...
	 * split of the space btrees, so we allow that many attr block owner
	 * updates per transaction.  This is an under-estimate since we're
	 * logging just the header, not full blocks.
	 */
	...
	for (offset = 0;
		...
		for (dabno = round_up(map.br_startoff, geo->fsbcount);
			...
			error = xrep_xattr_reset_owner(sc, bp);
			if (error) {

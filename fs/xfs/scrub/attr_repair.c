	...
	const struct xrep_xattr_pptr	*pptr)
{
	...
	case XREP_XATTR_PPTR_ADD:
		...
		trace_xrep_xattr_replay_parentadd(sc->tempip, &rx->name,
				&rx->pptr);

		error = xfs_parent_set(sc->tempip, sc->ip->i_ino, &rx->name,
				&rx->pptr, &rx->pptr_scratch);
		if (error) {
			ASSERT(error != -EEXIST);
			return error;
		}
	...
	case XREP_XATTR_PPTR_REMOVE:
		...
		trace_xrep_xattr_replay_parentremove(sc->tempip, &rx->name,
				&rx->pptr);

		error = xfs_parent_unset(sc->tempip, sc->ip->i_ino, &rx->name,
				&rx->pptr, &rx->pptr_scratch);
		if (error) {
	...
	struct xrep_xattr	*rx)
{
	...
	foreach_xfarray_idx(rx->pptr_recs, array_cur) {
		...
		error = xfblob_load(rx->pptr_names, pptr.name_cookie,
				rx->namebuf, pptr.namelen);
		if (error)
			goto out_unlock;
		rx->name.len = pptr.namelen;
		mutex_unlock(&rx->lock);

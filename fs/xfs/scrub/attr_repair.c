 ...
 * buffer will be passed to the callback one block at a time.
 */
STATIC int
xrep_xattr_find_buf(
	struct xfs_mount	*mp,
 ...
 *
 * The fn function /must/ stale any buffer that has no buffer ops attached.
 */
STATIC int
	...
	void			*priv)
{
	...
	for (offset = 0;
		...
		for (dabno = map.br_startoff, fsbno = map.br_startblock;
			...
			max_len = map.br_blockcount - (dabno - map.br_startoff);
			error = xrep_xattr_find_buf(mp, fsbno, max_len,
					can_read, &bp);
			if (error)
				break;
			if (!bp) {
				/* No buffer found?  Advance by one block. */
				len = ip->i_mount->m_attr_geo->fsbcount;
				continue;
			}
	...
	struct xrep_xattr	*rx)
{
	...
	if (sc->ip->i_afp->if_format == XFS_DINODE_FMT_LOCAL)
	...
	/* Read and walk every xattr buffer. */
	return xrep_attr_walk_blind(sc->ip, true, xrep_xattr_recover_block, rx);
}
	...
	struct xfs_inode	*ip)
{
	...
	error = xrep_attr_walk_blind(ip, false, xrep_xattr_stale_block, NULL);
	if (error)
		return error;

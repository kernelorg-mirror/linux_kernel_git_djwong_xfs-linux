	...
	xfs_dablk_t		dabno)
{
	 ...
	 */
	error = xfs_da_read_buf(rx->sc->tp, rx->sc->ip, dabno, -1, &bp,
			XFS_ATTR_FORK, NULL);
	if (error || !bp)
		return 0;
	...
	/* This looks like an attribute leaf; does it pass the verifier? */
	if (!xrep_buf_verify_struct(bp, &xfs_attr3_leaf_buf_ops))
		goto out;

	error = xrep_xattr_recover_leaf(rx, bp);

out:
	xfs_trans_brelse(rx->sc->tp, bp);
	return error;
}
	...
	struct xrep_xattr	*rx)
{
	...
	for_each_xfs_iext(ifp, &icur, &got) {
		...
		for (dabno = round_up(got.br_startoff, geo->fsbcount);
			...
			error = xrep_xattr_recover_leafblock(rx, dabno);
			if (error)
				return error;
		}
	}
...
/*
 * Try to stale any incore buffers that correspond to the start of this xattr
 * fork extent.  Remote value buffers can span multiple blocks within a single
 * extent mapping, so we have to loop through all possible lengths to make sure
 * we find and stale all possible buffers.
 *
 * We use TRYLOCK here (recall that we hold the ILOCK of the owner inode) so
 * that we skip any buffer that's locked on the assumption that we don't own
 * that block.
 */
STATIC xfs_filblks_t
xrep_xattr_stale_extent(
	struct xfs_scrub	*sc,
	struct xfs_bmbt_irec	*map)
{
	struct xfs_mount	*mp = sc->mp;
	struct xfs_buf		*bp;
	xfs_filblks_t		len = map->br_blockcount;
	...
	struct xfs_scrub	*sc)
{
	 ...
	 */
	if (sc->ip->i_d.di_format == XFS_DINODE_FMT_BTREE) {
		sc->ip->i_d.di_aformat = XFS_DINODE_FMT_LOCAL;
		ifp = XFS_IFORK_PTR(sc->ip, XFS_ATTR_FORK);
	...
	struct xfs_scrub	*sc)
{
	...
	ifp = XFS_IFORK_PTR(sc->ip, XFS_ATTR_FORK);
	for_each_xfs_iext(ifp, &icur, &got) {
		while (got.br_blockcount > 0) {
			xfs_filblks_t	len = xrep_xattr_stale_extent(sc, &got);

			got.br_blockcount -= len;
			got.br_startoff += len;
			got.br_startblock += len;
		}
	}
	...
	void				*priv)
{
	...
	return xfs_attr_set(rx->sc->ip, name, key->namelen, value,
			key->valuelen,

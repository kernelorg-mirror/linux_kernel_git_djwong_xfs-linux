	...
	struct xrep_xattr	**rxp)
{
	...
	descr = kasprintf(XCHK_GFP_FLAGS, "XFS (%s): inode 0x%llx xattr keys",
	...
	descr = kasprintf(XCHK_GFP_FLAGS, "XFS (%s): inode 0x%llx xattr names",
			sc->mp->m_super->s_id, sc->ip->i_ino);
	...
	if (xfs_has_parent(sc->mp)) {
		...
		descr = kasprintf(XCHK_GFP_FLAGS,
				"XFS (%s): inode 0x%llx xattr parent pointer entries",
				sc->mp->m_super->s_id, sc->ip->i_ino);
		...
		descr = kasprintf(XCHK_GFP_FLAGS,
				"XFS (%s): inode 0x%llx xattr parent pointer names",
				sc->mp->m_super->s_id, sc->ip->i_ino);

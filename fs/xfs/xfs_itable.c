	...
	bulkstat_one_fmt_pf	formatter)
{
	...
	if (xfs_bulkstat_already_done(breq->mp, breq->lastino))
		return 0;

	error = xfs_iwalk(breq->mp, breq->lastino + 1, xfs_bulkstat_iwalk,
			breq->icount, &bc);
	...
	inumbers_fmt_pf		formatter)
{
	...
	error = xfs_inobt_walk(breq->mp, breq->lastino + 1, xfs_inumbers_walk,
			breq->icount, &ic);

...
struct xfs_getparents_ctx {
	struct xfs_attr_list_context	context;
	struct xfs_getparents_by_handle	request;

	/* File to target */
	struct xfs_inode		*ip;

	/* Pointer to where we write the records */
	void __user			*urecords;
	...
	int				valuelen)
{
	...
	struct xfs_getparents		*gp = &gpx->request.gph_request;
	...
	struct xfs_getparents_ctx	*gpx)
{
	...
	if (gp->gp_bufsize > XFS_XATTR_LIST_MAX)
		return -ENOMEM;
	if (gp->gp_bufsize < sizeof(struct xfs_getparents))
		return -EINVAL;

	if (gp->gp_flags & ~XFS_GETPARENTS_FLAGS_ALL)
		return -EINVAL;
	if (gp->__pad)
		return -EINVAL;

	bufsize = round_down(gp->gp_bufsize - sizeof(struct xfs_getparents),
			     sizeof(uint32_t));
	...
	gp->gp_count = 0;
	gp->gp_flags &= ~XFS_GETPARENTS_OFLAGS_ALL;

	trace_xfs_getparents(ip, gp, &gpx->context.cursor);
	...
	if (ip->i_ino == mp->m_sb.sb_rootino)
		gp->gp_flags |= XFS_GETPARENTS_OFLAG_ROOT;
	 ...
	 */
	if (gpx->context.seen_enough == 0)
		gp->gp_flags |= XFS_GETPARENTS_OFLAG_DONE;
	...
	if (copy_to_user(gpx->urecords, gpx->krecords, gpx->context.firstu))
		error = -EFAULT;
	...
	struct xfs_getparents __user	*ureq)
{
	...
	struct xfs_getparents		*kreq = &gpx.request.gph_request;
	...
	gpx.urecords = &ureq->gp_records[0];
	...
	struct xfs_getparents_by_handle __user	*ureq)
{
	...
	struct xfs_mount		*mp = ip->i_mount;
	struct xfs_getparents_by_handle	*kreq = &gpx.request;
	struct dentry			*dentry;
	int				error;

	BUILD_BUG_ON(XFS_IOC_GETPARENTS_BY_HANDLE == XFS_IOC_GETPARENTS);
	...
	dentry = xfs_khandle_to_dentry(filp->f_path.mnt, &kreq->gph_handle);
	if (IS_ERR(dentry))
		return PTR_ERR(dentry);

	gpx.ip = XFS_I(dentry->d_inode);
	gpx.urecords = &ureq->gph_request.gp_records[0];
	error = xfs_getparents(&gpx);
	dput(dentry);
	if (error)
		return error;

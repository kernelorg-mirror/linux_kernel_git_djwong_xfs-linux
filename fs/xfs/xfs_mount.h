...
#if IS_ENABLED(CONFIG_XFS_ONLINE_SCRUB)
void xfs_fs_bump_intents(struct xfs_mount *mp, bool isrt, xfs_fsblock_t fsbno);
void xfs_fs_drop_intents(struct xfs_mount *mp, bool isrt, xfs_fsblock_t fsbno);
...
#else
# define xfs_fs_bump_intents(mp, isrt, fsbno)	((void)0)
# define xfs_fs_drop_intents(mp, isrt, fsbno)	((void)0)
# define xfs_rt_wait_intents(mp)		(-ENOSYS)

	...
	struct xfs_buftarg	**btpp)
{
	...
	xmb->target.bt_bdev = NULL; /* in-memory buftargs have no bdev */
	xmb->target.bt_file = file;
	...
	error = xfs_init_buftarg(&xmb->target, XMBUF_BLOCKSIZE, descr);
	if (error)
		goto out_bcache;
...
{
	struct xmbuf		*xmb = container_of(btp, struct xmbuf, target);

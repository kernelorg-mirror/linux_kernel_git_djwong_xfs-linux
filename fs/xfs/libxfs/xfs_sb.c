...
"Realtime extent size must not be zero.");
	...
	rgblocks = xfs_rtx_to_rtb(mp, (xfs_rtxnum_t)sbp->sb_rgextents);
	if (rgblocks > XFS_MAX_RGBLOCKS) {
		xfs_warn(mp,
"Realtime group size (%llu) must be less than %u rtblocks.",
			 rgblocks, XFS_MAX_RGBLOCKS);
		return -EINVAL;
	}

	if (sbp->sb_rgextents < XFS_MIN_RGEXTENTS) {
		xfs_warn(mp,
"Realtime group size (%llu) must be at least %u rt extents.",
			 rgblocks, XFS_MIN_RGEXTENTS);
		return -EINVAL;
	}
...
"Realtime groups (%u) must be less than %u.",
	...
	groups = howmany_64(sbp->sb_rblocks, rgblocks);
	...
	struct xfs_sb		*sbp)
{
	...
	mp->m_rgblocks = xfs_rtb_to_rtx(mp, sbp->sb_rgextents);

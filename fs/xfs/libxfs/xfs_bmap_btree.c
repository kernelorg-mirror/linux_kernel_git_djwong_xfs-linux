...
__xfs_bmbt_iroot_realloc(
	...
	int			rec_diff)
{
	return xfs_bmbt_iroot_realloc(cur->bc_ino.ip, rec_diff,
			cur->bc_ino.whichfork);
}
	...
	int			whichfork)
{
	...
	XFS_IFORK_FMT_SET(cur->bc_ino.ip, whichfork, ifake->if_format);
	XFS_IFORK_NEXT_SET(cur->bc_ino.ip, whichfork, ifake->if_extents);

...
kmem_zone_t		*xfs_extent_free_item_zone;
 ...
 * xfs_free_extent_later() with some special handling particular to AGFL
 * blocks.
 *
	...
	unsigned int			flags)
{
	...
	trace_xfs_extent_free_defer(mp, XFS_FREE_EXTENT_REGULAR, xefi);
	...
	xfs_extlen_t		*len)
{
	...
	cur = xfs_allocbt_init_cursor(mp, NULL, agf_bp, pag, XFS_BTNUM_BNO);

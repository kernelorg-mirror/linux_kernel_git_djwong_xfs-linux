	...
	struct xfs_swapext_req	*req)
{
	 ...
	 */
	bmbt_overhead = XFS_NEXTENTADD_SPACE_RES(mp, req->nr_exchanges,
						 req->whichfork);
	req->ip1_bcount += bmbt_overhead;
	req->ip2_bcount += bmbt_overhead;
	req->resblks += 2 * bmbt_overhead;
	...
	trace_xfs_swapext_estimate(req);

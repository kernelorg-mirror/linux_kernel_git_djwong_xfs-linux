	...
	struct xfs_swapext_intent	*sxi)
{
	...
	int				nimaps;
	int				bmap_flags = xfs_bmapi_aflag(
							sxi->si_whichfork);
	int				error;
	...
	while (sxi->si_blockcount > 0) {
		...
		xfs_bmap_unmap_extent(tp, sxi->si_ip1, sxi->si_whichfork,
				&irec1);
		xfs_bmap_unmap_extent(tp, sxi->si_ip2, sxi->si_whichfork,
				&irec2);
		...
		xfs_bmap_map_extent(tp, sxi->si_ip1, sxi->si_whichfork,
				&irec2);

	...
	struct xfs_btree_cur		**pcur)
{
	 ...
	 */
	rcur = *pcur;
	if (rcur != NULL && xfs_rmap_is_wrong_cursor(rcur, pag, rtg)) {
	...
	if (rcur == NULL) {
		if (ri->ri_realtime) {
			xfs_rtgroup_lock(tp, rtg, XFS_RTGLOCK_RMAP);
			rcur = xfs_rtrmapbt_init_cursor(mp, tp, rtg,
					rtg->rtg_rmapip);
			rcur->bc_ino.flags = 0;
		} else {
			/*
	...
	*pcur = rcur;
	...
	xfs_rmap_update_hook(tp, pag, rtg, ri->ri_type, bno,

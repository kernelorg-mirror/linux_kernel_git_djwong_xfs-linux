...
xfs_rtrmapbt_absolute_maxlevels(void)
{
	...
	return xfs_btree_compute_maxlevels_size(max_dblocks, minrecs[1]) + 1;
}

int __init
xfs_rtrmapbt_create_cursor_cache(void)
{
	return xfs_btree_create_cursor_cache(XFS_BTNUM_RTRMAP,
			"xfs_rtrmapbt_cur", xfs_rtrmapbt_absolute_maxlevels());
}

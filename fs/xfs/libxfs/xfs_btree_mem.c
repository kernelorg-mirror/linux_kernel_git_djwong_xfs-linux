...
xfbtree_set_root(
	...
	int				inc)
{
	ASSERT(cur->bc_ops->geom_flags & XFS_BTGEO_IN_MEM);
	...
	unsigned int		max_recs)
{
	...
	if (bno != NULLFSBLOCK &&
	    !xfbuf_verify_daddr(btp, xfs_daddr_to_xfbno(bno)))
		return __this_address;
...
{
	return XFBUF_BLOCKSIZE - XFS_BTREE_LBLOCK_CRC_LEN;
}
...
/*
 * Create an in-memory btree root that can be used with the given xfbuf.
	...
	const struct xfs_btree_ops	*ops)
{
	...
	if (!(ops->geom_flags & XFS_BTGEO_LONG_PTRS)) {
		ASSERT(ops->geom_flags & XFS_BTGEO_LONG_PTRS);
		return -EINVAL;
	}

	memset(xfbt, 0, sizeof(*xfbt));
	xfbt->target = xfb->target;
	...
	struct xfs_trans	*tp)
{
	...
	list_for_each_entry_safe(lip, n, &tp->t_items, li_trans) {
		...
		xfbuf_trans_bdetach(tp, bp, corrupt ? NULL : &buffer_list);
		xfs_buf_relse(bp);
	}
	...
	struct xfs_trans	*tp)
{
	...
	list_for_each_entry_safe(lip, n, &tp->t_items, li_trans) {
		...
		xfbuf_trans_bdetach(tp, bp, NULL);
		xfs_buf_relse(bp);
	}

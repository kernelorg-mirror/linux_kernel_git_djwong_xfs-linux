...
xfs_exchmaps_should_advertise(
	struct xfs_mount	*mp)
{
	/* If the fs has the log incompat feature set already, we're good. */
...
xfs_exchmaps_do_postop_work(
	...
	struct xfs_exchmaps_intent	*xmi)
{
	if (xmi->xmi_flags & XFS_EXCHMAPS_INO2_SHORTFORM) {
		int			error = 0;
		...
		xmi->xmi_flags &= ~XFS_EXCHMAPS_INO2_SHORTFORM;
		if (error)
			return error;
	}

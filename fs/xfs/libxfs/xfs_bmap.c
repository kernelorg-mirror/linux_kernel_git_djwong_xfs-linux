...
bool gud;
	...
	gud = xfs_iunlock_check_datafork(ip);
	if (!gud) {
		int newleafnr;

		if (gud)
			xfs_inode_dump_datafork(ip);
		xfs_err(ip->i_mount, "ino 0x%llx fork %d oldoff 0x%llx oldlen 0x%llx oldprealloc 0x%llx isize 0x%llx",
				ip->i_ino, whichfork, oldoff, oldlen, oldprealloc, i_size_read(VFS_I(ip)));

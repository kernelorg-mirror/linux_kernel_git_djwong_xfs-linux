...
#define	PREV		r[2]
	...
	if (da_new != da_old)
		xfs_mod_delayed(mp, (int64_t)da_new - da_old);
	...
	xfs_bmbt_irec_t		*new)	/* new data to add to file extents */
{
	...
	if (oldlen != newlen) {
		 ...
		 */
		xfs_mod_delayed(ip->i_mount, (int64_t)newlen - oldlen);
	}
}
	...
	int			eof)
{
	...
	xfs_mod_delayed(ip->i_mount, alen + indlen);

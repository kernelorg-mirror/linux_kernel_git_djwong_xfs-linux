	...
	int				rec_diff)
{
	...
	xfs_iroot_realloc(cur->bc_ino.ip, cur->bc_ino.whichfork,
			cur->bc_nlevels - 1, cur->bc_ops->iroot_ops, rec_diff);
}
	...
	int			*stat)		/* return status - 0 fail */
{
	...
	if (level > 0) {
		...
		/*
		 * Trickery here: The amount of memory we need per record for
		 * the root changes when we convert a leaf root to a node.
		 * Therefore, set the incore btree root record count to zero,
		 * increment the level, allocate a fresh incore btree block for
		 * the node data, and copy the new block's key/ptr pair into
		 * the root.
		 */
		xfs_btree_iroot_realloc(cur, -xfs_btree_get_numrecs(cblock));
		block = xfs_btree_get_iroot(cur);
		be16_add_cpu(&block->bb_level, 1);
	...
	struct xfs_btree_cur	*cur)
{
	...
	if (be16_to_cpu(cblock->bb_level) > 0) {
		...
		for (i = 0; i < numrecs; i++) {
			error = xfs_btree_debug_check_ptr(cur, cpp, i,
					level - 1);
			if (error)
				return error;
		}
		...
		/*
		 * Trickery here: The amount of memory we need per record for
		 * the root changes when we convert a node root to a leaf.
		 * Therefore, set the incore btree root record count to zero,
		 * decrement the level, allocate a fresh incore btree root for
		 * the leaf data, and copy the records out of the doomed block.
		 */
		xfs_btree_iroot_realloc(cur,
				-cur->bc_ops->get_maxrecs(cur, level));
		block = xfs_btree_get_iroot(cur);
		be16_add_cpu(&block->bb_level, -1);

		xfs_btree_iroot_realloc(cur, numrecs);
		block = xfs_btree_get_iroot(cur);
...
/* Allocate a block for an inode-rooted btree. */
int
xbtree_alloc_ino_block(
	struct xfs_btree_cur	*cur,
...
/* Free a block from an inode-rooted btree. */
int
xbtree_free_ino_block(
	struct xfs_btree_cur	*cur,
	struct xfs_buf		*bp)
{
	struct xfs_mount	*mp = cur->bc_mp;

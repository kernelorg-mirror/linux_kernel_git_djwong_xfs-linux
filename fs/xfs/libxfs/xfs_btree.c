	...
	__be64			dsibling)
{
	...
	if (level >= 0) {
		if (!xfs_btree_check_lptr(cur, sibling, level + 1))
			return __this_address;
	} else if (cur && (cur->bc_ops->geom_flags & XFS_BTGEO_IN_MEM)) {
	...
	struct xfs_buf		*bp)
{
	...
	if ((cur->bc_ops->geom_flags & XFS_BTGEO_IN_MEM) && bp)
...
{
	if (cur->bc_ops->geom_flags & XFS_BTGEO_IN_MEM) {
		xfbno_t bno		= be64_to_cpu((&ptr->l)[index]);

		ASSERT(cur->bc_ops->geom_flags & XFS_BTGEO_LONG_PTRS);

		if (xfbtree_verify_bno(cur->bc_mem.xfbtree, bno))
			return 0;
	...
	void				*data)
{
	 ...
	 */
	if (cur->bc_ops->geom_flags & XFS_BTGEO_IN_MEM) {
		if (be64_to_cpu(rptr.l) == xfs_daddr_to_xfbno(
							xfs_buf_daddr(bp)))
			return -EFSCORRUPTED;
	} else if (cur->bc_ops->geom_flags & XFS_BTGEO_LONG_PTRS) {

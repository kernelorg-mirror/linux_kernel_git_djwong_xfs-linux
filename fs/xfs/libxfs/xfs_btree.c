...
/*
 * Initialize a new fakeroot structure so that we can build a new AG-rooted
 * btree.  @agbno is the address of the new root block, which must be an empty
 * btree block.
 */
void
xbtree_afakeroot_init(
	struct xfs_mount	*mp,
 ...
 * @ops for overriding by duplicating them into @new_ops.  The caller must
 * replace pointers in @new_ops as necessary once this call completes.
 */
void
...
/* Adjust the block count of a fakeroot. */
void
xbtree_afakeroot_adjust_blocks(
	struct xfs_btree_cur	*cur,
	int			nr)
{
	struct xbtree_afakeroot	*afake = cur->bc_private.a.afake;
...
void
xbtree_afakeroot_init_ptr_from_cur(
	struct xfs_btree_cur	*cur,
	union xfs_btree_ptr	*ptr)
{
	struct xbtree_afakeroot	*afake = cur->bc_private.a.afake;

	...
	struct xfs_dir_update	*du)
{
	...
	struct xfs_parent_defer	*parent = du->parent;
	int			error;
...
xfs_dir_exchange_children(
	struct xfs_trans	*tp,
	struct xfs_dir_update	*i1,
	struct xfs_dir_update	*i2,
	unsigned int		spaceres)
{
	struct xfs_inode	*dp1 = i1->dp;
	const struct xfs_name	*name1 = i1->name;
	struct xfs_inode	*ip1 = i1->ip;
	struct xfs_parent_defer	*ip1_pptr = i1->parent;
	struct xfs_inode	*dp2 = i2->dp;
	const struct xfs_name	*name2 = i2->name;
	struct xfs_inode	*ip2 = i2->ip;
	struct xfs_parent_defer	*ip2_pptr = i2->parent;
	int			ip1_flags = 0;
	int			ip2_flags = 0;
	int			dp2_flags = 0;
	int			error;
...
xfs_dir_rename_children(
	struct xfs_trans	*tp,
	struct xfs_dir_update	*src,
	struct xfs_dir_update	*tgt,
	unsigned int		spaceres,
	struct xfs_inode	*wip,
	struct xfs_parent_defer	*wip_pptr)
{
	struct xfs_mount	*mp = tp->t_mountp;
	struct xfs_inode	*src_dp = src->dp;
	const struct xfs_name	*src_name = src->name;
	struct xfs_inode	*src_ip = src->ip;
	struct xfs_parent_defer	*src_ip_pptr = src->parent;
	struct xfs_inode	*target_dp = tgt->dp;
	const struct xfs_name	*target_name = tgt->name;
	struct xfs_inode	*target_ip = tgt->ip;
	struct xfs_parent_defer	*target_ip_pptr = tgt->parent;
	...
	if (target_ip_pptr) {
		error = xfs_parent_remove(tp, target_ip_pptr, target_dp,

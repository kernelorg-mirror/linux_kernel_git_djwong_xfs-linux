	...
	struct xfs_dir_update	*du)
{
	 ...
	 */
	if (du->ppargs)
		xfs_parent_addname(tp, du->ppargs, dp, name, ip);
	return 0;
}
	...
	struct xfs_dir_update	*du)
{
	...
	if (du->ppargs)
		return xfs_parent_removename(tp, du->ppargs, dp, name, ip);
	return 0;
}
	...
	unsigned int		spaceres)
{
	...
	if (du2->ppargs)
		return xfs_parent_replacename(tp, du2->ppargs, dp2, name2, dp1,
				name1, ip2);
	return 0;
}
	...
	struct xfs_dir_update	*du_wip)
{
	...
	if (du_wip->ppargs)
		xfs_parent_addname(tp, du_wip->ppargs, src_dp, src_name,
				du_wip->ip);
	...
	if (du_tgt->ppargs)
		return xfs_parent_removename(tp, du_tgt->ppargs, target_dp,

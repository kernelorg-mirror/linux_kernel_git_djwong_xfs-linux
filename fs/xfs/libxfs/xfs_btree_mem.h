...
#define XFBNO_BLOCKSIZE			(XFBUF_BLOCKSIZE)
#define XFBNO_BBSHIFT			(XFBUF_BLOCKSHIFT - BBSHIFT)
...
static inline bool xfbtree_verify_bno(struct xfbtree *xfb, xfbno_t bno)
{
	return xfbuf_verify_daddr(xfb->target, xfbno_to_daddr(bno));
}
...
int xfbtree_init(struct xfs_mount *mp, struct xfbtree *xfbt,
		const struct xfbuf *xfb, const struct xfs_btree_ops *ops);

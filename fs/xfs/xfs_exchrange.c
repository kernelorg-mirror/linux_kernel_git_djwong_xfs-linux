	...
	unsigned int		allocunit)
{
	...
	uint64_t		allocmask = allocunit - 1;
	int64_t			test_len;
	uint64_t		blen;
	 ...
	 * unit.
	 */
	if (!IS_ALIGNED(fxr->file1_offset, allocunit) ||
	    !IS_ALIGNED(fxr->file2_offset, allocunit))
		return -EINVAL;
	...
	if (fxr->file1_offset + fxr->length == size1)
		blen = ALIGN(size1, allocunit) - fxr->file1_offset;
	else if (fxr->file2_offset + fxr->length == size2)
		blen = ALIGN(size2, allocunit) - fxr->file2_offset;
	else if (!IS_ALIGNED(fxr->length, allocunit))
		return -EINVAL;
	else
		blen = fxr->length;
	...
	unsigned int		allocunit)
{
	...
	error = xfs_generic_exchrange_checks(fxr, allocunit);
	...
	struct xfs_exchrange	*fxr)
{
	...
	if (fxr->flags & ~(XFS_EXCHRANGE_ALL_FLAGS | XFS_EXCHRANGE_CHECK_FRESH2))
		return -EINVAL;
	...
	if (!(fxr->file1->f_mode & FMODE_NOCMTIME) && !IS_NOCMTIME(inode1))
		fxr->flags |= XFS_EXCHRANGE_UPD_CMTIME1;
	if (!(fxr->file2->f_mode & FMODE_NOCMTIME) && !IS_NOCMTIME(inode2))
		fxr->flags |= XFS_EXCHRANGE_UPD_CMTIME2;

	file_start_write(fxr->file2);
	ret = xfs_file_exchrange(fxr);
	...
	struct xfs_mount	*mp)
{
	 ...
	 * if the filesystem featureset already protects the exchmaps log items.
	 */
	if (xfs_exchmaps_can_use_without_log_assistance(mp))
		return 0;

	/*
	 * If log-assisted file mapping exchanges are already enabled, we don't
	 ...
	 * and large extent counts to avoid having to deal with really old
	 * kernels.
	 */
	...
	struct xfs_inode	*ip2)
{
	 ...
	 * volume if the extent size is set.
	 */
	if (!is_power_of_2(xfs_inode_alloc_unitsize(ip2)))
		return -EOPNOTSUPP;

	error = xfs_generic_exchrange_prep(fxr, xfs_inode_alloc_unitsize(ip2));
	if (error || fxr->length == 0)
		return error;

	if (fxr->flags & XFS_EXCHRANGE_CHECK_FRESH2) {
...
retry:
	...
	if (fxr->flags & XFS_EXCHRANGE_UPD_CMTIME1)
		xfs_trans_ichgtime(tp, ip1, XFS_ICHGTIME_MOD | XFS_ICHGTIME_CHG);
	if (fxr->flags & XFS_EXCHRANGE_UPD_CMTIME2)
	...
	struct xfs_exchrange	*fxr)
{
	...
	error = xfs_exchrange_grab_log_assist(mp);
	if (error)
		goto out_unlock;

...
void xfs_healthmon_unmount(struct xfs_mount *mp, struct xfs_healthmon *hm);
...
void xfs_healthmon_report_metadata(struct xfs_healthmon *hmon,
		const struct xfs_health_update_params *p);

void xfs_healthmon_report_shutdown(struct xfs_healthmon *hmon, uint32_t flags);

struct xfs_media_error_params;
void xfs_healthmon_report_media(struct xfs_healthmon *hmon,
		const struct xfs_media_error_params *p);

void xfs_healthmon_report_file_ioerror(struct xfs_healthmon *hmon,

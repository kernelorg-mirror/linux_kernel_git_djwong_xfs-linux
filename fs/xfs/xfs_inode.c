...
{
	return xfs_hooks_add(&mp->m_dir_update_hooks, &hook->delta_hook);
}
...
xfs_dir_hook_del(
	...
	struct xfs_dir_hook	*hook)
{
	xfs_hooks_del(&mp->m_dir_update_hooks, &hook->delta_hook);
}
	...
	struct xfs_inode	**ipp)
{
	...
	error = xfs_dir_create_child(tp, resblks, dp, name, ip);
	if (error)
		goto out_trans_cancel;
	...
	struct xfs_name		*target_name)
{
	...
	error = xfs_dir_add_child(tp, resblks, tdp, target_name, sip);
	if (error)
		goto error_return;
...
retry:
	...
	error = xfs_dir_rename_children(tp, src_dp, src_name, src_ip,
			target_dp, target_name, target_ip, spaceres, wip);
	if (error)
		goto out_trans_cancel;

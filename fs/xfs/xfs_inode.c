	...
	int			spaceres)
{
	...
	error = xfs_dir_exchange_children(tp, &i1, &i2, spaceres);
	if (error)
	...
	unsigned int			flags)
{
	...
	error = xfs_parent_start(mp, &src.parent);
	if (error)
	...
	if (target_ip) {
		error = xfs_parent_start_locked(mp, &tgt.parent);
		if (error)
			goto out_wip_pptr;
	}
...
retry:
	...
	if (flags & RENAME_EXCHANGE) {
		error = xfs_cross_rename(tp, src_dp, src_name, src_ip,
				src.parent, target_dp, target_name, target_ip,
				tgt.parent, spaceres);
		nospace_error = 0;
		goto out_unlock;
	}
	...
	error = xfs_dir_rename_children(tp, &src, &tgt, spaceres, wip,
			wip_pptr);
	if (error)
		goto out_trans_cancel;
...
out_unlock:
	xfs_iunlock_rename(inodes, num_inodes);
out_tgt_ip_pptr:
	xfs_parent_finish(mp, tgt.parent);
out_wip_pptr:
	xfs_parent_finish(mp, wip_pptr);
out_src_ip_pptr:
	xfs_parent_finish(mp, src.parent);
out_release_wip:
	if (wip)
		xfs_irele(wip);
	if (error == -ENOSPC && nospace_error)

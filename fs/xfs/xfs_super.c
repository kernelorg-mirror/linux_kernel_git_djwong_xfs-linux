	...
	struct super_block	*sb)
{
	...
	fs_send_uevent(mp->m_super, &mp->m_kobj.kobject, FSU_UNMOUNT);
...
{
	/* We already know about non-inode and metadata errors */
	if (!event->inode || event->type == FSERR_METADATA)
		return;

	with_xfs_healthmon(XFS_M(event->sb), hmon)
		xfs_healthmon_report_file_ioerror(hmon, event);
}

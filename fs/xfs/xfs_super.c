	...
	struct xfs_mount	*mp)
{
	...
	error = xfs_format_uevent_strings(mp, buf, sizeof(buf), &env[0]);
	if (!error)
...
{
	/* We already know about non-inode filesystem errors */
	if (!event->inode)
		return;

	with_xfs_healthmon(XFS_M(event->sb), hmon) {
		xfs_healthmon_file_ioerror_hook(hmon, event);
	}
}

	...
	struct inode		*inode)
{
	...
	/*
	 * If the inode doesn't need inactivation, that means we're going
	 * directly into reclaim and must drop the dquots.  It also means that
	 * there shouldn't be any delalloc reservations or speculative CoW
	 * preallocations remaining.
	 */
	if (!need_inactive) {
		xfs_qm_dqdetach(ip);
	...
	struct super_block	*sb)
{
	...
	/* XXX can we move the FREEZE_PAGEFAULTS inodegc_stop here? */

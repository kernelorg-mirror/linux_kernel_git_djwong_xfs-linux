	...
	struct xfs_mount	*mp)
{
	...
	mp->m_unwritten_workqueue = alloc_workqueue("xfs-conv/%s",
			WQ_MEM_RECLAIM | WQ_FREEZABLE | WQ_SYSFS,
			0, mp->m_super->s_id);
 ...
 *
 * Don't let userspace freeze while scrub has the filesystem frozen.  Take our
 * own private reference to the vfs superblock so that we don't lose the xfs
 * superblock while frozen or trying to freeze.
 */
STATIC int
xfs_fs_freeze_super(
	struct super_block	*sb)
...
/*
 * Thaw the superblock.  We don't let userspace undo a fs repair freeze.
 *
 * Freeze takes an s_active reference to the filesystem and fs thaw drops it.
 * If a filesystem on a frozen (dm) block device is unmounted before the block
 * device is thawed, the freezer state could contain the one remaining s_active
 * reference to the vfs superblock.  Since thaw_super drops that reference
 * (which frees the xfs_mount) and we still have to unlock the scrub freeze
 * mutex, we must take our own private s_active reference to avoid a
 * use-after-free.
 */
STATIC int
xfs_fs_thaw_super(

	...
	*stat = false;
	...
	if (info->rtg)
		cur = xfs_rtrefcountbt_init_cursor(mp, tp, info->rtg,
				info->rtg->rtg_inodes[XFS_RTGI_REFCOUNT]);
	else if (info->pag)
	...
	struct xfs_fsmap_irec		*frec)
{
	...
	if (info->last && info->end_daddr != XFS_BUF_DADDR_NULL)
		...
		frec->start_daddr = xfs_agbno_to_daddr(pag, startblock);

	frec->len_daddr = XFS_FSB_TO_BB(mp, blockcount);
	...
	void				*priv)
{
	...
	return xfs_getfsmap_perag_helper(info, cur->bc_tp, cur->bc_ag.pag,
	...
	void				*priv)
{
	...
	for_each_perag_range(mp, ag, end_ag, pag) {
		...
		trace_xfs_fsmap_low_group_key(mp, info->dev, pag->pag_agno,
				&info->low);
		trace_xfs_fsmap_high_group_key(mp, info->dev, pag->pag_agno,
				&info->high);
	...
	struct xfs_fsmap_irec		*frec)
{
	...
	if (info->last && info->end_daddr != XFS_BUF_DADDR_NULL)
		...
		frec->start_daddr = xfs_rtb_to_daddr(mp,
					xfs_rgbno_to_rtb(rtg, startblock));
	...
	void				*priv)
{
	...
	return xfs_getfsmap_rtgroup_helper(info, cur->bc_tp, cur->bc_ino.rtg,
			rec->rm_startblock, rec->rm_blockcount, &frec);
}
	...
	struct xfs_getfsmap_info	*info)
{
	...
	for_each_rtgroup_range(mp, start_rg, end_rg, rtg) {
		...
		trace_xfs_fsmap_low_group_key(mp, info->dev, rtg->rtg_rgno,
				&info->low);
		trace_xfs_fsmap_high_group_key(mp, info->dev, rtg->rtg_rgno,
				&info->high);

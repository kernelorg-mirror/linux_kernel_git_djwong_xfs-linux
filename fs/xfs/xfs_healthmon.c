 ...
 * free it after an RCU grace period to eliminate possibility of races with
 * xfs_healthmon_get.
 */
void
xfs_healthmon_put(
 ...
 * from this healthmon instance.  Only call this if you hold s_umount.
 */
void
	...
	const struct xfs_health_update_params *hup)
{
	...
	unsigned int			mask =
			health_update_event_mask(hm, hup);

	if (!mask)
		return;

	trace_xfs_healthmon_report_metadata(hm, hup);
	...
	const struct xfs_media_error_params *p)
{
	...
	trace_xfs_healthmon_report_media(hm, p);

	...
	struct xfs_rmap_intent		*rmap)
{
	...
	if (rmap->ri_bmap.br_state == XFS_EXT_UNWRITTEN)
	...
	struct list_head		*capture_list)
{
	...
	for (i = 0; i < ruip->rui_format.rui_nextents; i++) {
		...
		me = &ruip->rui_format.rui_extents[i];
		fake.ri_whichfork = (me->me_flags & XFS_RMAP_EXTENT_ATTR_FORK) ?
				XFS_ATTR_FORK : XFS_DATA_FORK;
		fake.ri_realtime = me->me_flags & XFS_RMAP_EXTENT_REALTIME;
		switch (me->me_flags & XFS_RMAP_EXTENT_TYPE_MASK) {
		case XFS_RMAP_EXTENT_MAP:
		...
		fake.ri_owner = me->me_owner;
		fake.ri_bmap.br_startblock = me->me_startblock;
		fake.ri_bmap.br_startoff = me->me_startoff;
		fake.ri_bmap.br_blockcount = me->me_len;
		fake.ri_bmap.br_state = (me->me_flags & XFS_RMAP_EXTENT_UNWRITTEN) ?

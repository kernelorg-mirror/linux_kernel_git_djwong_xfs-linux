...
/*
 * Estimate the worst-case number of bmbt blocks needed to map a number of
 * blocks.
 */
static xfs_filblks_t
xfs_bmap_estimate_bmbt_blocks(
	struct xfs_mount	*mp,
	int			whichfork,
	xfs_filblks_t		nr_blocks)
{
	return howmany_64(nr_blocks, XFS_MAX_CONTIG_EXTENTS_PER_BLOCK(mp)) *
				XFS_EXTENTADD_SPACE_RES(mp, whichfork);
}
	...
	xfs_filblks_t		blockcount)
{
	...
	if (error)
		return error;
	ip1_mapped += xfs_bmap_estimate_bmbt_blocks(mp, whichfork, ip1_mapped);
	ip2_mapped += xfs_bmap_estimate_bmbt_blocks(mp, whichfork, ip2_mapped);
	...
	const struct file_swap_range *fsr)
{
	...
	if (xfs_sb_version_hasatomicswap(&mp->m_sb)) {
		...
		error = xfs_swapext_deferred_bmap(&tp, ip1, ip2, XFS_DATA_FORK,
		   ...
		   (fsr->flags & FILE_SWAP_RANGE_DEFRAGMENT) &&

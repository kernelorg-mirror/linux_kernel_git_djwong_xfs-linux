	...
	struct xfs_dquot	*dq)
{
	...
	if (defq->bsoftlimit && !dq->q_blk_softlimit) {
		dq->q_blk_softlimit = defq->bsoftlimit;
		prealloc = 1;
	}
	if (defq->bhardlimit && !dq->q_blk_hardlimit) {
		dq->q_blk_hardlimit = defq->bhardlimit;
		prealloc = 1;
	}
	if (defq->isoftlimit && !dq->q_ino_softlimit)
		dq->q_ino_softlimit = defq->isoftlimit;
	if (defq->ihardlimit && !dq->q_ino_hardlimit)
		dq->q_ino_hardlimit = defq->ihardlimit;
	if (defq->rtbsoftlimit && !dq->q_rtb_softlimit)
		dq->q_rtb_softlimit = defq->rtbsoftlimit;
	if (defq->rtbhardlimit && !dq->q_rtb_hardlimit)
		dq->q_rtb_hardlimit = defq->rtbhardlimit;
...
{
	if (over && *timer == 0)
		xfs_dquot_set_timer(dqp, timer,
				    ktime_get_real_seconds() + grace);
...
#ifdef DEBUG
	if (dq->q_blk_hardlimit)
		ASSERT(dq->q_blk_softlimit <= dq->q_blk_hardlimit);
	if (dq->q_ino_hardlimit)
		ASSERT(dq->q_ino_softlimit <= dq->q_ino_hardlimit);
	if (dq->q_rtb_hardlimit)
...
xfs_dquot_set_prealloc_limits(struct xfs_dquot *dqp)
{
	...
	dqp->q_prealloc_hi_wmark = dqp->q_blk_hardlimit;
	dqp->q_prealloc_lo_wmark = dqp->q_blk_softlimit;
	...
	struct xfs_buf		*bp)
{
	...
	dqp->q_blk_hardlimit = be64_to_cpu(ddqp->d_blk_hardlimit);
	dqp->q_blk_softlimit = be64_to_cpu(ddqp->d_blk_softlimit);
	dqp->q_ino_hardlimit = be64_to_cpu(ddqp->d_ino_hardlimit);
	dqp->q_ino_softlimit = be64_to_cpu(ddqp->d_ino_softlimit);
	dqp->q_rtb_hardlimit = be64_to_cpu(ddqp->d_rtb_hardlimit);
	dqp->q_rtb_softlimit = be64_to_cpu(ddqp->d_rtb_softlimit);

	dqp->q_bcount = be64_to_cpu(ddqp->d_bcount);
	...
	xfs_dquot_from_disk_timestamp(ddqp, &dqp->q_btimer, ddqp->d_btimer);
	xfs_dquot_from_disk_timestamp(ddqp, &dqp->q_itimer, ddqp->d_itimer);
	xfs_dquot_from_disk_timestamp(ddqp, &dqp->q_rtbtimer, ddqp->d_rtbtimer);
	...
	struct xfs_dquot	*dqp)
{
	...
	ddqp->d_blk_hardlimit = cpu_to_be64(dqp->q_blk_hardlimit);
	ddqp->d_blk_softlimit = cpu_to_be64(dqp->q_blk_softlimit);
	ddqp->d_ino_hardlimit = cpu_to_be64(dqp->q_ino_hardlimit);
	ddqp->d_ino_softlimit = cpu_to_be64(dqp->q_ino_softlimit);
	ddqp->d_rtb_hardlimit = cpu_to_be64(dqp->q_rtb_hardlimit);
	ddqp->d_rtb_softlimit = cpu_to_be64(dqp->q_rtb_softlimit);
	...
	xfs_dquot_to_disk_timestamp(dqp, &ddqp->d_btimer, dqp->q_btimer);
	xfs_dquot_to_disk_timestamp(dqp, &ddqp->d_itimer, dqp->q_itimer);
	xfs_dquot_to_disk_timestamp(dqp, &ddqp->d_rtbtimer, dqp->q_rtbtimer);
}
	...
	struct xfs_dquot	*dqp)
{
	...
	if (dqp->q_blk_softlimit && dqp->q_bcount > dqp->q_blk_softlimit &&
	    !dqp->q_btimer)
		return __this_address;

	if (dqp->q_ino_softlimit && dqp->q_icount > dqp->q_ino_softlimit &&
	    !dqp->q_itimer)
		return __this_address;

	if (dqp->q_rtb_softlimit && dqp->q_rtbcount > dqp->q_rtb_softlimit &&
	    !dqp->q_rtbtimer)
		return __this_address;

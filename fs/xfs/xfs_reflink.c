...
retry:
	...
	*lockmode = XFS_ILOCK_EXCL;
	...
	/*
	 * If we find that the rt quota reservation has landed us back here to
	 * retry a quota allocation after clearing space, we must treat the
	 * regular block reservation as being in retry mode (i.e. instant death
	 * on EDQUOT/ENOSPC) to avoid infinite retry loops.
	 */
	if (rtquota_retry)
		quota_retry = true;
	...
	error = xfs_trans_reserve_quota_nblks(&tp, ip, rres, 0,
			XFS_QMOPT_RES_RTBLKS, &rtquota_retry);
	if (error) {
		/* This function must return with the ILOCK held. */
		xfs_ilock(ip, *lockmode);
		return error;

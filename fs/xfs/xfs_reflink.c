	...
	bool			*shared)
{
	...
	if (XFS_IS_REALTIME_INODE(ip)) {
		orig_bno = irec->br_startblock;
		error = xfs_reflink_find_rtshared(mp, NULL, orig_bno,
	...
	xfs_fsblock_t		bno)
{
	...
	if (!xfs_has_rmapbt(mp))
		return 0;
	if (XFS_IS_REALTIME_INODE(ip)) {
		if (xfs_imeta_resv_critical(mp->m_rrmapip) ||
		    xfs_imeta_resv_critical(mp->m_rrefcountip))
			return -ENOSPC;
		return 0;
	}
	...
	bool				*has_shared)
{
	...
	*has_shared = false;
	...
	while (found) {
		...
		if (XFS_IS_REALTIME_INODE(ip)) {
			error = xfs_reflink_find_rtshared(mp, tp,

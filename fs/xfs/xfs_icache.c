	...
	struct xfs_eofblocks	*eofb)
{
	...
	error = xfs_inode_walk(mp, XFS_ICWALK_BLOCKGC, eofb);
	if (error)
		return error;
	...
	struct xfs_eofblocks	*eofb)
{
	...
	return xfs_inode_walk(mp, XFS_ICWALK_INODEGC, eofb);
}
	...
	struct work_struct	*work)
{
	...
	error = xfs_inode_walk_ag(pag, XFS_ICWALK_INODEGC, NULL);
...
restart:
	...
	do {
		...
		for (i = 0; i < nr_found; i++) {
			...
			if (done || !xfs_grabbed_for_walk(goal, ip, eofb))
				batch[i] = NULL;

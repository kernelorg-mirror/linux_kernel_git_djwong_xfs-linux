	...
	bool			*inuse)
{
	 ...
	 *
	 * If the incore inode is in the inactivation machinery, either it is
	 * before the point where it would get freed ondisk (in which case
	 * i_mode is still nonzero), or it has already been freed, in which
	 * case i_mode is zero.  We don't take the ILOCK here, but difree
	 * and dialloc require the AGI, which we do hold.
	 *

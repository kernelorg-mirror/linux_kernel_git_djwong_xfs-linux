 ...
 *
 * The metafile type must match the inode exactly.  Caller must supply a
 * transaction (even if empty) to avoid livelocking if the inobt has certain
 * types of cycles.  Untrusted lookups are used here because bad space metadata
 * can cause disproportionately more problems and we only do this once per file
 * per mount.
 */
int

...
xfs_verify_iosize(
	...
	uint64_t			bbcount)
{
	unsigned int			iosize = SZ_1M;
	const unsigned int		lbasize =
...
{
	struct bio		*bio;
	const unsigned int	biosize =
		min(bbcount, folio_size(folio) >> SECTOR_SHIFT) << SECTOR_SHIFT;
	...
	bio_add_folio_nofail(bio, folio, biosize, 0);

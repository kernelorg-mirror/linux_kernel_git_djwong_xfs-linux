...
/*
 * If the post-grow filesystem will have an rt superblock; we're initializing
 * the first rtgroup; and the filesystem didn't have a realtime section, write
 * the rtsb now, and attach the rtsb buffer to the real mount.
 */
static int
	...
	xfs_fileoff_t		bmbno)
{
	...
	/* Initialize the realtime super, if necessary. */
	error = xfs_growfs_rt_init_super(&nargs, rtg, &args);
	if (error)
		goto out_cancel;

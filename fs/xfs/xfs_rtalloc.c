	...
	struct xfs_rtgroup	*rtg)
{
	...
	if (ino != NULLFSINO) {
		...
		lockdep_set_class_and_subclass(&ip->i_lock.mr_lock,
	...
	lockdep_set_class_and_subclass(&ip->i_lock.mr_lock, &rtg->lock_class,
			XFS_RTRMAP_SUBCLASS);
	...
	struct xfs_inode	**ipp)
{
	...
	lockdep_set_class_and_subclass(&(*ipp)->i_lock.mr_lock, lockdep_key,
			subclass);
	return 0;
}

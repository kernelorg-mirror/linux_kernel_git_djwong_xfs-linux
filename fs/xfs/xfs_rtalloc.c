	...
	xfs_extlen_t		*nrbmblocks)
{
	...
	*nrbmblocks = nmp->m_sb.sb_rbmblocks;
	...
	if (error)
		goto out_free;
	error = xfs_rtfile_initialize_blocks(rtg, XFS_RTG_SUMMARY, orsumblocks,
...
/*
 * Compute the new number of rt groups.
 *
	...
	struct xfs_growfs_rt	*in)
{
	...
	if (xfs_has_rtgroups(mp)) {
		new_rgcount = howmany_64(in->newblocks,
			mp->m_sb.sb_rgextents * in->extsize);
		if (new_rgcount > XFS_MAX_RGNUMBER)
			return -EINVAL;

		error = xfs_growfs_rt_prep_rtgs(mp, in->newblocks, in->extsize);
		if (error)
			return error;
	}

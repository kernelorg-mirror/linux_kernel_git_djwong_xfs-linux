	...
	struct xfs_mount	*mp)
{
	...
	/*
	 * No need to write the superblock right now; the commit of the primary
	 * super on the data device will write out the buffer.
	 */
	...
	struct xfs_rtgroup	*rtg)
{
	...
	if (ino != NULLFSINO) {
		...
		error = xfs_imeta_iget(tp, ino, XFS_DIR3_FT_REG_FILE, &upd.ip);
		xfs_trans_cancel(tp);
		tp = NULL;
		if (error)
			goto out_path;
		...
		if (XFS_IS_CORRUPT(mp, upd.ip->i_df.if_format !=
							XFS_DINODE_FMT_RMAP)) {
			error = -EFSCORRUPTED;
			xfs_imeta_irele(upd.ip);
			goto out_path;
		}

		lockdep_set_class_and_subclass(&upd.ip->i_lock,
				&rtg->lock_class, XFS_RTRMAP_SUBCLASS);
	...
	struct xfs_trans	*tp)
{
	...
	error = xfs_rt_iget(tp, ino, &rtg->lock_class, XFS_RTREFC_SUBCLASS, &ip);
	if (error)
		goto out_path;

	...
	xfs_growfs_rt_t	*in)		/* growfs rt input struct */
{
	...
	if (xfs_has_rtgroups(mp)) {
		...
		new_rgcount = howmany_64(nrblocks, mp->m_rgblocks);
	...
	struct xfs_mount	*mp)
{
	...
	for_each_rtgroup(mp, rgno, rtg) {
		for (i = 0; i < XFS_RTG_MAX; i++)
			xfs_imeta_resv_free_inode(rtg->rtg_inodes[i]);
	}
}
	...
	struct xfs_mount	*mp)
{
	...
	for_each_rtgroup(mp, rgno, rtg) {
		...
		err2 = xfs_imeta_resv_init_inode(rtg->rtg_inodes[XFS_RTG_RMAP],
				ask);
		if (err2 && !error)
			error = err2;

		ask = xfs_rtrefcountbt_calc_reserves(mp);
		err2 = xfs_imeta_resv_init_inode(

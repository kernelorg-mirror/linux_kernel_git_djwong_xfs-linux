	...
	struct xfs_inode		**ipp)
{
	...
	if (path == &XFS_IMETA_RTRMAPBT) {
		...
		error = xfs_rtrefcountbt_create(&tp, &upd, &ip);
	} else {
		ASSERT(0);
		error = -EIO;
	}
	...
	struct xfs_mount	*mp)
{
	...
	error = xfs_imeta_start_update(mp, &XFS_IMETA_RTRMAPBT, &upd);
	if (error)
		return error;

	error = xfs_imeta_lookup_update(mp, &XFS_IMETA_RTRMAPBT, &upd,
			&ino);
	if (error)
		goto out_end;
	...
	xfs_extlen_t		*len_rtx)
{
	...
	max_rt_extlen = div_u64(XFS_MAX_BMBT_EXTLEN, mp->m_sb.sb_rextsize);

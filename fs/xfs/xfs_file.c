	...
	const struct xfs_map_freesp *mf)
{
	 ...
	 * change the file size on completion without holding any locks we
	 * currently hold. We must do this first because AIO can update both
	 * the on disk and in memory inode sizes, and the operations that
	 * follow require the in-memory size to be fully up-to-date.
	 */
	inode_dio_wait(inode);
	...
	/*
	 * Bail out now if we aren't allowed to make the file size at least the

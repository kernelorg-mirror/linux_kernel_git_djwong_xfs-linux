	...
	const struct xfs_swapext_res	*res)
{
	...
	error = xfs_xchg_range_reserve_quota_blocks(tp, req, res->ip1_bcount,
			res->ip2_bcount, XFS_QMOPT_RES_REGBLKS);
	if (error)
		return error;
	return xfs_xchg_range_reserve_quota_blocks(tp, req, res->ip1_rtbcount,
	...
	unsigned int			private_flags)
{
	 ...
	 */
	error = xfs_xchg_range_reserve_quota(tp, &req, &res);
	if (error)
		goto out_trans_cancel;

	...
	void				*priv)
{
	...
	ASSERT(next_agno >= cur->bc_group->xg_index);
	 ...
	 */
	if (next_agno > cur->bc_group->xg_index)
		return 0;
	...
	void				*priv)
{
	...
	ASSERT(next_rgno >= info->group->xg_index);
	 ...
	 */
	if (next_rgno > info->group->xg_index)
		return 0;
	...
	xfs_rgblock_t			rgbno)
{
	...
	ASSERT(xfs_rtb_to_rgno(mp, start_rtbno) == info->group->xg_index);

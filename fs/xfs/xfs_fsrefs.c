	...
	void				*priv)
{
	...
	ASSERT(next_rgno >= info->rtg->rtg_rgno);
	 ...
	 */
	if (next_rgno > info->rtg->rtg_rgno)
		return 0;
	...
	/* Emit a record for the in-use space. */
	irec.rc_startblock = next_rgbno;
	irec.rc_blockcount = rec_rgbno - next_rgbno;
	irec.rc_refcount = 1;
	irec.rc_domain = XFS_REFC_DOMAIN_SHARED;
	rec_daddr = xfs_rtb_to_daddr(mp, next_rtb);

	return xfs_fsrefs_helper(tp, info, &irec, rec_daddr, 0);
}
	...
	xfs_rgblock_t			rgbno)
{
	...
	xfs_rtblock_t			end_rtb =
			xfs_rgbno_to_rtb(mp, info->rtg->rtg_rgno, rgbno);
	xfs_rtxnum_t			low_rtx;
	xfs_daddr_t			rec_daddr;
	int				error;

	ASSERT(xfs_rtb_to_rgno(mp, start_rtb) == info->rtg->rtg_rgno);

	low_rtx = xfs_rtb_to_rtx(mp, start_rtb);
	if (rgbno == -1U) {
		/*
		...
		high.ar_startext = xfs_rtb_to_rtx(mp, end_rtb);
		if (xfs_rtb_to_rtxoff(mp, end_rtb) > 0)
			high.ar_startext++;
	}
	if (low_rtx >= high.ar_startext)
		return 0;

	error = xfs_rtalloc_query_range(info->rtg, tp, low_rtx,
			high.ar_startext, xfs_fsrefs_rtdev_bitmap_helper,
			info);
	if (error)
		return error;
	 ...
	 * fsrefcount record emitted and the end of the query range.
	 */
	high.ar_startext = min(high.ar_startext, info->rtg->rtg_extents);
	rec_daddr = XFS_FSB_TO_BB(mp,
			xfs_rtx_to_rtb(info->rtg, high.ar_startext));
	if (info->next_daddr > rec_daddr)
		return 0;

	info->last = true;
	return xfs_fsrefs_rtdev_bitmap_helper(info->rtg, tp, &high, info);
}
	...
	struct xfs_fsrefs_info	*info)
{
	...
	if (keys[0].fcr_physical >= eofs)
		return 0;
	start_rtb = xfs_daddr_to_rtb(mp, keys[0].fcr_physical);
	end_rtb = xfs_daddr_to_rtb(mp, min(eofs - 1, keys[1].fcr_physical));
	 ...
	 * rtgroup.
	 */
	info->low.rc_startblock = xfs_rtb_to_rgbno(mp, start_rtb);
	info->low.rc_blockcount = XFS_BB_TO_FSBT(mp, keys[0].fcr_length);
	info->low.rc_refcount = 0;
	info->low.rc_domain = XFS_REFC_DOMAIN_SHARED;
	...
	/* Query each rtgroup */
	rgno = start_rg;
	for_each_rtgroup_range(mp, rgno, end_rg, rtg) {
		/*
	...
	void				*priv)
{
	struct xfs_refcount_irec	irec = {
		.rc_refcount		= 1,
		.rc_domain		= XFS_REFC_DOMAIN_SHARED,
	};
	struct xfs_mount		*mp = rtg->rtg_mount;
	struct xfs_fsrefs_info		*info = priv;
	xfs_rtblock_t			next_rtb, rec_rtb, rtb;
	...
	rec_daddr = xfs_rtb_to_daddr(mp, next_rtb);
	len_daddr = xfs_rtb_to_daddr(mp, rec_rtb - next_rtb);
	...
	struct xfs_fsrefs_info	*info)
{
	 ...
	 */
	low_rtx = xfs_rtb_to_rtx(mp, start_rtb);
	high_rtx = xfs_rtb_to_rtx(mp, end_rtb);
	if (xfs_rtb_to_rtxoff(mp, end_rtb) > 0)
		high_rtx++;
	if (low_rtx < high_rtx)

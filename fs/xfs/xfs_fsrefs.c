	...
	xfs_daddr_t			rec_daddr)
{
	...
	trace_xfs_fsrefs_mapping(mp, info->dev, info->agno, rec);
	...
	void				*priv)
{
	...
	ASSERT(next_agno >= cur->bc_ag.agno);
	 ...
	 */
	if (next_agno > cur->bc_ag.agno)
		return 0;

	info->next_daddr = XFS_AGB_TO_DADDR(mp, cur->bc_ag.agno,
	...
	irec.rc_refcount = 1;
	rec_daddr = XFS_AGB_TO_DADDR(mp, cur->bc_ag.agno, irec.rc_startblock);
	...
	xfs_agblock_t			agbno)
{
	...
	ASSERT(xfs_daddr_to_agno(mp, info->next_daddr) == cur->bc_ag.agno);
	...
	/*
	 * Synthesize a record if there's a gap between the last record and the
	 * end, since that space is in use.
	 */
	...
	void				*priv)
{
	...
	rec_daddr = XFS_AGB_TO_DADDR(mp, cur->bc_ag.agno, rec->rc_startblock);
	info->next_daddr = XFS_AGB_TO_DADDR(mp, cur->bc_ag.agno,
	...
	struct xfs_fsrefs_info	*info)
{
	...
	if (keys[0].fcr_physical >= eofs)
		return 0;
	if (keys[1].fcr_physical >= eofs)
		keys[1].fcr_physical = eofs - 1;
	start_fsb = XFS_DADDR_TO_FSB(mp, keys[0].fcr_physical);
	end_fsb = XFS_DADDR_TO_FSB(mp, keys[1].fcr_physical);
	...
	for (info->agno = start_ag; info->agno <= end_ag; info->agno++) {
		...
		error = xfs_alloc_read_agf(mp, tp, info->agno, 0, &agf_bp);
		if (error)
			goto err;

		trace_xfs_fsrefs_low_key(mp, info->dev, info->agno, &info->low);
		trace_xfs_fsrefs_high_key(mp, info->dev, info->agno,
				&info->high);

		info->bno_cur = xfs_allocbt_init_cursor(mp, tp, agf_bp,
	...
	struct xfs_fsrefs_info		*info)
{
	...
	trace_xfs_fsrefs_low_key(mp, info->dev, info->agno, &info->low);
	trace_xfs_fsrefs_high_key(mp, info->dev, info->agno, &info->high);
	...
	xfs_rtblock_t		rtbno)
{
	 ...
	 * end, since that space is in use.
	 */
	low.ar_startext = min_t(xfs_rtblock_t, rtbno, mp->m_sb.sb_rblocks);
	do_div(low.ar_startext, mp->m_sb.sb_rextsize);
	low.ar_extcount = 0;
	return xfs_fsrefs_rtdev_bitmap_helper(tp, &low, info);
}
	...
	struct xfs_fsrefs_info	*info)
{
	...
	end_fsb = XFS_BB_TO_FSB(mp, keys[1].fcr_physical);

	...
	const struct xfs_fsrefs_irec	*frec)
{
	...
	if (info->pag)
		...
		trace_xfs_fsrefs_mapping(mp, info->dev, NULLAGNUMBER, frec);
	...
	void				*priv)
{
	...
	frec.start_daddr = XFS_AGB_TO_DADDR(mp, cur->bc_ag.pag->pag_agno,
					    next_agbno);
	...
	struct xfs_fsrefs_info	*info)
{
	...
	for_each_perag_range(mp, agno, end_ag, pag) {
		...
		trace_xfs_fsrefs_low_group_key(mp, info->dev, pag->pag_agno,
				&info->low);
		trace_xfs_fsrefs_high_group_key(mp, info->dev, pag->pag_agno,
				&info->high);
	...
	xfs_rgblock_t			rgbno)
{
	...
	xfs_rtblock_t			end_rtbno =
			xfs_rgbno_to_rtb(info->rtg, rgbno);
	xfs_rtxnum_t			low_rtx;
	xfs_daddr_t			rec_daddr;
	int				error;

	ASSERT(xfs_rtb_to_rgno(mp, start_rtbno) == info->rtg->rtg_rgno);
	...
	void				*priv)
{
	...
	/* Report the refcount record from the refcount btree. */
	rec_rtbno = xfs_rgbno_to_rtb(cur->bc_ino.rtg, rec->rc_startblock);
	...
	struct xfs_fsrefs_info	*info)
{
	...
	for_each_rtgroup_range(mp, rgno, end_rg, rtg) {
		...
		trace_xfs_fsrefs_low_group_key(mp, info->dev, rtg->rtg_rgno,
				&info->low);
		trace_xfs_fsrefs_high_group_key(mp, info->dev, rtg->rtg_rgno,
				&info->high);

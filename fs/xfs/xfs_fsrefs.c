	...
	xfs_rtblock_t		end_rtb)
{
	 ...
	 * fsrefcount record emitted and the end of the query range.
	 */
	high.ar_startext = min(high.ar_startext, mp->m_sb.sb_rextents);
	...
	struct xfs_fsrefs_info	*info)
{
	...
	/* Synthesize refcount==1 records from the free space data. */
	return xfs_fsrefs_rtdev_fill_refcount_gap(tp, info, start_rtb,
			end_rtb);
}
	...
	struct xfs_fsrefs_info	*info)
{
	...
	eofs = XFS_FSB_TO_BB(mp, xfs_blen_to_rtbxlen(mp, mp->m_sb.sb_rextents));
	if (keys[0].fcr_physical >= eofs)
		return 0;
...
#else
/* XXX this code is really broken */
STATIC int
xfs_fsrefs_rtdev(
	struct xfs_trans	*tp,
	const struct xfs_fsrefs	*keys,
...
#define xfs_fsrefs_rtdev_rtbitmap	xfs_fsrefs_rtdev
#endif

	...
	bool			*did_zero)
{
	 ...
	 *
	 * Alternate designs could be: (a) zeroing all multi-fsblock COW
	 * staging extents at allocation time; (b) don't allow sharing of an
	 * allocation unit that spans EOF because of the unwritten blocks; (c)
	 * rewrite the reflink code to allow shared unwritten extents in this
	 * one corner case; or (d) insert zeroed pages into the pagecache to
	 * get around the checks in iomap_zero_range
	 *
	...
	return iomap_zero_range(inode, pos, len, did_zero,
			&xfs_buffered_write_iomap_ops, ac, zeroing_flags);
}

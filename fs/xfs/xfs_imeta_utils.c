	...
	struct xfs_imeta_update		*upd)
{
	...
	return xfs_parent_start(mp, &upd->parent);
}
...
xfs_imeta_end_update(
	...
	int				error)
{
	trace_xfs_imeta_end_update(upd, error);
	...
	xfs_parent_finish(upd->mp, upd->parent);
}
	...
	struct xfs_imeta_update		*upd)
{
	...
	xfs_imeta_end_update(upd, error);
	return error;
}
	...
	const struct xfs_imeta_path	*path)
{
	...
	error = xfs_imeta_start_create(mp, path, true, &upd);
	if (error)
		return error;
	...
	const struct xfs_imeta_path	*path)
{
	...
	/*
	 * Make sure the root dquots are loaded before we create more
	 * root-owned metadata subdirectories.
	 */
	error = xfs_qm_dqattach(mp->m_metadirip);
	if (error)
		return error;

...
DEFINE_INODE_EVENT(xfs_alloc_free_space);
...
TRACE_EVENT(xfs_bmapi_freesp,
	...
	TP_printk("dev %d:%d ino 0x%llx size 0x%llx bno 0x%llx len 0x%x",
		  MAJOR(__entry->dev), MINOR(__entry->dev),
		  __entry->ino,
		  __entry->size,
		  __entry->bno,
...
DECLARE_EVENT_CLASS(xfs_rtextent_class,
	...
	TP_printk("dev %d:%d rtx 0x%llx len 0x%llx",
		  MAJOR(__entry->dev), MINOR(__entry->dev),
		  __entry->off_rtx,
		  __entry->len_rtx)
);
...
DECLARE_EVENT_CLASS(xfs_imeta_resv_class,
	...
	TP_printk("dev %d:%d freeblks %llu ino 0x%llx resv %llu ask %llu used %llu len %llu",
		  MAJOR(__entry->dev), MINOR(__entry->dev),
		  __entry->freeblks,
		  __entry->ino,
		  __entry->reserved,
...
DEFINE_AG_ERROR_EVENT(xfs_imeta_resv_init_error);
...
DECLARE_EVENT_CLASS(xfs_refcount_extent_class,
	...
	TP_printk("dev %d:%d agno %u bno %llu len %llu refcount %u",
		  MAJOR(__entry->dev), MINOR(__entry->dev),
		  __entry->agno,
		  __entry->startblock,
		  __entry->blockcount,
...
DECLARE_EVENT_CLASS(xfs_refcount_extent_at_class,
	TP_PROTO(struct xfs_mount *mp, xfs_agnumber_t agno,
		 struct xfs_refcount_irec *irec, xfs_fsblock_t bno),
	TP_ARGS(mp, agno, irec, bno),
	TP_STRUCT__entry(
		__field(dev_t, dev)
		__field(xfs_agnumber_t, agno)
		__field(xfs_fsblock_t, startblock)
	...
	TP_printk("dev %d:%d agno %u bno %llu len %llu refcount %u @ bno %llu",
		  MAJOR(__entry->dev), MINOR(__entry->dev),
		  __entry->agno,
		  __entry->startblock,
		  __entry->blockcount,
...
DECLARE_EVENT_CLASS(xfs_refcount_double_extent_at_class,
	...
	TP_printk("dev %d:%d agno %u bno %llu len %llu refcount %u -- "
		  "bno %llu len %llu refcount %u @ bno %llu",
		  MAJOR(__entry->dev), MINOR(__entry->dev),
		  __entry->agno,
		  __entry->i1_startblock,
		  __entry->i1_blockcount,
...
TRACE_EVENT(xfs_refcount_finish_one_leftover,
	TP_PROTO(struct xfs_mount *mp, xfs_agnumber_t agno,
		 int type, xfs_fsblock_t bno, xfs_extlen_t len,
		 xfs_fsblock_t new_bno, xfs_extlen_t new_len),
	TP_ARGS(mp, agno, type, bno, len, new_bno, new_len),
	TP_STRUCT__entry(
		__field(dev_t, dev)
		__field(xfs_agnumber_t, agno)
		__field(int, type)
		__field(xfs_fsblock_t, bno)
		__field(xfs_filblks_t, len)
		__field(xfs_fsblock_t, new_bno)
		__field(xfs_filblks_t, new_len)
	),
	TP_fast_assign(
		__entry->dev = mp->m_super->s_dev;
		__entry->agno = agno;
		...
		__entry->bno = bno;
		__entry->len = len;
		__entry->new_bno = new_bno;
		__entry->new_len = new_len;
	),
	TP_printk("dev %d:%d type %d agno %u bno %llu len %llu new_bno %llu new_len %llu",
		  MAJOR(__entry->dev), MINOR(__entry->dev),
		  __entry->type,
		  __entry->agno,
		  __entry->bno,
...
DECLARE_EVENT_CLASS(xfs_fsrefs_class,
	...
	TP_printk("dev %d:%d keydev %d:%d agno %d bno %llu len %llu owners %llu",
		  MAJOR(__entry->dev), MINOR(__entry->dev),
		  MAJOR(__entry->keydev), MINOR(__entry->keydev),
		  __entry->agno,
		  __entry->bno,
...
DECLARE_EVENT_CLASS(xfs_getfsrefs_class,
	...
	TP_printk("dev %d:%d keydev %d:%d block %llu len %llu owners %lld flags 0x%x",
		  MAJOR(__entry->dev), MINOR(__entry->dev),
		  MAJOR(__entry->keydev), MINOR(__entry->keydev),
		  __entry->block,
		  __entry->len,
...
DECLARE_EVENT_CLASS(xfs_ag_noalloc_class,
	...
	TP_printk("dev %d:%d agno %u freeblks %u flcount %u btreeblks %u metaresv %u rmapresv %u resblks %llu resblks_avail %llu",
		  MAJOR(__entry->dev), MINOR(__entry->dev),
		  __entry->agno,
		  __entry->freeblks,
		  __entry->flcount,

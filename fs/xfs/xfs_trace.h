...
TRACE_EVENT(xfbuf_create,
	TP_PROTO(struct xfbuf *xm),
	TP_ARGS(xm),
	TP_STRUCT__entry(
		__field(dev_t, dev)
	...
	TP_fast_assign(
		...
		__entry->ino = file_inode(xm->file)->i_ino;
		memset(pathname, 0, sizeof(pathname));
		path = file_path(xm->file, pathname, sizeof(pathname) - 1);
		if (IS_ERR(path))
			path = "(unknown)";
...
TRACE_EVENT(xfbuf_destroy,
	...
	TP_fast_assign(
		struct inode *inode = file_inode(xm->file);

...
TRACE_EVENT(xfs_healthmon_copybuf,
	...
	TP_STRUCT__entry(
		__field(dev_t, dev)
		__field(size_t, seqbuf_pos)
		__field(size_t, seqbuf_len)
		__field(size_t, iter_count)
	),
	TP_fast_assign(
		__entry->dev = mp ? mp->m_super->s_dev : 0;
		__entry->seqbuf_pos = outpos;
		__entry->seqbuf_len = seqbuf->len - outpos;
		__entry->iter_count = iov_iter_count(iov);
	),
	TP_printk("dev %d:%d pos %zu len %zu iter_count %zu",
		  MAJOR(__entry->dev), MINOR(__entry->dev),
		  __entry->seqbuf_pos,
		  __entry->seqbuf_len,
		  __entry->iter_count)
...
DEFINE_EVENT(xfs_healthmon_class, name, \
	TP_PROTO(const struct xfs_mount *mp, unsigned int events, bool lost_prev), \
	TP_ARGS(mp, events, lost_prev))
DEFINE_HEALTHMON_EVENT(xfs_healthmon_read);

...
TRACE_EVENT(xfs_verify_media,
	TP_PROTO(const struct xfs_mount *mp, const struct xfs_verify_media *me,
		 ...
		 unsigned int iosize, unsigned int bufsize),
	TP_ARGS(mp, me, fdev, bio_daddr, bio_bbcount, iosize, bufsize),
	TP_STRUCT__entry(
		__field(dev_t, dev)
		__field(dev_t, fdev)
		__field(xfs_daddr_t, start_daddr)
	...
	TP_fast_assign(
		...
		__entry->iosize = iosize;
		__entry->bufsize = bufsize;
	),
	TP_printk("dev %d:%d fdev %d:%d start_daddr 0x%llx end_daddr 0x%llx flags 0x%x bio_daddr 0x%llx bio_bbcount 0x%llx iosize 0x%x bufsize 0x%x",
		  MAJOR(__entry->dev), MINOR(__entry->dev),

...
TRACE_EVENT(xfs_btree_free_block,
	...
	TP_fast_assign(
		__entry->dev = cur->bc_mp->m_super->s_dev;
		__entry->agno = xfs_daddr_to_agno(cur->bc_mp, bp->b_bn);
		if (cur->bc_flags & XFS_BTREE_ROOT_IN_INODE)
			__entry->ino = cur->bc_ino.ip->i_ino;
		else
			__entry->ino = 0;
		__entry->btnum = cur->bc_btnum;
		__entry->agbno = xfs_daddr_to_agbno(cur->bc_mp, bp->b_bn);
	),
...
DEFINE_INODE_ERROR_EVENT(xfs_map_freesp_trans_alloc_fail);

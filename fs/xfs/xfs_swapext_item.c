	...
	struct xfs_sxi_log_item	*sxi_lip)
{
	...
	if (atomic_dec_and_test(&sxi_lip->sxi_refcount)) {
		xfs_trans_ail_remove(&sxi_lip->sxi_item, SHUTDOWN_LOG_IO_ERROR);
...

{
	...
	sxi_lip = kmem_zone_zalloc(xfs_sxi_zone, 0);
	...
	struct xfs_sxi_log_item		*sxi_lip)
{
	...
	sxd_lip = kmem_zone_zalloc(xfs_sxd_zone, 0);
	...
	struct xfs_btree_cur		**state)
{
	 ...
	 */
	error = xfs_trans_log_finish_swapext_update(tp, SXD_ITEM(done), sxi);
	...
	struct xfs_defer_capture	**dfcp)
{
	 ...
	 */
	sx = &sxi_lip->sxi_format.sxi_extent;
	if (sx->sx_blockcount == 0 ||
	    sxi_lip->sxi_format.__pad != 0 ||
	    !xfs_verify_ino(mp, sx->sx_inode1) ||
	    !xfs_verify_ino(mp, sx->sx_inode2) ||
	    (sx->sx_flags & ~XFS_SWAP_EXTENT_FLAGS) ||
	    ((sx->sx_flags & XFS_SWAP_EXTENT_SET_SIZES) &&
	...
	error = xfs_iget(mp, tp, sx->sx_inode1, 0, 0, &sxi.sxi_ip1);
	if (error)
		goto out_fail;
	error = xfs_iget(mp, tp, sx->sx_inode2, 0, 0, &sxi.sxi_ip2);
	if (error)
		goto out_fail;

	xfs_lock_two_inodes(sxi.sxi_ip1, XFS_ILOCK_EXCL,
			    sxi.sxi_ip2, XFS_ILOCK_EXCL);
	xfs_trans_ijoin(tp, sxi.sxi_ip1, 0);
	xfs_trans_ijoin(tp, sxi.sxi_ip2, 0);
	 ...
	 */
	if (VFS_I(sxi.sxi_ip1)->i_nlink == 0)
		xfs_iflags_set(sxi.sxi_ip1, XFS_IRECOVERY);
	if (VFS_I(sxi.sxi_ip2)->i_nlink == 0)
		xfs_iflags_set(sxi.sxi_ip2, XFS_IRECOVERY);
	...
	error = xfs_trans_log_finish_swapext_update(tp, sxd_lip, &sxi);
	if (error)
		goto out_fail;
	...
	if (error || !more_work)
		goto out_rele;
	error = xfs_defer_capture_inode(*dfcp, sxi.sxi_ip2);
	if (error)
		goto out_rele1;
	return xfs_defer_capture_inode(*dfcp, sxi.sxi_ip1);

out_fail:
	xfs_trans_cancel(tp);
out_rele:
	if (sxi.sxi_ip2) {
		xfs_iunlock(sxi.sxi_ip2, XFS_ILOCK_EXCL);
		xfs_irele(sxi.sxi_ip2);
	}
out_rele1:
	if (sxi.sxi_ip1) {
		xfs_iunlock(sxi.sxi_ip1, XFS_ILOCK_EXCL);

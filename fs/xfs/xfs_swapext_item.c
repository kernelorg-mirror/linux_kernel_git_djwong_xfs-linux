	...
	struct xfs_defer_capture	**dfcp)
{
	...
	error = xfs_trans_alloc(mp, &M_RES(mp)->tr_itruncate, res.resblks, 0,
			0, &tp);
	if (error)
		goto err_rele2;
	...
	if (error)
		goto err_unlock2;
	error = xfs_defer_capture_inode(*dfcp, sxi.sxi_ip1);
	if (error) {
		/* The freezer owns ip2, so we can only touch ip1. */
